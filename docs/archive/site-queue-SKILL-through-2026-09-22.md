# Archived: the site-queue skill as it stood through 2026-09-22

Moved verbatim out of `.claude/skills/site-queue/SKILL.md` on 2026-09-23 (visibility audit
PROCESS-8). The skill was 601 lines, 51 of them dated incident notes, and it restated caps,
the rise floor, cadence and a land path that other files contradicted. The live protocol is
`docs/RUN_LOOP.md`; the live skill keeps only lane mechanics. The 70 finish line and the
`demoMatch` completion gate below were retired by Matt on 2026-09-23. Read this file for the
history behind a rule or a named failure, not as instructions.
