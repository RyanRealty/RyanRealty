# Archived handoff blocks, 2026-09-15 through 2026-09-22

Moved verbatim out of `docs/plans/CROSS_AGENT_HANDOFF.md` on 2026-09-23 (visibility audit,
PROCESS-8). That file had grown back to 52 stacked `# Current` blocks against its
one-block rule, and the loop brief printed only the first 18 lines of the stack, which hid
six of Matt's ten 2026-09-22 directives at boot. The still-open Matt directives from these
blocks are listed in the single Current block that replaced them; `ci:handoff-current`
now refuses a second `# Current` block. Older history:
`docs/archive/CROSS_AGENT_HANDOFF-through-2026-09-12.md`.

Nothing below was edited. Read it for a named SHA, branch, or PR.
