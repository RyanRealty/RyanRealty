...
**Generated:** 2026-09-25T17:45:27.314Z
...
**Exports:** `ACTIVE_INVENTORY_CITIES`, `ACTIVE_INVENTORY_METHODOLOGY`, `ACTIVE_INVENTORY_GEO`, `activeInventoryCitySlug`, `inventorySnapshotHttpStatus`, `snapshotActiveInventory`

**Tables:** `listings`, `analytics_inventory_snapshot`
...
### `lib/data/blog/getAllPublishedBlogRefs.ts`

**Exports:** `PUBLISHED_BLOG_REF_CAP`, `toPublishedBlogRefs`, `getAllPublishedBlogRefs`

**Tables:** `blog_posts`

**Selected columns:** `slug`, `title`, `excerpt`, `published_at`, `tags`, `category`
...
### `lib/data/brokers/directory.ts`

**Exports:** `ensureBrokerDirectory`, `getCrmMailboxes`, `ownMailboxForSlug`, `mailboxForSlug`

**Tables:** `brokers`

**Selected columns:** `crm_slug`, `display_name`, `email`, `crm_active`
...
### `lib/data/brokers/workspace-sync.ts`

**Exports:** `listWorkspaceUsers`, `runWorkspaceBrokerSync`, `recordTeamPageDecision`
...
### `lib/data/cma/areaUnsoldReads.ts`

**Exports:** `UNSOLD_STATUSES`, `UNSOLD_MAX_MONTHS`, `areaFilterTrace`, `getCmaAreaUnsoldCycles`
...
**Exports:** `getCmaBandInventory`, `getCmaAreaBandInventory`
...
**Exports:** `CMA_ADMIN_REVIEW_COLUMNS`, `getCmaAdminRowBySlug`, `getCmaAdminReviewRowBySlug`, `getCmaProspectAsk`, `getCmaServeHead`, `getCmaStoredHtmlBySlug`, `getCmaRenderSourceBySlug`, `getCmaHtmlBySlug`, `updateCmaRowFieldsBySlug`, `deleteCmaRowById`, `snapshotCmaVersion`, `replaceCmaComps`, `getCmaAccessIdentity`
...
### `lib/data/cma/localOutcomeReads.ts`

**Exports:** `CMA_SFR_FILTER`, `CMA_FAILED_STATUSES`, `getCmaCityClosedOutcomes`, `getCmaCityFailedOutcomes`, `getCmaListingPriceEvents`, `getClosedCompListStarts`, `getCmaCityFailedCycles`, `getCmaCityClosedSales`, `CMA_LOCAL_OUTCOME_READS`

**Tables:** `listings`, `price_history`, `listing_history`

**Selected columns:** `old_price`, `new_price`, `changed_at`, `event`, `event_date`, `price`, `description`, `ListingKey`, `OnMarketDate`, `ListDate`, `original_entry_timestamp`, `original_on_market_timestamp`, `listing_key`

**TTL windows:** `CACHE_WINDOWS.marketStats`

**Cache tags:** `cacheTag.listings, cacheTag.market`, `cacheTag.listings`
...
**Tables:** `cmas`, `crm_timeline`, `visitor_sessions`, `visitor_events`, `crm_people`

**Selected columns:** `id`, `slug`, `person_id`, `client_email`, `delivered_at`, `ts`, `session_id`, `crm_person_id`, `page_url`, `event_at`, `event_type`, `stage`
...
### `lib/data/cma/rural-coverage.ts`

**Exports:** `getRuralFieldCoverage`, `getRuralZoningValues`

**Tables:** `listings`

**Selected columns:** `ListingKey`, `zoning:details->>Zoning`, `horse_yn`, `fencing`, `public_remarks`, `lot_size_acres`

---

### `lib/data/cma/sale-zone-cache.ts`

**Exports:** `getSaleZoneCache`, `upsertSaleZoneCache`

**Tables:** `cma_sale_zone_cache`

**Selected columns:** `listing_key`, `lat`, `lng`, `zone`, `zone_type`, `source`
...
**Exports:** `getResortCommunityBySlug`, `getAllResortCommunities`, `getResortCommunitiesForCity`, `getCanonicalCityForSubdivision`, `getResortCommunityBySubdivisionName`, `isVerifiedRegistryChild`
...
### `lib/data/crawl-probe/rows.ts`

**Exports:** `CRAWL_PROBE_CHANNEL`, `CRAWL_PROBE_SCOPE`, `CRAWL_PROBE_SOURCE`, `CRAWL_PROBE_STALE_HOURS`, `upsertCrawlProbeRows`, `readCrawlProbeBaseline`, `readCrawlProbeStatus`, `formatCrawlProbeLine`

**Tables:** `marketing_channel_daily`

**Selected columns:** `date`, `scope_id`, `metadata`, `value`, `fetched_at`
...
**Selected columns:** `id`, `custom`, `kind`, `ts`, `broker`, `source`, `payload`
...
**Selected columns:** `person_id`, `id`, `tags`, `custom`, `addresses`, `stage`, `source`
...
**Selected columns:** `id`, `crm_slug`, `display_name`, `email`, `phone`, `twilio_number`, `title`, `crm_active`, `routing_eligible`, `sms_agent_enabled`
...
### `lib/data/crm/getSiteActivity.ts`

**Exports:** `ACTIVITY_WINDOWS`, `readListingFacts`, `getActiveKnownPeople`, `getPersonSiteActivity`

**Tables:** `visitor_sessions`, `visitor_events`, `listings`, `crm_people`

**Selected columns:** `ListNumber`, `StreetNumber`, `StreetName`, `City`, `ListPrice`, `StandardStatus`, `id`, `name`, `first_name`, `last_name`, `stage`, `assigned_broker`, `tags`, `emails`
...
### `lib/data/crm/gmailBounceWatch.ts`

**Exports:** `listWatchedSentEvents`, `getEmailKeyFlags`, `insertEmailBounceTimeline`

**Tables:** `email_events`, `crm_timeline`

**Selected columns:** `id`, `message_id`, `recipient_email`, `person_id`, `email_key`, `broker`, `subject`, `send_type`, `occurred_at`, `meta`, `event`
...
**Cache keys:** `plat-parent-community-v1`
...
**Exports:** `taxlotSourceFor`, `taxlotSourceCounties`, `getTaxlotsNear`, `getTaxlotsInBoundary`
...
### `lib/data/geo/nearby-on-market-homes.ts`

**Exports:** `NEARBY_MEDIAN_MIN_N`, `publishNearbyMedianListPrice`, `searchMvRowToNearbyTile`, `summarizeNearbyHomes`, `fetchOnMarketHomesInBox`, `fetchOnMarketHomesForSchool`
...
### `lib/data/geo/place-on-market-stock.ts`

**Exports:** `getBoundaryOnMarketKeys`, `getBoundaryChildRows`, `getSubdivisionOnMarketRows`

**Tables:** `listing_boundary_xref_mv`

**Selected columns:** `listing_key`, `geo_slug`, `property_type`, `property_sub_type`
...
**Exports:** `resolvePlaceContextFromListing`

---

### `lib/data/geo/subdivision-ring-cached.ts`

**Exports:** `getSubdivisionRingCached`
...
**Exports:** `getSubdivisionRing`, `readSubdivisionRing`, `assignSubdivisionSlugs`

---

### `lib/data/geo/taxlot-disclaimer.ts`

**Exports:** `TAXLOT_DISCLAIMER`
...
### `lib/data/identity/sessionIdentity.ts`

**Exports:** `readSessionIdentity`, `isMissingColumnError`, `insertVisitorSession`, `identifySessionAndBrowser`, `clearProvisionalAutomation`, `readIdentityMapForVid`, `stampSessionEmailOnly`

**Tables:** `visitor_sessions`, `visitor_identity_map`

**Selected columns:** `crm_person_id`, `identified_at`, `session_id`, `${LEGACY_PERSON_COL}`, `email`
...
**Exports:** `parseActivityPriceDrop`, `loadRecentPriceDropEvents`, `attachListingCardExtras`

**Tables:** `activity_events`, `listings`

**Selected columns:** `listing_key`, `event_at`, `payload`, `ListingKey`, `original_list_price`, `virtual_tour_url`, `ListOfficeName`, `PhotoURL`, `details`

---

### `lib/data/listings/dedupeListingTilesByStreet.ts`

**Exports:** `dedupeListingTilesByStreet`
...
### `lib/data/listings/getBoundarySentinelCoverage.ts`

**Exports:** `BOUNDARY_CITY_SENTINEL`, `getBoundarySentinelCoverage`

**Tables:** `listings`

**Selected columns:** `ListingKey`

**Cache keys:** `boundary-sentinel-coverage-v1`
...
### `lib/data/listings/getLeaseRateOptions.ts`

**Exports:** `getLeaseRateOptions`

**Tables:** `listings`

**Selected columns:** `ListingKey`, `rate:details->>`
...
**Exports:** `getListingTiles`, `getListingTilesOrThrow`, `getTotalListingCount`, `getListingTilesCount`, `getCityListings`, `getCommunityListings`, `getZipListings`, `getNeighborhoodListings`
...
**Exports:** `getPlaceOpeningListings`
...
**Tables:** `listing_search_mv`, `subdivision_descriptions`, `neighborhoods`

**Selected columns:** `entity_key`, `description`, `name`, `slug`, `cities(slug)`
...
### `lib/data/listings/listingCanonicalPathCore.ts`

**Exports:** `LISTING_CANONICAL_PATH_COLUMNS`, `mayDisplayListingPublicly`, `mapListingCanonicalPathRow`, `listingCanonicalPathFromFields`
...
**Exports:** `arrayLiteral`, `ilikeExact`, `orLiteral`, `tokenScalarOrExpr`, `BOOLEAN_PREDICATES`, `BOOLEAN_FILTER_KEYS`, `MULTI_FIELD_DEFS`, `LEGACY_PROPERTY_SUB_TYPE_MAP`, `resolveLegacyPropertySubType`, `TEXT_FIELD_COLUMNS`, `RANGE_FIELD_COLUMNS`, `listedWithinDaysCutoff`, `listedWithinDaysCeiling`
...
**Exports:** `SERVICE_AREA_CITIES_LOWER`, `SERVICE_AREA_CITIES_PROPER`, `isServiceAreaCity`, `outOfAreaListingPolicy`

---

### `lib/data/loop/answer-engine-citations.test.ts`

**Tables:** `site_signal`

---

### `lib/data/loop/answer-engine-citations.ts`

**Exports:** `foldAnswerEngineRows`, `readAnswerEngineCitations`, `ranThisMonth`

**Tables:** `site_signal`

**Selected columns:** `date`, `surface`, `metric`, `value`, `scope`
...
### `lib/data/loop/gsc-api.ts`

**Exports:** `GSC_PAGE_SIZE`, `GSC_SETTLE_DAYS`, `gscSiteUrl`, `createGscQuery`, `pullAllGscRows`, `isoDate`, `addDays`, `daysBetweenInclusive`, `settledEndDate`, `isIsoDate`, `chunkDateRange`

---

### `lib/data/loop/gsc-page-class.ts`

**Exports:** `GSC_PAGE_CLASSES`, `GSC_MONEY_CLASSES`, `normalizeGscPath`, `classifyGscPage`, `isCentralOregonMoneyClass`, `isTrackableQuery`, `normalizeGscQuery`

---

### `lib/data/loop/gsc-ranking-seed.ts`

**Exports:** `MAX_DRAFTS`, `HIGH_IMP`, `TITLE_META_IMP`, `RESEED_AFTER_DAYS`, `ctr`, `nextSiteNumber`, `formatSiteGap`, `normalizeQuery`, `WINNERS`, `LANDING_BUCKETS`, `OTHER_CLASS_MODIFIERS`, `queryStaysInWinnerClass`, `winnerFor`, `diagnose`, `pathOf`, `alreadySeeded`, `kindLabel`, `toDraft`, `formatDrafts`, `cannibalsFromLandings`, `cannibalsFromQueryPages`, `dropTrackerQueries`, `buildGscGapDrafts`, `CLASS_ROUTE`, `buildClassSlipDrafts`, `seedWindow`, `pullGscSeedRows`, `readSeedContext`, `insertGscGapNodes`, `seedClassSlipNodes`

**Tables:** `loop_work_nodes`, `target_queries`

**Selected columns:** `version_gap`, `title`, `state`, `updated_at`, `query`, `segment`, `priority`, `target_url`, `id`

---

### `lib/data/loop/gsc-store.ts`

**Exports:** `GSC_STORE_MIGRATION`, `isMissingRelationError`, `foldPageRows`, `foldQueryPageRows`, `pullGscStoreRange`, `writeGscStore`, `readGscStoreCoverage`, `readGscClassRollup`, `readGscStorePageTotals`

**Tables:** `gsc_page_daily`

**Selected columns:** `date`, `page`, `clicks`, `impressions`

---

### `lib/data/loop/gsc-trend.ts`

**Exports:** `DEGRADED_IMPRESSIONS_DROP_PCT`, `DEGRADED_POSITION_WORSE`, `DEGRADED_MIN_PRIOR_IMPRESSIONS`, `STORE_STALE_DAYS`, `trendWindows`, `diffRollups`, `isDegradedClass`, `degradedReason`, `topMovers`, `formatClassDelta`, `GSC_TREND_SOURCE`, `buildGscTrend`, `readGscTrend`
...
### `lib/data/loop/learn-close.ts`

**Exports:** `GSC_SNAPSHOT_SOURCE`, `learnWindow`, `isWindowSettled`, `surfacePaths`, `surfaceUrlVariants`, `verdictFor`, `measureLedgerRow`, `closeLedgerRowWith`, `liveGscPageTotals`, `learnDueWindows`

**Tables:** `site_signal`, `web_vitals`, `site_improvement_ledger`

**Selected columns:** `surface`, `metric`, `value`, `date`, `id`

---

### `lib/data/loop/ledger-draft.ts`

**Exports:** `assertLedgerDraft`, `windowEndsAt`, `isLedgerRowOpen`, `isExpiredUnlearned`
...
**Selected columns:** `id`, `shipped_at`, `window_days`, `actual_delta`, `verdict`, `domain`, `change_class`, `surface`, `description`, `metric`, `baseline_value`, `predicted_delta`, `measured_at`, `commit_sha`
...
### `lib/data/loop/place-membership-freshness.ts`

**Exports:** `PLACE_MEMBERSHIP_STALE_HOURS`, `classifyPlaceMembershipFreshness`, `readPlaceMembershipFreshness`, `formatPlaceMembershipLine`

**Tables:** `mv_refresh_state`, `place_membership`

**Selected columns:** `refreshed_at`, `computed_at`

---

### `lib/data/loop/scoreboard-snapshot.ts`

**Exports:** `SNAPSHOT_STALE_DAYS`, `writeScoreboardSnapshot`, `readLatestScoreboardSnapshot`, `formatMeasurerBrief`

**Tables:** `loop_scoreboard_snapshots`

**Selected columns:** `id`, `taken_at`, `source`, `gsc_status`, `gsc`, `signals`, `run`
...
**Selected columns:** `stage`, `id`, `status`, `last_delta_sync_at`, `last_full_sync_at`, `gci`, `domain`, `actual_delta`, `verdict`, `shipped_at`, `window_days`, `methodology_version`, `query`, `refreshed_at`, `rr_vid`, `endpoint`, `response_status`, `logged_at`

---

### `lib/data/loop/silent-zero.ts`

**Exports:** `KNOWN_DORMANT`, `RETIRED_METRICS`, `classifyFeed`, `formatSilentZeroReport`, `WATCHED_SERIES`, `watchKey`, `classifyWatchedSeries`, `formatWatchReport`

---

### `lib/data/loop/site-queue-done.ts`

**Exports:** `SITE_DONE_RISE_FLOOR`, `SITE_DONE_RISE_FLOOR_FROM`, `resolveSiteQueueKit`, `regressionTolerance`, `tasteFloorProblems`, `siteQueueDoneEvidenceProblems`
...
### `lib/data/loop/weekly-measure.ts`

**Exports:** `STORE_TREND_DAYS`, `STORE_MAX_DAYS_PER_RUN`, `STORE_OVERLAP_DAYS`, `storePullRange`, `runWeeklyMeasure`

---

### `lib/data/loop/work-graph.ts`

**Exports:** `createWorkNode`, `listWorkNodes`, `listStaleInProgressNodes`, `touchWorkNode`, `countSiteClaims`, `claimWorkNode`, `claimShipClass`, `blockWorkNode`, `releaseWorkNode`, `completeWorkNode`, `resolvePunchLines`, `killWorkNode`

**Tables:** `loop_work_nodes`

**Selected columns:** `id`, `state`, `domain`, `version_gap`, `title`, `objective`

---

### `lib/data/loop/work-node.ts`

**Exports:** `WORK_NODE_STATES`, `assertWorkNodeDraft`, `isLegalTransition`, `assertTransition`, `siteServeTier`, `fleetNodePriority`, `STALE_IN_PROGRESS_DAYS`, `SITE_CLAIM_IDLE_HOURS`, `isSiteClaim`, `MAX_SITE_CLAIMS_PER_SESSION`, `MAX_SITE_WORKERS`, `isStaleInProgress`, `isMeasurementWindowDue`, `isCloudAgentSession`, `ORPHAN_GRACE_MIN`, `shouldAutoRelease`
...
### `lib/data/market-truth/getListingCutFacts.ts`

**Exports:** `CUT_FACTS_WINDOW_MONTHS`, `getListingCutFacts`

**Cache tags:** `'market-truth', 'listing-cut-facts'`
...
### `lib/data/market-truth/public-segment-view.ts`

**Exports:** `PUBLIC_PLACE_SEGMENTS`, `publicSegmentNoun`, `publicSegmentFilterParams`, `publicSegmentBrowseHref`, `publicSegmentVerdictLabel`, `formatPaceShare`

---

### `lib/data/market-truth/public-segments.ts`

**Exports:** `PUBLIC_SEGMENT_LEFTOVER_STATS`, `PUBLIC_SEGMENT_STATS`, `publicSegmentDisplayBits`, `publicSegmentItems`, `getPublicPlaceSegments`
...
### `lib/data/mv/getTileRefreshInProgress.ts`

**Exports:** `getTileRefreshInProgress`
...
**Cache keys:** `park-detail-v2-full-set`
...
### `lib/data/places/getPlaceAmenityLayers.ts`

**Exports:** `getPlaceAmenityLayers`

**TTL windows:** `CACHE_WINDOWS.geoCity`

**Cache tags:** `cacheTag.listings, 'boundaries', 'trails', 'trail-lines', 'parks'`
...
**Exports:** `PLACE_DOCUMENTS_BUCKET`, `PUBLISHABLE_KINDS`, `sortPlaceDocuments`, `getPlaceDocuments`, `getPlaceDocumentsByPlatLabel`
...
**Exports:** `recordingFaceText`, `recordingLabel`, `documentKindLabel`, `summarizePlaceDocuments`, `placeDocumentsLead`
...
**Exports:** `countSalePricingFacts`, `selectPricingFactsPool`, `selectPricingFactsNear`, `getPricingMarketIndex`, `getPricingSubdivisionCells`, `getListingWaterSource`
...
**Selected columns:** `email_key`, `event`, `occurred_at`, `meta`, `person_id`, `ts`, `page_url`, `event_at`

**Cache keys:** `prospecting-engagement-expired-v3`, `prospecting-engagement-fsbo-v3`, `doc-engagement-v3`
...
### `lib/data/schools/getPlaceSchools.ts`

**Exports:** `PLACE_SCHOOL_MIN_SHARE`, `placeSchoolsFromRows`, `getPlaceSchools`
...
**Cache keys:** `school-detail-v2-full-set`
...
### `lib/data/subdivisions/getPlatBoundaryCity.ts`

**Exports:** `walkPlatBoundaryCity`, `getPlatBoundaryCity`
...
### `lib/data/subdivisions/getPlatClosedCounts.ts`

**Exports:** `getPlatClosedCounts`, `getPlatClosedCount`

**Tables:** `subdivision_plat_closed_mv`

**Selected columns:** `plat_slug`, `plat_label`, `closed_count`, `closed_in_polygon`, `closed_by_name`, `closed_count_sfr`, `top_city_lower`, `last_close_date`, `closed_by_year`
...
**Exports:** `registryFamilyCommunities`, `reservedPlaceSlugs`, `assemblePlatFamilies`, `getPlatFamilies`
...
### `lib/data/subdivisions/getPlatFamilyFootprint.ts`

**Exports:** `getPlatFamilyFootprint`, `pointInGeometry`, `countClosedInside`, `getPlatFamilyClosedSales`

**Tables:** `listing_tile_mv_src`

**Selected columns:** `listing_key`, `lat`, `lng`, `close_date`

**TTL windows:** `CACHE_WINDOWS.marketStats`

**Cache tags:** `cacheTag.community(slug), 'boundaries'`, `cacheTag.market, 'boundaries'`

---

### `lib/data/subdivisions/getPlatFamilyInventory.ts`

**Exports:** `rollupPlatFamilyInventory`, `getPlatFamilyInventory`
...
### `lib/data/subdivisions/getPlatFootprintTaxlots.ts`

**Exports:** `getPlatFootprintTaxlots`

---

### `lib/data/subdivisions/getPlatUnsoldOutcomes.ts`

**Exports:** `mapPlatUnsoldRow`, `getPlatUnsoldOutcome`

**Tables:** `subdivision_plat_unsold_mv`

**Selected columns:** `geo_slug`, `geo_label`, `unsold_count`, `unsold_in_polygon`, `unsold_by_name`, `median_days_listed`, `days_sample`, `cut_count`, `median_cut_pct`, `window_start`, `window_end`
...
**Exports:** `readRecordedPlatLabel`, `getRecordedPlatLabel`

**Tables:** `boundaries`

**Selected columns:** `geo_label`
...
### `lib/data/subdivisions/getRecordedPlatTree.ts`

**Exports:** `buildRecordedPlatTree`, `getRecordedPlatTree`

**Tables:** `boundaries`

**Selected columns:** `id`, `geo_type`, `geo_slug`, `geo_label`, `parent_id`
...
**Exports:** `citySlugToMlsCityLower`, `getSubdivisionCityInventory`, `getSubdivisionCityInventoryByCityLower`, `getSubdivisionBrowsePairsByCity`

**Tables:** `subdivision_city_inventory_mv`

**Selected columns:** `city_lower`, `subdivision_name`, `listing_count`, `status_counts`

**TTL windows:** `CACHE_WINDOWS.marketStats`

**Cache tags:** `cacheTag.listings, cacheTag.market`

---

### `lib/data/subdivisions/getSubdivisionFootprint.ts`

**Exports:** `PLAT_POLYGON_SOURCE`, `platMemberSlugs`, `parseFootprintRow`, `footprintProvenance`, `getSubdivisionFootprint`

**TTL windows:** `CACHE_WINDOWS.geoCommunity`

**Cache tags:** `cacheTag.community(slug), 'boundaries'`
...
### `lib/data/tc/client-transactions.ts`

**Exports:** `getClientIdentity`, `hasClientDeals`, `listClientDeals`, `getClientDeal`, `clientDocumentUrl`, `getPortalSigningRecipient`

**Tables:** `crm_contact_points`, `profiles`, `tc_deal_people`, `tc_envelopes`, `tc_envelope_recipients`, `tc_deals`, `tc_cycles`, `tc_tasks`, `tc_documents`, `tc_deal_contacts`, `tc_events`, `brokers`

**Selected columns:** `person_id`, `value`, `crm_person_id`, `deal_id`, `role`, `id`, `name`, `status`, `sent_at`, `cycle_id`, `envelope_id`, `email`, `action_required`, `signing_order`, `completed_at`, `declined_at`, `address`, `city`, `stage`, `broker_name`, `updated_at`, `kind`, `listing_date`, `contract_acceptance_date`, `escrow_closing_date`, `actual_closing_date`, `expiration_date`, `inspection_days`, `financing_days`, `dead_date`, `created_at`, `title`, `detail`, `due_date`, `assignee_email`, `executed_document_id`, `ingested_at`, `company`, `phone`, `action` (+8 more)
...
### `lib/data/tc/continuation.ts`

**Exports:** `nextAddendumNumber`, `sectionsOf`, `applyEnvelopeSectionText`, `_internal`, `renderContinuationPreview`, `getPacketSections`, `getPacketDealBroker`

**Tables:** `tc_documents`, `tc_envelopes`, `tc_envelope_documents`, `tc_envelope_fields`, `tc_envelope_recipients`, `tc_cycles`, `tc_form_versions`, `tc_events`, `tc_form_templates`

**Selected columns:** `id`, `sha256`, `classification`, `cycle_id`, `status`, `document_id`, `form_version_id`, `sort_order`, `recipient_id`, `type`, `page`, `x`, `y`, `w`, `h`, `value`, `role`, `action_required`, `signing_order`, `name`, `buyers`, `sellers`, `tc_deals(address`, `city`, `state`, `zip)`, `storage_path`, `field_map`, `tc_form_libraries(code)`, `updated_at`, `blank_pdf_storage_path`, `tc_form_libraries!inner(code)`, `tc_cycles(tc_deals(broker_name))`

**Cache keys:** `tc-document-facts-v1`

**Cache tags:** `'tc-document-facts'`
...
### `lib/data/tc/cycle-term-writes.ts`

**Exports:** `stampedTermsPatch`, `writeCycleTermsByPerson`

**Tables:** `tc_cycles`, `tc_events`

**Selected columns:** `term_provenance`, `deal_id`

---

### `lib/data/tc/deal-audit.ts`

**Exports:** `listDealAudits`
...
### `lib/data/tc/deal-subdivisions.ts`

**Exports:** `getSubdivisionNamesByMlsNumbers`

**Tables:** `listings`

**Selected columns:** `ListNumber`, `SubdivisionName`

---

### `lib/data/tc/deal-terms.ts`

**Exports:** `MAX_TERMS_ATTEMPTS`, `MAX_TIEBREAK_ATTEMPTS`, `wantsTiebreak`, `termsFormsForDocument`, `readDocumentTerms`, `pendingTermsDocuments`, `getCycleTerms`, `applyCycleTerms`, `acceptContractValue`, `keepFileValue`, `getTermsReviewQueue`, `confirmTermReading`

**Tables:** `tc_document_readings`, `tc_documents`, `tc_cycles`, `tc_events`

**Selected columns:** `anatomy`, `verdict`, `id`, `name`, `storage_path`, `sha256`, `archived`, `classification`, `cycle_id`, `ingested_at`, `reader_version:classification->reader->>version`, `forms:classification->reader->forms`, `terms_version:classification->terms->>version`, `terms_status:classification->terms->>status`, `terms_attempts:classification->terms->>attempts`, `terms_error:classification->terms->>error`, `terms_disagreements:classification->terms->disagreements`, `terms_tiebreak:classification->terms->tiebreak`, `tc_deals!inner(stage)`, `deal_id`, `updated_at`, `term_provenance`, `tc_deals(property_key`, `broker_name)`, `tc_deals(property_key)`, `${CYCLE_TERM_SELECT}`, `actual_closing_date`, `tc_deals!inner(property_key`, `address`, `stage`, `tc_cycles(tc_deals(property_key))`

---

### `lib/data/tc/document-review.ts`

**Exports:** `listDocumentReview`, `countDocumentReview`

**Tables:** `tc_events`, `tc_documents`, `tc_deals`

**Selected columns:** `document_id`, `deal_id`, `action`, `actor`, `detail`, `created_at`, `id`, `name`, `archived`, `classification`, `address`, `property_key`, `stage`, `broker_name`
...
**Exports:** `listEnvelopeSigningRoster`, `listEnvelopeAddressBook`

**Tables:** `tc_envelope_recipients`

**Selected columns:** `role`, `action_required`, `signing_order`, `completed_at`, `id`, `name`, `email`, `declined_at`
...
**Selected columns:** `id`, `property_key`, `address`, `broker_name`, `stage`, `deal_id`, `kind`, `source`, `contract_acceptance_date`, `cycle_id`, `name`, `sort_order`, `item_id`, `document_id`
...
### `lib/data/tc/mail-coverage.ts`

**Exports:** `getMailCoverage`

**Tables:** `tc_mail_review_cursors`

**Selected columns:** `mailbox`, `listed`, `started_at`, `finished_at`

---

### `lib/data/tc/mail-reads.ts`

**Exports:** `listDealMail`, `listMailQueue`, `countMailQueue`, `listDealConversations`, `getMailMessagesForAction`, `listDealOptions`, `getDealScopeRow`, `getDocumentDealScope`

**Tables:** `tc_deals`, `tc_mail_messages`, `tc_deal_people`, `tc_cycles`, `crm_timeline`, `tc_documents`

**Selected columns:** `id`, `address`, `property_key`, `stage`, `person_id`, `crm_people(name)`, `listing_date`, `contract_acceptance_date`, `created_at`, `ts`, `kind`, `title`, `body`, `broker`, `payload`, `status`, `gmail_refs`, `subject`, `deal_id`, `message_key`, `gmail_thread_ids`, `sent_at`, `broker_name`, `updated_at`, `name`, `cycle_id`, `tc_cycles(deal_id`, `tc_deals(broker_name`, `property_key))`
...
### `lib/data/tc/reader-health.ts`

**Exports:** `STALL_AFTER_MS`, `checkReaderHealth`

**Tables:** `tc_document_readings`

**Selected columns:** `created_at`

---

### `lib/data/tc/review-documents.ts`

**Exports:** `getReviewDocuments`

**Tables:** `tc_documents`

**Selected columns:** `id`, `name`, `storage_path`, `page_count`, `bytes`, `source_uploaded_at`, `archived`, `classification`

---

### `lib/data/tc/signing-session.ts`

**Exports:** `findSigningRecipient`, `getSigningDeal`, `getSigningDocuments`, `getEnvelopeFieldsForSigning`, `updateSigningRecipient`, `saveSignerValues`, `voidEnvelopeOnDecline`, `logSigningEvent`, `findSharedAddressNextSigner`

**Tables:** `tc_envelope_recipients`, `tc_cycles`, `tc_envelope_documents`, `tc_documents`, `tc_envelope_fields`, `tc_envelopes`, `tc_events`

**Selected columns:** `id`, `name`, `email`, `phone`, `role`, `signing_order`, `consented_at`, `viewed_at`, `completed_at`, `declined_at`, `auth_token_hash`, `tc_envelopes(id`, `status`, `cycle_id`, `require_text_code`, `created_by)`, `tc_deals(address`, `property_key)`, `document_id`, `sort_order`, `page_count`, `storage_path`, `recipient_id`, `type`, `page`, `x`, `y`, `w`, `h`, `required`, `value`, `signed_at`, `label`, `group_key`, `group_min`, `group_max`, `action_required`, `auth_token_enc`

---

### `lib/data/tc/signing-token-vault.ts`

**Exports:** `mintOrReuseSigningLink`

**Tables:** `tc_envelope_recipients`

---

### `lib/data/tc/skyslope-intake.ts`

**Exports:** `tryTakeSkySlopeIntakeLease`, `releaseSkySlopeIntakeLease`, `writeSkySlopeIntakeRunLog`, `runSkySlopeVaultIntake`

**Tables:** `sync_logs`, `tc_deals`, `tc_cycles`, `tc_documents`, `tc_checklist_items`, `tc_checklist_assignments`, `tc_deal_contacts`, `tc_events`

**Selected columns:** `id`, `property_key`, `address`, `city`, `stage`, `stage_detail`, `source_doc_id`, `archived`, `source_activity_id`, `status`, `item_id`, `document_id`, `role`, `email`, `name`, `source_contact_guid`
...
### `lib/data/videos/getListingCardVideo.ts`

**Exports:** `getListingCardVideo`
...
| Table | DAL functions |
|---|---|
| `activity_events` | `parseActivityPriceDrop()`, `loadRecentPriceDropEvents()`, `attachListingCardExtras()`, `tileAndEventToDrop()`, `getPriceDrops()`, `getPriceDropDigest()`, `PENDING_SOURCE_COVERAGE_START_ISO()`, `isPendingWindowCovered()`, `getWentPendingInWindow()`, `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()` <br /> `lib/data/listings/attachListingCardExtras.ts` · `lib/data/listings/getPriceDrops.ts` · `lib/data/listings/getWentPendingInWindow.ts` · `lib/data/sync/syncWrites.ts` |
...
| `admin_roles` | `listWorkspaceUsers()`, `runWorkspaceBrokerSync()`, `recordTeamPageDecision()` <br /> `lib/data/brokers/workspace-sync.ts` |
...
| `analytics_inventory_snapshot` | `ACTIVE_INVENTORY_CITIES()`, `ACTIVE_INVENTORY_METHODOLOGY()`, `ACTIVE_INVENTORY_GEO()`, `activeInventoryCitySlug()`, `inventorySnapshotHttpStatus()`, `snapshotActiveInventory()` <br /> `lib/data/analytics/snapshotActiveInventory.ts` |
...
| `blog_posts` | `MATT_BROKER_ID()`, `publishBlogPost()`, `PUBLISHED_BLOG_REF_CAP()`, `toPublishedBlogRefs()`, `getAllPublishedBlogRefs()`, `getBlogPostBySlug()`, `getBlogPostsBySlugs()`, `getPopularBlogSlugs()`, `getPublishedBlogPosts()`, `getRecentBlogPosts()`, `getRelatedBlogPosts()`, `getMattBrokerRecord()`, `getBrokerSelfRecord()`, `getBrokerSelfRecordByEmail()`, `updateBrokerById()`, `getBrokerBySlug()`, `getBrokerForOgBySlug()`, `getBlogPostForOgBySlug()`, `searchBrokersByDisplayName()`, `getBrokers()`, `searchSiteContentTitles()` <br /> `lib/data/blog/blogPostWrites.ts` · `lib/data/blog/getAllPublishedBlogRefs.ts` · `lib/data/blog/getBlogPostBySlug.ts` · `lib/data/blog/getBlogPostsBySlugs.ts` · `lib/data/blog/getPopularBlogSlugs.ts` · `lib/data/blog/getPublishedBlogPosts.ts` · `lib/data/blog/getRecentBlogPosts.ts` · `lib/data/blog/getRelatedBlogPosts.ts` · `lib/data/brokers/getBrokers.ts` · `lib/data/search/searchSiteContentTitles.ts` |
| `boundaries` | `getCrmNeighborhoodOptions()`, `collectCompanyScoreboardSignals()`, `walkPlatBoundaryCity()`, `getPlatBoundaryCity()`, `readRecordedPlatLabel()`, `getRecordedPlatLabel()`, `buildRecordedPlatTree()`, `getRecordedPlatTree()`, `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()` <br /> `lib/data/crm/getCrmNeighborhoodOptions.ts` · `lib/data/loop/signals.ts` · `lib/data/subdivisions/getPlatBoundaryCity.ts` · `lib/data/subdivisions/getRecordedPlatLabel.ts` · `lib/data/subdivisions/getRecordedPlatTree.ts` · `lib/data/sync/syncWrites.ts` |
...
| `brokers` | `isSmsAgentBrokerSlug()`, `isBrokerSmsAgentEnvEnabled()`, `isAgentEnabledForBroker()`, `setAgentEnabled()`, `rangeToIso()`, `personName()`, `personHref()`, `readPeople()`, `readSessions()`, `countAudienceSessions()`, `readAccountSnapshots()`, `readDeals()`, `readWorkingPersonIds()`, `sellerClientPersonIds()`, `readActiveBrokers()`, `readJoinConverts()`, `namesForIds()`, `getBlogPostBySlug()`, `getPublishedBlogPosts()`, `ensureBrokerDirectory()`, `getCrmMailboxes()`, `ownMailboxForSlug()`, `mailboxForSlug()`, `getMattBrokerRecord()`, `getBrokerSelfRecord()`, `getBrokerSelfRecordByEmail()`, `updateBrokerById()`, `getBrokerBySlug()`, `getBrokerForOgBySlug()`, `getBlogPostForOgBySlug()`, `searchBrokersByDisplayName()`, `getBrokers()`, `getDayOneChecklist()`, `resolveCrmSlugForAccess()`, `listWorkspaceUsers()`, `runWorkspaceBrokerSync()`, `recordTeamPageDecision()`, `findCmaSubjectByMls()`, `findCmaSubjectByAddress()`, `getListingPhotosCount()`, `selectCmaCompsPool()`, `selectCmaCompsByKeys()`, `getCmaMarketStatsRow()`, `getCmaMarketPulseRow()`, `CMA_MARKET_TREND_MEASURE()`, `getCmaMarketTrendRows()`, `getCmaBrokerBySlugOrEmail()`, `listActiveBrokersForCma()`, `getCmaCityClosedSkinny()`, `getCmaCityClosedDuring()`, `getLikeHomeSales()`, `getCmaSubdivisionClosed()`, `getCmaSubdivisionHistory()`, `getCmaPriorSaleAtAddress()`, `resolveSigningBrokerForPerson()`, `resolveDateRange()`, `getAgentActivityReport()`, `getAgentGoalsReport()`, `getAppointmentsReport()`, `getBatchEmailsReport()`, `getBrokerNotifyPrefs()`, `countBrokerAlertsLast24h()`, `getBrokerTelephony()`, `CALL_LOGS_PAGE_SIZE()`, `getCallLogsReport()`, `getCallsReport()`, `getContactAttemptsReport()`, `mapCrmBrokerRow()`, `mapCrmBrokerRows()`, `getCrmBrokers()`, `getCrmBrokerBySlug()`, `getLeadSourcesReport()`, `getOverviewReport()`, `getSpeedToLeadReport()`, `getTextsReport()`, `collectCompanyScoreboardSignals()`, `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/agent/broker-agent-flags.ts` · `lib/data/analytics/salesFunnelRead.ts` · `lib/data/blog/getBlogPostBySlug.ts` · `lib/data/blog/getPublishedBlogPosts.ts` · `lib/data/brokers/directory.ts` · `lib/data/brokers/getBrokers.ts` · `lib/data/brokers/getDayOneChecklist.ts` · `lib/data/brokers/resolveCrmSlug.ts` · `lib/data/brokers/workspace-sync.ts` · `lib/data/cma/builderReads.ts` · `lib/data/cma/signing-broker.ts` · `lib/data/crm/getAgentActivityReport.ts` · `lib/data/crm/getAgentGoalsReport.ts` · `lib/data/crm/getAppointmentsReport.ts` · `lib/data/crm/getBatchEmailsReport.ts` · `lib/data/crm/getBrokerNotifyPrefs.ts` · `lib/data/crm/getBrokerTelephony.ts` · `lib/data/crm/getCallLogsReport.ts` · `lib/data/crm/getCallsReport.ts` · `lib/data/crm/getContactAttemptsReport.ts` · `lib/data/crm/getCrmBrokers.ts` · `lib/data/crm/getLeadSourcesReport.ts` · `lib/data/crm/getOverviewReport.ts` · `lib/data/crm/getSpeedToLeadReport.ts` · `lib/data/crm/getTextsReport.ts` · `lib/data/loop/signals.ts` · `lib/data/tc/client-transactions.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
...
| `listing_boundary_xref_mv` | `bendNeighborhoodCanonicalHref()`, `neighborhoodGeoSlug()`, `medianListPrice()`, `rollupNeighborhoodPublicInventory()`, `getBendNeighborhoodPublicInventory()`, `getNeighborhoodPublicInventory()`, `getBoundaryOnMarketKeys()`, `getBoundaryChildRows()`, `getSubdivisionOnMarketRows()`, `rollupPlatFamilyInventory()`, `getPlatFamilyInventory()` <br /> `lib/data/geo/neighborhood-public-inventory.ts` · `lib/data/geo/place-on-market-stock.ts` · `lib/data/subdivisions/getPlatFamilyInventory.ts` |
| `listing_history` | `getListingHistoryRowCount()`, `getActiveNeedingHistoryCount()`, `getHistoryFinalizedCount()`, `getHistoryVerifiedFullCount()`, `getFinalizedUnverifiedCount()`, `getTerminalBucketTotal()`, `getTerminalBucketFinalized()`, `getClosedFinalizedListingRows()`, `getListingHistoryTableStatus()`, `getAllListingsCount()`, `getStatusIlikeCount()`, `getPendingNonContingentCount()`, `getActiveBucketCount()`, `getTerminalBucketStrictBacklog()`, `CMA_SFR_FILTER()`, `CMA_FAILED_STATUSES()`, `getCmaCityClosedOutcomes()`, `getCmaCityFailedOutcomes()`, `getCmaListingPriceEvents()`, `getClosedCompListStarts()`, `getCmaCityFailedCycles()`, `getCmaCityClosedSales()`, `CMA_LOCAL_OUTCOME_READS()`, `getHeroPhotosByListingKeys()`, `getOpenHousesInRange()`, `getListingDetailPhotos()`, `getListingKeysForBrokerByLicense()`, `getListingKeysForBrokerByEmail()`, `getListingKeysByListAgentEmail()`, `getListingDetailAgents()`, `getOpenHouseById()`, `getListingDetailOpenHouses()`, `getListingDetailVideos()`, `getListingKeysWithPriceChangeSince()`, `seedListingDetailHistory()`, `getListingPriceHistory()`, `getListingDetailHistory()`, `resolveCommunityChainBySlug()`, `selectListingHistoryForKey()`, `selectNewExpiredListings()`, `getExistingExpiredListingKeys()`, `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()` <br /> `lib/data/admin/syncCounts.ts` · `lib/data/cma/localOutcomeReads.ts` · `lib/data/listings/getListingDetailBundles.ts` · `lib/data/sync/expiredSelect.ts` · `lib/data/sync/syncWrites.ts` |
...
| `listing_search_mv` | `searchAdminListingsRemarks()`, `DSCR_DEFAULTS()`, `DEAL_SCORE_WEIGHTS()`, `applyDealScores()`, `CENTRAL_OREGON_COUNTIES()`, `getDscrScreen()`, `NEARBY_MEDIAN_MIN_N()`, `publishNearbyMedianListPrice()`, `searchMvRowToNearbyTile()`, `summarizeNearbyHomes()`, `fetchOnMarketHomesInBox()`, `fetchOnMarketHomesForSchool()`, `getSearchMatrixInventory()`, `getSubdivisionLifetimeCounts()`, `getSubdivisionDescriptionKeys()`, `getMatrixNeighborhoods()`, `SEARCH_FEATURE_FILTER_KEYS()`, `pickSearchFeatureFilters()`, `searchListingsAll()`, `searchListingsAllCount()`, `ShapesSchema()`, `mvRowToTile()`, `fetchSearchListingsAllInShapes()`, `countSearchListingsAllInShapes()`, `normalizeShapesForCacheKey()` <br /> `lib/data/admin/remarksSearch.ts` · `lib/data/dscr/screen.ts` · `lib/data/geo/nearby-on-market-homes.ts` · `lib/data/listings/getSearchMatrixInventory.ts` · `lib/data/listings/searchListingsAll.ts` · `lib/data/listings/searchShapes.ts` |
| `listing_tile_mv` | `filtersToParamMap()`, `otherFilterChips()`, `enabledEventTypes()`, `summarizeAreaShapes()`, `activityLabel()`, `toClientPortalAlert()`, `collectAreaIds()`, `getClientPortalView()`, `rollupSavedHomeRows()`, `buildHomesPanelUnion()`, `buildSavedHomesIdentityOrFilter()`, `getContactSavedHomes()`, `getOwnedHomeMatches()`, `getOwnedHomePlace()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `platInventoryKey()`, `platCityAliases()`, `rowMatchesPlat()`, `isDisplayablePlatName()`, `registryChildPlats()`, `rollupPlatPublicInventory()`, `getRegistryPlatPublicInventory()`, `getPlatPublicInventory()`, `getAtlasTiles()`, `getListingTiles()`, `getListingTilesOrThrow()`, `getTotalListingCount()`, `getListingTilesCount()`, `getCityListings()`, `getCommunityListings()`, `getZipListings()`, `getNeighborhoodListings()`, `EMPTY_SITE_INDEX()`, `getSiteIndexLinks()`, `getDerivedPopularSearches()`, `serviceAreaSitemapTiles()`, `getListingSitemapRows()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()` <br /> `lib/data/crm/getClientPortalView.ts` · `lib/data/crm/getContactSavedHomes.ts` · `lib/data/crm/getOwnedHome.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/geo/plat-public-inventory.ts` · `lib/data/listings/getAtlasTiles.ts` · `lib/data/listings/getListingTiles.ts` · `lib/data/seo/getSiteIndexLinks.ts` · `lib/data/sitemap/getListingSitemapRows.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/envelope-composer-reads.ts` |
| `listing_tile_mv_src` | `toPrefixTsQuery()`, `searchListingSuggestTiles()`, `getPlatFamilyFootprint()`, `pointInGeometry()`, `countClosedInside()`, `getPlatFamilyClosedSales()` <br /> `lib/data/listings/searchSuggestTiles.ts` · `lib/data/subdivisions/getPlatFamilyFootprint.ts` |
...
| `listings` | `getAdminEditableListingRow()`, `updateAdminEditableListingRow()`, `applyAdminOverridesToListingRow()`, `getListingPhotosForKey()`, `appendListingPhoto()`, `deleteListingPhoto()`, `setListingHeroPhoto()`, `reorderListingPhotos()`, `mergeListingRowsWithAdminOverrides()`, `getListingHistoryRowCount()`, `getActiveNeedingHistoryCount()`, `getHistoryFinalizedCount()`, `getHistoryVerifiedFullCount()`, `getFinalizedUnverifiedCount()`, `getTerminalBucketTotal()`, `getTerminalBucketFinalized()`, `getClosedFinalizedListingRows()`, `getListingHistoryTableStatus()`, `getAllListingsCount()`, `getStatusIlikeCount()`, `getPendingNonContingentCount()`, `getActiveBucketCount()`, `getTerminalBucketStrictBacklog()`, `PROPERTY_SHOOTS_BUCKET()`, `ensureShootsBucket()`, `uploadShootAsset()`, `findAssetBySourceId()`, `upsertAssetLibraryRow()`, `resolveListingLatLng()`, `searchPropertyCandidates()`, `getListingStateSignals()`, `getCoAgentShare()`, `getCoOfficeShare()`, `getCoOfficeShareMerged()`, `getRyanBrandShare()`, `PERMANENT_ZERO_MLS_CITY_LABELS()`, `analyticsClosedCityLabels()`, `normOfficeKey()`, `isMlsNoOffice()`, `buildOfficeDimIndex()`, `resolveOfficeId()`, `rebuildAnalyticsMarts()`, `ACTIVE_INVENTORY_CITIES()`, `ACTIVE_INVENTORY_METHODOLOGY()`, `ACTIVE_INVENTORY_GEO()`, `activeInventoryCitySlug()`, `inventorySnapshotHttpStatus()`, `snapshotActiveInventory()`, `getBpoListingCyclesByAddress()`, `getBpoAdminRowBySlug()`, `getBpoHtmlBySlug()`, `upsertBpoRowBySlug()`, `updateBpoRowFieldsBySlug()`, `deleteBpoRowById()`, `replaceBpoComps()`, `listBposForAdmin()`, `getBpoWorklistRowById()`, `getBrokerSales()`, `UNSOLD_STATUSES()`, `UNSOLD_MAX_MONTHS()`, `areaFilterTrace()`, `getCmaAreaUnsoldCycles()`, `getCmaBandInventory()`, `getCmaAreaBandInventory()`, `findCmaSubjectByMls()`, `findCmaSubjectByAddress()`, `getListingPhotosCount()`, `selectCmaCompsPool()`, `selectCmaCompsByKeys()`, `getCmaMarketStatsRow()`, `getCmaMarketPulseRow()`, `CMA_MARKET_TREND_MEASURE()`, `getCmaMarketTrendRows()`, `getCmaBrokerBySlugOrEmail()`, `listActiveBrokersForCma()`, `getCmaCityClosedSkinny()`, `getCmaCityClosedDuring()`, `getLikeHomeSales()`, `getCmaSubdivisionClosed()`, `getCmaSubdivisionHistory()`, `getCmaPriorSaleAtAddress()`, `CMA_SFR_FILTER()`, `CMA_FAILED_STATUSES()`, `getCmaCityClosedOutcomes()`, `getCmaCityFailedOutcomes()`, `getCmaListingPriceEvents()`, `getClosedCompListStarts()`, `getCmaCityFailedCycles()`, `getCmaCityClosedSales()`, `CMA_LOCAL_OUTCOME_READS()`, `getCmaMarketAreaRows()`, `getRuralFieldCoverage()`, `getRuralZoningValues()`, `getLookingAtNow()`, `getPersonGlance()`, `getPropertiesReport()`, `ACTIVITY_WINDOWS()`, `readListingFacts()`, `getActiveKnownPeople()`, `getPersonSiteActivity()`, `listExpiredOutreachQueue()`, `getExpiredOutreachRow()`, `getExpiredListingDetail()`, `getCmaExpiredLinks()`, `markExpiredOutreachSent()`, `parseActivityPriceDrop()`, `loadRecentPriceDropEvents()`, `attachListingCardExtras()`, `getAtlasTiles()`, `BOUNDARY_CITY_SENTINEL()`, `getBoundarySentinelCoverage()`, `getLeaseRateOptions()`, `getListingCanonicalPathFields()`, `getListingDetail()`, `getHeroPhotosByListingKeys()`, `getOpenHousesInRange()`, `getListingDetailPhotos()`, `getListingKeysForBrokerByLicense()`, `getListingKeysForBrokerByEmail()`, `getListingKeysByListAgentEmail()`, `getListingDetailAgents()`, `getOpenHouseById()`, `getListingDetailOpenHouses()`, `getListingDetailVideos()`, `getListingKeysWithPriceChangeSince()`, `seedListingDetailHistory()`, `getListingPriceHistory()`, `getListingDetailHistory()`, `resolveCommunityChainBySlug()`, `getListingEventStatesByKeys()`, `getListingPhotos()`, `getListingFloorPlans()`, `getListingRawRowByKey()`, `getListingVideoCandidates()`, `getMotivatedListings()`, `brokerageOfficeNames()`, `chooseBrokerageRows()`, `sortBrokerageListings()`, `getBrokerageListingTiles()`, `getBrokerageListings()`, `getPriceDropTiles()`, `getPropertyFactsByMls()`, `getRepeatSalesAppreciation()`, `resolveCanonicalListingKey()`, `toIsoDate()`, `to24hTime()`, `getUpcomingOpenHouses()`, `countSalePricingFacts()`, `selectPricingFactsPool()`, `selectPricingFactsNear()`, `getPricingMarketIndex()`, `getPricingSubdivisionCells()`, `getListingWaterSource()`, `proofWindow()`, `getProofBlock()`, `resolveDocsBatch()`, `resolveComplianceBatch()`, `verifyNotRelisted()`, `verifyFsboStillActive()`, `EXPIRED_OUTREACH_ON_MARKET()`, `EXPIRED_OUTREACH_STATUS_OR()`, `EXPIRED_OUTREACH_LISTING_SELECT()`, `normalizeParcelNumber()`, `parcelFromEnrichmentNotes()`, `expiredOutreachListingHits()`, `getProspectHardStop()`, `isRelistedNow()`, `isFsboRelistedNow()`, `resolveComplianceState()`, `EXPIRED_SELECT()`, `FSBO_SELECT()`, `prospectSelect()`, `prospectSelectLegacy()`, `shouldRetryWithoutEmailColumns()`, `markEmailOutreachColumnsAbsent()`, `numOrNull()`, `fetchExpiredListingJoinBatch()`, `resolvePersonId()`, `computeSendable()`, `engagementKeyFor()`, `finalizeRow()`, `mapExpiredSkeleton()`, `mapFsboSkeleton()`, `buildExpiredRowSkeleton()`, `buildFsboRowSkeleton()`, `getProspect()`, `ownershipYearsFromDate()`, `deriveOwnershipSince()`, `getExpiredOwnershipSince()`, `getProspectDetail()`, `getRecentStudioTriggers()`, `selectListingHistoryForKey()`, `selectNewExpiredListings()`, `getExistingExpiredListingKeys()`, `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()`, `getSubdivisionNamesByMlsNumbers()`, `getBrokerageTrackRecord()`, `getListingVideos()` <br /> `lib/data/admin/listingEdit.ts` · `lib/data/admin/syncCounts.ts` · `lib/data/agent/asset-registry.ts` · `lib/data/agent/resolve-property.ts` · `lib/data/analytics/getCoAgentShare.ts` · `lib/data/analytics/getCoOfficeShare.ts` · `lib/data/analytics/getCoOfficeShareMerged.ts` · `lib/data/analytics/getRyanBrandShare.ts` · `lib/data/analytics/rebuildAnalyticsMarts.ts` · `lib/data/analytics/snapshotActiveInventory.ts` · `lib/data/bpo/reads.ts` · `lib/data/brokers/getBrokerSales.ts` · `lib/data/cma/areaUnsoldReads.ts` · `lib/data/cma/bandInventory.ts` · `lib/data/cma/builderReads.ts` · `lib/data/cma/localOutcomeReads.ts` · `lib/data/cma/marketAreaReads.ts` · `lib/data/cma/rural-coverage.ts` · `lib/data/crm/getLookingAtNow.ts` · `lib/data/crm/getPersonGlance.ts` · `lib/data/crm/getPropertiesReport.ts` · `lib/data/crm/getSiteActivity.ts` · `lib/data/expired/outreach.ts` · `lib/data/listings/attachListingCardExtras.ts` · `lib/data/listings/getAtlasTiles.ts` · `lib/data/listings/getBoundarySentinelCoverage.ts` · `lib/data/listings/getLeaseRateOptions.ts` · `lib/data/listings/getListingCanonicalPathFields.ts` · `lib/data/listings/getListingDetail.ts` · `lib/data/listings/getListingDetailBundles.ts` · `lib/data/listings/getListingEventStates.ts` · `lib/data/listings/getListingPhotos.ts` · `lib/data/listings/getListingRawRow.ts` · `lib/data/listings/getListingVideoCandidates.ts` · `lib/data/listings/getMotivatedListings.ts` · `lib/data/listings/getPriceDropTiles.ts` · `lib/data/listings/getPropertyFactsByMls.ts` · `lib/data/listings/getRepeatSalesAppreciation.ts` · `lib/data/listings/resolveCanonicalListingKey.ts` · `lib/data/market/subdivision-stats.int.test.ts` · `lib/data/open-houses/getUpcomingOpenHouses.ts` · `lib/data/pricing/facts.ts` · `lib/data/proof/getProofBlock.ts` · `lib/data/prospecting/batch.ts` · `lib/data/prospecting/compliance.ts` · `lib/data/prospecting/get.ts` · `lib/data/studio/listing-photos.ts` · `lib/data/studio/triggers.ts` · `lib/data/sync/expiredSelect.ts` · `lib/data/sync/syncWrites.ts` · `lib/data/tc/deal-subdivisions.ts` · `lib/data/track-record.ts` · `lib/data/videos/getListingVideos.ts` |
| `loop_scoreboard_snapshots` | `SNAPSHOT_STALE_DAYS()`, `writeScoreboardSnapshot()`, `readLatestScoreboardSnapshot()`, `formatMeasurerBrief()` <br /> `lib/data/loop/scoreboard-snapshot.ts` |
| `loop_work_nodes` | `FLEET_PACKS()`, `isFleetPack()`, `packRunToken()`, `buildFleetPack()`, `FLEET_PUNCH_GAP()`, `FLEET_PUNCH_TITLE_BODY()`, `FLEET_PUNCH_OUTPUT()`, `FLEET_PUNCH_ACCEPT()`, `FLEET_PUNCH_CONTRACT()`, `fleetFingerprintTag()`, `objectiveHasFingerprint()`, `isFleetPunchListTitle()`, `isFleetPunchListNode()`, `findFleetPunchListNode()`, `fleetPunchListTitle()`, `punchTitleSeverity()`, `formatFleetPunchLine()`, `appendPunchLine()`, `parsePunchLines()`, `punchDispositionFingerprints()`, `openPunchLines()`, `canCompletePunchList()`, `formatPunchDisposition()`, `appendPunchDispositions()`, `regressGapOf()`, `isFoldableFleetSingle()`, `punchLineFromSingleNode()`, `initialPunchObjective()`, `mergeFleetIntake()`, `runFleetIntake()`, `MAX_DRAFTS()`, `HIGH_IMP()`, `TITLE_META_IMP()`, `RESEED_AFTER_DAYS()`, `ctr()`, `nextSiteNumber()`, `formatSiteGap()`, `normalizeQuery()`, `WINNERS()`, `LANDING_BUCKETS()`, `OTHER_CLASS_MODIFIERS()`, `queryStaysInWinnerClass()`, `winnerFor()`, `diagnose()`, `pathOf()`, `alreadySeeded()`, `kindLabel()`, `toDraft()`, `formatDrafts()`, `cannibalsFromLandings()`, `cannibalsFromQueryPages()`, `dropTrackerQueries()`, `buildGscGapDrafts()`, `CLASS_ROUTE()`, `buildClassSlipDrafts()`, `seedWindow()`, `pullGscSeedRows()`, `readSeedContext()`, `insertGscGapNodes()`, `seedClassSlipNodes()`, `LOOP_SENTINEL_DEFAULT_OFF()`, `isLoopSentinelDisarmed()`, `runLoopSentinel()`, `createWorkNode()`, `listWorkNodes()`, `listStaleInProgressNodes()`, `touchWorkNode()`, `countSiteClaims()`, `claimWorkNode()`, `claimShipClass()`, `blockWorkNode()`, `releaseWorkNode()`, `completeWorkNode()`, `resolvePunchLines()`, `killWorkNode()` <br /> `lib/data/loop/fleet-cases.ts` · `lib/data/loop/fleet-intake-core.ts` · `lib/data/loop/gsc-ranking-seed.ts` · `lib/data/loop/sentinel.ts` · `lib/data/loop/work-graph.ts` |
...
| `marketing_channel_daily` | `getGscScopeAggregate()`, `getGscAccountTotals()`, `rangeToIso()`, `personName()`, `personHref()`, `readPeople()`, `readSessions()`, `countAudienceSessions()`, `readAccountSnapshots()`, `readDeals()`, `readWorkingPersonIds()`, `sellerClientPersonIds()`, `readActiveBrokers()`, `readJoinConverts()`, `namesForIds()`, `CRAWL_PROBE_CHANNEL()`, `CRAWL_PROBE_SCOPE()`, `CRAWL_PROBE_SOURCE()`, `CRAWL_PROBE_STALE_HOURS()`, `upsertCrawlProbeRows()`, `readCrawlProbeBaseline()`, `readCrawlProbeStatus()`, `formatCrawlProbeLine()` <br /> `lib/data/analytics/getGscMetrics.ts` · `lib/data/analytics/salesFunnelRead.ts` · `lib/data/crawl-probe/rows.ts` |
...
| `place_document_link` | `PENDING_GROUP_PAGE_SIZE()`, `isGoverningDocKind()`, `pendingKindLabel()`, `getPendingPlaceDocuments()`, `PLACE_DOCUMENTS_BUCKET()`, `PUBLISHABLE_KINDS()`, `sortPlaceDocuments()`, `getPlaceDocuments()`, `getPlaceDocumentsByPlatLabel()`, `publishPlaceDocumentGroup()`, `rejectPlaceDocumentGroup()` <br /> `lib/data/places/getPendingPlaceDocuments.ts` · `lib/data/places/getPlaceDocuments.ts` · `lib/data/places/reviewPlaceDocumentLinks.ts` |
| `place_membership` | `PLACE_MEMBERSHIP_STALE_HOURS()`, `classifyPlaceMembershipFreshness()`, `readPlaceMembershipFreshness()`, `formatPlaceMembershipLine()` <br /> `lib/data/loop/place-membership-freshness.ts` |
| `price_history` | `CMA_SFR_FILTER()`, `CMA_FAILED_STATUSES()`, `getCmaCityClosedOutcomes()`, `getCmaCityFailedOutcomes()`, `getCmaListingPriceEvents()`, `getClosedCompListStarts()`, `getCmaCityFailedCycles()`, `getCmaCityClosedSales()`, `CMA_LOCAL_OUTCOME_READS()`, `getHeroPhotosByListingKeys()`, `getOpenHousesInRange()`, `getListingDetailPhotos()`, `getListingKeysForBrokerByLicense()`, `getListingKeysForBrokerByEmail()`, `getListingKeysByListAgentEmail()`, `getListingDetailAgents()`, `getOpenHouseById()`, `getListingDetailOpenHouses()`, `getListingDetailVideos()`, `getListingKeysWithPriceChangeSince()`, `seedListingDetailHistory()`, `getListingPriceHistory()`, `getListingDetailHistory()`, `resolveCommunityChainBySlug()`, `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()` <br /> `lib/data/cma/localOutcomeReads.ts` · `lib/data/listings/getListingDetailBundles.ts` · `lib/data/sync/syncWrites.ts` |
| `pricing_market_index` | `countSalePricingFacts()`, `selectPricingFactsPool()`, `selectPricingFactsNear()`, `getPricingMarketIndex()`, `getPricingSubdivisionCells()`, `getListingWaterSource()` <br /> `lib/data/pricing/facts.ts` |
| `pricing_subdivision_cells` | `countSalePricingFacts()`, `selectPricingFactsPool()`, `selectPricingFactsNear()`, `getPricingMarketIndex()`, `getPricingSubdivisionCells()`, `getListingWaterSource()` <br /> `lib/data/pricing/facts.ts` |
| `profiles` | `filtersToParamMap()`, `otherFilterChips()`, `enabledEventTypes()`, `summarizeAreaShapes()`, `activityLabel()`, `toClientPortalAlert()`, `collectAreaIds()`, `getClientPortalView()`, `rollupSavedHomeRows()`, `buildHomesPanelUnion()`, `buildSavedHomesIdentityOrFilter()`, `getContactSavedHomes()`, `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()` <br /> `lib/data/crm/getClientPortalView.ts` · `lib/data/crm/getContactSavedHomes.ts` · `lib/data/tc/client-transactions.ts` |
...
| `sale_pricing_facts` | `findCmaSubjectByMls()`, `findCmaSubjectByAddress()`, `getListingPhotosCount()`, `selectCmaCompsPool()`, `selectCmaCompsByKeys()`, `getCmaMarketStatsRow()`, `getCmaMarketPulseRow()`, `CMA_MARKET_TREND_MEASURE()`, `getCmaMarketTrendRows()`, `getCmaBrokerBySlugOrEmail()`, `listActiveBrokersForCma()`, `getCmaCityClosedSkinny()`, `getCmaCityClosedDuring()`, `getLikeHomeSales()`, `getCmaSubdivisionClosed()`, `getCmaSubdivisionHistory()`, `getCmaPriorSaleAtAddress()`, `countSalePricingFacts()`, `selectPricingFactsPool()`, `selectPricingFactsNear()`, `getPricingMarketIndex()`, `getPricingSubdivisionCells()`, `getListingWaterSource()` <br /> `lib/data/cma/builderReads.ts` · `lib/data/pricing/facts.ts` |
...
| `site_improvement_ledger` | `GSC_SNAPSHOT_SOURCE()`, `learnWindow()`, `isWindowSettled()`, `surfacePaths()`, `surfaceUrlVariants()`, `verdictFor()`, `measureLedgerRow()`, `closeLedgerRowWith()`, `liveGscPageTotals()`, `learnDueWindows()`, `insertImprovementLedgerRow()`, `listOpenImprovementWindows()`, `listExpiredUnlearnedWindows()`, `closeImprovementLedgerRow()`, `getChangeClassConfidence()`, `collectCompanyScoreboardSignals()`, `getLoopStatus()` <br /> `lib/data/loop/learn-close.ts` · `lib/data/loop/ledger.ts` · `lib/data/loop/signals.ts` · `lib/data/loop/status.ts` |
| `site_signal` | `foldAnswerEngineRows()`, `readAnswerEngineCitations()`, `ranThisMonth()`, `GSC_SNAPSHOT_SOURCE()`, `learnWindow()`, `isWindowSettled()`, `surfacePaths()`, `surfaceUrlVariants()`, `verdictFor()`, `measureLedgerRow()`, `closeLedgerRowWith()`, `liveGscPageTotals()`, `learnDueWindows()` <br /> `lib/data/loop/answer-engine-citations.test.ts` · `lib/data/loop/answer-engine-citations.ts` · `lib/data/loop/learn-close.ts` |
...
| `strict_verify_runs` | `getSyncState()`, `getSyncStateFields()`, `updateSyncStateLastDelta()`, `getExistingListingsByListNumbers()`, `replaceListingHistoryForKey()`, `upsertListingRows()`, `insertPriceHistoryRows()`, `insertStatusHistoryRows()`, `getActivityEvents()`, `insertActivityEventRows()`, `getListingPhotoUrl()`, `updateListingPhotoUrl()`, `upsertExpiredListingRow()`, `findCommunityIdByName()`, `findCommunityIdBySlug()`, `insertCommunityRowReturnId()`, `findPropertyIdByAddress()`, `insertPropertyAddressOnly()`, `insertPropertyFullRow()`, `updatePropertyById()`, `findListingBySnakeKey()`, `upsertListingSnakeRow()`, `insertStatusHistoryRow()`, `insertPriceHistoryRow()`, `replaceListingPhotosForKey()`, `deleteListingAgentsForKey()`, `insertListingAgentRow()`, `replaceListingVideosForKey()`, `upsertSyncState()`, `insertActivityEventRow()`, `updateListingByListNumber()`, `updateListingByListingKey()`, `insertListingHistoryRows()`, `deleteListingHistoryForKey()`, `getListingFieldsByListingKey()`, `getListingFieldsByListNumber()`, `selectHistorySyncCandidates()`, `getOpenHouseByIdAndListing()`, `insertOpenHouseRsvp()`, `bumpOpenHouseRsvpCount()`, `insertNotificationQueueRow()`, `insertStrictVerifyRun()`, `selectStrictVerifyCandidates()`, `getExpiredListingLookupAttempts()`, `findPropertiesByAddressFilter()`, `getPropertyById()`, `selectClosedListingsForCma()`, `getListingForCmaSubject()`, `findPropertiesByPostalAndStreet()`, `selectCmaSubjectListings()`, `insertValuationRequest()`, `listExpiredListingsForAdmin()`, `updateExpiredListingById()`, `updateExpiredListingByKey()`, `getCmaBySlug()`, `insertCmaRow()`, `upsertCmaRowBySlug()`, `listCmasForAdmin()`, `listCmasForLeadEmail()`, `countCmasInRange()`, `getBoundariesByGeoType()`, `upsertVideoToursCacheRow()`, `getExpiredListingsForDigest()`, `selectListingsAdmin()`, `getSyncCursor()`, `countListingsByOr()`, `countAllListingsByListingKey()`, `getLatestMarketPulseUpdatedAt()`, `countListingInquiriesSince()`, `countSavedSearchesSince()`, `insertOptimizationRun()`, `getAnyListingKey()`, `listingHistoryExistsForAnyKey()`, `countListingsByStatusOr()`, `countListingsByStatusOrAndFinalized()`, `countHistorySyncCandidates()` <br /> `lib/data/sync/syncWrites.ts` |
| `subdivision_city_inventory_mv` | `citySlugToMlsCityLower()`, `getSubdivisionCityInventory()`, `getSubdivisionCityInventoryByCityLower()`, `getSubdivisionBrowsePairsByCity()` <br /> `lib/data/subdivisions/getSubdivisionCityInventory.ts` |
...
| `subdivision_plat_closed_mv` | `getPlatClosedCounts()`, `getPlatClosedCount()` <br /> `lib/data/subdivisions/getPlatClosedCounts.ts` |
| `subdivision_plat_unsold_mv` | `mapPlatUnsoldRow()`, `getPlatUnsoldOutcome()` <br /> `lib/data/subdivisions/getPlatUnsoldOutcomes.ts` |
...
| `sync_logs` | `LOOP_SENTINEL_DEFAULT_OFF()`, `isLoopSentinelDisarmed()`, `runLoopSentinel()`, `collectCompanyScoreboardSignals()`, `getLoopStatus()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/loop/sentinel.ts` · `lib/data/loop/signals.ts` · `lib/data/loop/status.ts` · `lib/data/tc/skyslope-intake.ts` |
...
| `target_queries` | `MAX_DRAFTS()`, `HIGH_IMP()`, `TITLE_META_IMP()`, `RESEED_AFTER_DAYS()`, `ctr()`, `nextSiteNumber()`, `formatSiteGap()`, `normalizeQuery()`, `WINNERS()`, `LANDING_BUCKETS()`, `OTHER_CLASS_MODIFIERS()`, `queryStaysInWinnerClass()`, `winnerFor()`, `diagnose()`, `pathOf()`, `alreadySeeded()`, `kindLabel()`, `toDraft()`, `formatDrafts()`, `cannibalsFromLandings()`, `cannibalsFromQueryPages()`, `dropTrackerQueries()`, `buildGscGapDrafts()`, `CLASS_ROUTE()`, `buildClassSlipDrafts()`, `seedWindow()`, `pullGscSeedRows()`, `readSeedContext()`, `insertGscGapNodes()`, `seedClassSlipNodes()` <br /> `lib/data/loop/gsc-ranking-seed.ts` |
| `target_query_benchmark` | `collectCompanyScoreboardSignals()` <br /> `lib/data/loop/signals.ts` |
| `tc_checklist_assignments` | `getPrincipalSignOffQueue()`, `getTcDealContactRoles()`, `getTcCycleReferralFeeTotal()`, `getTcChecklistItemNames()`, `getTcAnticipatePresence()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/getTcAnticipatedReads.ts` · `lib/data/tc/skyslope-intake.ts` |
| `tc_checklist_items` | `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `listDealAudits()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getPrincipalSignOffQueue()`, `getTcDealContactRoles()`, `getTcCycleReferralFeeTotal()`, `getTcChecklistItemNames()`, `getTcAnticipatePresence()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/oversight/health.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/getTcAnticipatedReads.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/skyslope-intake.ts` |
...
| `tc_cycles` | `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `updateCyclePrices()`, `stampedTermsPatch()`, `writeCycleTermsByPerson()`, `listDealAudits()`, `countDealContacts()`, `getCycleRawForDeal()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `MAX_TIEBREAK_ATTEMPTS()`, `wantsTiebreak()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `keepFileValue()`, `getTermsReviewQueue()`, `confirmTermReading()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `getPrincipalSignOffQueue()`, `getTcCycleRawById()`, `listDealOffers()`, `getDealOffer()`, `getLatestSaleCycle()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `listDealOptions()`, `getDealScopeRow()`, `getDocumentDealScope()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/oversight/health.ts` · `lib/data/tc/client-transactions.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/continuation.ts` · `lib/data/tc/cycle-prices.ts` · `lib/data/tc/cycle-term-writes.ts` · `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-contact-reads.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/envelope-composer-reads.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/getTcCycleRawById.ts` · `lib/data/tc/listDealOffers.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/mail-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/signing-session.ts` · `lib/data/tc/skyslope-intake.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
| `tc_deal_contacts` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `countDealContacts()`, `getCycleRawForDeal()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getTcDealContactRoles()`, `getTcCycleReferralFeeTotal()`, `getTcChecklistItemNames()`, `getTcAnticipatePresence()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/deal-contact-reads.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/getTcAnticipatedReads.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/skyslope-intake.ts` |
| `tc_deal_people` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `listDealOptions()`, `getDealScopeRow()`, `getDocumentDealScope()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/mail-reads.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
| `tc_deals` | `collectCompanyScoreboardSignals()`, `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `listDealAudits()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `listDocumentReview()`, `countDocumentReview()`, `getPrincipalSignOffQueue()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `listDealOptions()`, `getDealScopeRow()`, `getDocumentDealScope()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/loop/signals.ts` · `lib/data/oversight/health.ts` · `lib/data/tc/client-transactions.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/document-review.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/mail-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/skyslope-intake.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
| `tc_document_readings` | `MAX_TERMS_ATTEMPTS()`, `MAX_TIEBREAK_ATTEMPTS()`, `wantsTiebreak()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `keepFileValue()`, `getTermsReviewQueue()`, `confirmTermReading()`, `STALL_AFTER_MS()`, `checkReaderHealth()` <br /> `lib/data/tc/deal-terms.ts` · `lib/data/tc/reader-health.ts` |
| `tc_documents` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `listDealAudits()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `MAX_TIEBREAK_ATTEMPTS()`, `wantsTiebreak()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `keepFileValue()`, `getTermsReviewQueue()`, `confirmTermReading()`, `listDocumentReview()`, `countDocumentReview()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `getPrincipalSignOffQueue()`, `getTcDealContactRoles()`, `getTcCycleReferralFeeTotal()`, `getTcChecklistItemNames()`, `getTcAnticipatePresence()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `listDealOptions()`, `getDealScopeRow()`, `getDocumentDealScope()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `getReviewDocuments()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/continuation.ts` · `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/document-review.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/getTcAnticipatedReads.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/mail-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/review-documents.ts` · `lib/data/tc/signing-session.ts` · `lib/data/tc/skyslope-intake.ts` |
| `tc_envelope_documents` | `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()` <br /> `lib/data/tc/continuation.ts` · `lib/data/tc/envelope-composer-reads.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/signing-session.ts` |
| `tc_envelope_fields` | `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()` <br /> `lib/data/tc/continuation.ts` · `lib/data/tc/envelope-composer-reads.ts` · `lib/data/tc/signing-session.ts` |
| `tc_envelope_recipients` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `listOutstandingEnvelopes()`, `listEnvelopeSigningRoster()`, `listEnvelopeAddressBook()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()`, `mintOrReuseSigningLink()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/continuation.ts` · `lib/data/tc/envelope-overview.ts` · `lib/data/tc/envelope-recipient-reads.ts` · `lib/data/tc/signing-session.ts` · `lib/data/tc/signing-token-vault.ts` |
| `tc_envelopes` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `listOutstandingEnvelopes()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/continuation.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/envelope-overview.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/signing-session.ts` |
| `tc_events` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `stampedTermsPatch()`, `writeCycleTermsByPerson()`, `listDealEvents()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `MAX_TIEBREAK_ATTEMPTS()`, `wantsTiebreak()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `keepFileValue()`, `getTermsReviewQueue()`, `confirmTermReading()`, `listDocumentReview()`, `countDocumentReview()`, `findSigningRecipient()`, `getSigningDeal()`, `getSigningDocuments()`, `getEnvelopeFieldsForSigning()`, `updateSigningRecipient()`, `saveSignerValues()`, `voidEnvelopeOnDecline()`, `logSigningEvent()`, `findSharedAddressNextSigner()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/continuation.ts` · `lib/data/tc/cycle-term-writes.ts` · `lib/data/tc/deal-events.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/document-review.ts` · `lib/data/tc/signing-session.ts` · `lib/data/tc/skyslope-intake.ts` |
...
| `tc_form_templates` | `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()` <br /> `lib/data/tc/continuation.ts` |
| `tc_form_versions` | `nextAddendumNumber()`, `sectionsOf()`, `applyEnvelopeSectionText()`, `_internal()`, `renderContinuationPreview()`, `getPacketSections()`, `getPacketDealBroker()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `getTcFormLibraryBoard()`, `applyFormCatalogSnapshots()`, `listFormPackets()`, `findFormVersionIdByNumber()`, `findFormVersionIdByNeedle()`, `getFormVersionBlankRow()`, `listLiveFormVersionsForMapping()`, `listClauses()`, `ingestLicensedBlankPdf()`, `listEnvelopeTemplates()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()` <br /> `lib/data/tc/continuation.ts` · `lib/data/tc/envelope-composer-reads.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/form-catalog.ts` · `lib/data/tc/form-library-reads.ts` · `lib/data/tc/ingest-licensed-blank.ts` · `lib/data/tc/listEnvelopeTemplates.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/oref-packet-reads.ts` |
| `tc_mail_messages` | `listDealAudits()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `listDealOptions()`, `getDealScopeRow()`, `getDocumentDealScope()` <br /> `lib/data/tc/deal-audit.ts` · `lib/data/tc/mail-reads.ts` |
| `tc_mail_review_cursors` | `getMailCoverage()` <br /> `lib/data/tc/mail-coverage.ts` |
| `tc_offers` | `listDealAudits()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `listDealOffers()`, `getDealOffer()`, `getLatestSaleCycle()` <br /> `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/listDealOffers.ts` |
| `tc_principal_reviews` | `listDealAudits()` <br /> `lib/data/tc/deal-audit.ts` |
| `tc_tasks` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `listDealTasks()`, `listFileDeadlineTasks()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/task-reads.ts` |
...
| `visitor_events` | `getListingPerformance()`, `getPlacePopularity()`, `rangeToIso()`, `personName()`, `personHref()`, `readPeople()`, `readSessions()`, `countAudienceSessions()`, `readAccountSnapshots()`, `readDeals()`, `readWorkingPersonIds()`, `sellerClientPersonIds()`, `readActiveBrokers()`, `readJoinConverts()`, `namesForIds()`, `REPLY_TIMELINE_KINDS()`, `emptyCmaOutcome()`, `getCmaOutcomes()`, `getCmaLaneFunnel()`, `HOT_ANONYMOUS_SOURCE()`, `captureHotAnonymous()`, `topByCount()`, `pickLatestListingView()`, `deriveIntentSignals()`, `getContactBehaviorSummary()`, `getLookingAtNow()`, `getPersonGlance()`, `getPropertiesReport()`, `ACTIVITY_WINDOWS()`, `readListingFacts()`, `getActiveKnownPeople()`, `getPersonSiteActivity()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `recordSaveListingEvent()`, `saveAnonymousPartialAddress()`, `JOIN_CONVERT_EVENT()`, `JOIN_PAGE_CATEGORY()`, `RECRUIT_JOIN_TAG()`, `JOIN_CONVERSION_SOURCE()`, `isJoinInquiry()`, `isJoinPageUrl()`, `summarizeJoinEvents()`, `readJoinConversionStats()`, `getJoinConversionStats()`, `recordJoinConversion()`, `tagRecruitJoin()`, `collectCompanyScoreboardSignals()`, `EMPTY_DOC_ENGAGEMENT_DETAIL()`, `projectEngagement()`, `getDocEngagementDetail()`, `getDocEngagement()`, `getProspectEngagement()` <br /> `lib/data/analytics/getListingPerformance.ts` · `lib/data/analytics/getPlacePopularity.ts` · `lib/data/analytics/salesFunnelRead.ts` · `lib/data/cma/outcomes.ts` · `lib/data/crm/captureHotAnonymous.ts` · `lib/data/crm/getContactBehaviorSummary.ts` · `lib/data/crm/getLookingAtNow.ts` · `lib/data/crm/getPersonGlance.ts` · `lib/data/crm/getPropertiesReport.ts` · `lib/data/crm/getSiteActivity.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/crm/recordSaveListingEvent.ts` · `lib/data/leads/saveAnonymousPartialAddress.ts` · `lib/data/loop/join-conversion.ts` · `lib/data/loop/signals.ts` · `lib/data/prospecting/engagement.ts` |
| `visitor_identity_map` | `HOT_ANONYMOUS_SOURCE()`, `captureHotAnonymous()`, `filtersToParamMap()`, `otherFilterChips()`, `enabledEventTypes()`, `summarizeAreaShapes()`, `activityLabel()`, `toClientPortalAlert()`, `collectAreaIds()`, `getClientPortalView()`, `rollupSavedHomeRows()`, `buildHomesPanelUnion()`, `buildSavedHomesIdentityOrFilter()`, `getContactSavedHomes()`, `getLookingAtNow()`, `getStitchedCrmPersonId()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `normalizeEmail()`, `normalizePhone()`, `dedupeContactPoints()`, `resolvePersonIdentity()`, `readSessionIdentity()`, `isMissingColumnError()`, `insertVisitorSession()`, `identifySessionAndBrowser()`, `clearProvisionalAutomation()`, `readIdentityMapForVid()`, `stampSessionEmailOnly()`, `collectCompanyScoreboardSignals()` <br /> `lib/data/crm/captureHotAnonymous.ts` · `lib/data/crm/getClientPortalView.ts` · `lib/data/crm/getContactSavedHomes.ts` · `lib/data/crm/getLookingAtNow.ts` · `lib/data/crm/getStitchedCrmPersonId.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/crm/resolvePersonIdentity.ts` · `lib/data/identity/sessionIdentity.ts` · `lib/data/loop/signals.ts` |
| `visitor_sessions` | `getListingPerformance()`, `getLpLeaderboard()`, `classifySocial()`, `getSocialHeadlineSummary()`, `getSocialChannelBreakdown()`, `getSocialLiveFeed()`, `getLeadSources()`, `rangeToIso()`, `personName()`, `personHref()`, `readPeople()`, `readSessions()`, `countAudienceSessions()`, `readAccountSnapshots()`, `readDeals()`, `readWorkingPersonIds()`, `sellerClientPersonIds()`, `readActiveBrokers()`, `readJoinConverts()`, `namesForIds()`, `REPLY_TIMELINE_KINDS()`, `emptyCmaOutcome()`, `getCmaOutcomes()`, `getCmaLaneFunnel()`, `HOT_ANONYMOUS_SOURCE()`, `captureHotAnonymous()`, `topByCount()`, `pickLatestListingView()`, `deriveIntentSignals()`, `getContactBehaviorSummary()`, `pickFirstTouch()`, `getFirstTouchAttribution()`, `TRIAGE_WEIGHTS()`, `TRIAGE_HALF_LIFE_HOURS()`, `SEQUENCE_RANK()`, `triageRank()`, `rankTriageItems()`, `mergeNeedsAction()`, `replySignal()`, `classifyDocEvent()`, `docSignal()`, `visitSignal()`, `isTriageTaskCandidate()`, `taskSignal()`, `formatTriageAge()`, `isSuppressedByStateTouch()`, `isUnreadStatus()`, `TRIAGE_WINDOW_HOURS()`, `getInboundTriage()`, `getLookingAtNow()`, `getMarketingUtmReport()`, `getPeopleListSignals()`, `ACTIVITY_WINDOWS()`, `readListingFacts()`, `getActiveKnownPeople()`, `getPersonSiteActivity()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `readLastSiteByPerson()`, `ACTIVITY_WEIGHTS()`, `clampSinceDays()`, `rankCohortActivity()`, `getWestsideCohortActivity()`, `recordSaveListingEvent()`, `normalizeEmail()`, `normalizePhone()`, `dedupeContactPoints()`, `resolvePersonIdentity()`, `readSessionIdentity()`, `isMissingColumnError()`, `insertVisitorSession()`, `identifySessionAndBrowser()`, `clearProvisionalAutomation()`, `readIdentityMapForVid()`, `stampSessionEmailOnly()`, `JOIN_CONVERT_EVENT()`, `JOIN_PAGE_CATEGORY()`, `RECRUIT_JOIN_TAG()`, `JOIN_CONVERSION_SOURCE()`, `isJoinInquiry()`, `isJoinPageUrl()`, `summarizeJoinEvents()`, `readJoinConversionStats()`, `getJoinConversionStats()`, `recordJoinConversion()`, `tagRecruitJoin()` <br /> `lib/data/analytics/getListingPerformance.ts` · `lib/data/analytics/getLpLeaderboard.ts` · `lib/data/analytics/getSocialChannels.ts` · `lib/data/analytics/leadSources.ts` · `lib/data/analytics/salesFunnelRead.ts` · `lib/data/cma/outcomes.ts` · `lib/data/crm/captureHotAnonymous.ts` · `lib/data/crm/getContactBehaviorSummary.ts` · `lib/data/crm/getFirstTouchAttribution.ts` · `lib/data/crm/getInboundTriage.ts` · `lib/data/crm/getLookingAtNow.ts` · `lib/data/crm/getMarketingUtmReport.ts` · `lib/data/crm/getPeopleListSignals.ts` · `lib/data/crm/getSiteActivity.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/crm/getVisitorLastSeen.ts` · `lib/data/crm/getWestsideCohortActivity.ts` · `lib/data/crm/recordSaveListingEvent.ts` · `lib/data/crm/resolvePersonIdentity.ts` · `lib/data/identity/sessionIdentity.ts` · `lib/data/loop/join-conversion.ts` |
| `web_vitals` | `GSC_SNAPSHOT_SOURCE()`, `learnWindow()`, `isWindowSettled()`, `surfacePaths()`, `surfaceUrlVariants()`, `verdictFor()`, `measureLedgerRow()`, `closeLedgerRowWith()`, `liveGscPageTotals()`, `learnDueWindows()` <br /> `lib/data/loop/learn-close.ts` |
...
| `workspace_directory` | `listWorkspaceUsers()`, `runWorkspaceBrokerSync()`, `recordTeamPageDecision()` <br /> `lib/data/brokers/workspace-sync.ts` |
