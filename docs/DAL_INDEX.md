...
**Generated:** 2026-09-25T13:36:27.832Z
...
### `lib/data/market-report/compute.ts`

**Exports:** `REPORT_DEFINITION_ID`, `refreshMarketReportListingBatch`, `refreshMarketReportGeo`, `refreshMarketReportFacts`, `computeMarketReportPeriod`, `refreshMarketReportListingSince`, `refreshMarketReportFactsSince`, `pruneMarketFactSale`, `refreshMarketFactSale`, `refreshMarketFactSpans`, `refreshMarketFactSpansForKeys`, `upsertMarketReportListings`

---

### `lib/data/market-report/editions.ts`

**Exports:** `REPORT_BUCKET`, `editionFiguresOf`, `editionPdfPath`, `editionPdfObjectUrl`, `listPublishedEditions`, `getPublishedEdition`, `getEditionForWrite`, `uploadEditionPdf`, `upsertEdition`

**Tables:** `market_report_editions`

**Selected columns:** `${LIST_COLUMNS}`, `status`, `payload`, `citations`, `definition_id`, `generated_at`, `hold_reason`, `published_at`
...
### `lib/data/market-report/reconcile.ts`

**Exports:** `getReportSalesInWindow`, `getFactSaleStatus`, `getServiceAreaCities`

**Tables:** `market_report_sale`, `market_fact_sale`, `market_service_area`

**Selected columns:** `listing_key`, `close_date`, `close_price`, `base_segment`, `lot_acres`, `geos`, `is_publishable`, `exclusion_reasons`, `city_proper`, `city_slug`

---

### `lib/data/market-report/series.ts`

**Exports:** `getReportSeries`, `getReportBands`

**Tables:** `market_report_series`, `market_report_band`

**Selected columns:** `period_end`, `geo_type`, `geo_slug`, `segment`, `band_idx`, `closed_n`, `active_end_n`
...
### `lib/data/sync/closingsReconcile.ts`

**Exports:** `getListingsForReconcile`, `getClosedListingKeysInWindow`, `rebuildPlaceMembershipForKeys`, `recordAbsentFromMls`, `getAbsentFromMlsKeys`, `clearAbsentFromMls`, `recordRepairLog`, `setRepairLogOutcome`

**Tables:** `listings`, `market_listing_absent_from_mls`, `listing_mls_repair_log`

**Selected columns:** `ListingKey`, `listing_key`, `id`
...
**Selected columns:** `last_delta_sync_at`, `ListNumber`, `ListingKey`, `StandardStatus`, `ListPrice`, `is_finalized`, `City`, `CloseDate`, `ClosePrice`, `property_sub_type`, `TotalLivingAreaSqFt`, `media_finalized`, `overrides:details->admin_overrides`, `PhotoURL`, `has_virtual_tour`, `OpenHouses`, `Photos:details->Photos`, `FloorPlans:details->FloorPlans`, `Videos:details->Videos`, `VirtualTours:details->VirtualTours`, `Documents:details->Documents`, `DetailOpenHouses:details->OpenHouses`, `id`, `listing_key`, `event_type`, `event_at`, `payload`, `standard_status`, `list_price`, `event_date`, `start_time`, `end_time`, `rsvp_count`, `owner_lookup_attempts`, `unparsed_address`, `city`, `state`, `postal_code`, `street_number`, `StreetNumber` (+50 more)
...
| `listing_mls_repair_log` | `getListingsForReconcile()`, `getClosedListingKeysInWindow()`, `rebuildPlaceMembershipForKeys()`, `recordAbsentFromMls()`, `getAbsentFromMlsKeys()`, `clearAbsentFromMls()`, `recordRepairLog()`, `setRepairLogOutcome()` <br /> `lib/data/sync/closingsReconcile.ts` |
...
| `market_listing_absent_from_mls` | `getListingsForReconcile()`, `getClosedListingKeysInWindow()`, `rebuildPlaceMembershipForKeys()`, `recordAbsentFromMls()`, `getAbsentFromMlsKeys()`, `clearAbsentFromMls()`, `recordRepairLog()`, `setRepairLogOutcome()` <br /> `lib/data/sync/closingsReconcile.ts` |
...
| `market_report_editions` | `REPORT_BUCKET()`, `editionFiguresOf()`, `editionPdfPath()`, `editionPdfObjectUrl()`, `listPublishedEditions()`, `getPublishedEdition()`, `getEditionForWrite()`, `uploadEditionPdf()`, `upsertEdition()` <br /> `lib/data/market-report/editions.ts` |
