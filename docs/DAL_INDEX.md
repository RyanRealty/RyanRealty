...
**Generated:** 2026-09-25T02:26:51.592Z
...
**Selected columns:** `${LIST_COLUMNS}`, `status`, `payload`, `citations`, `definition_id`, `generated_at`, `hold_reason`
...
**Exports:** `getOurDetachedClosings`

**Tables:** `market_fact_sale`

**Selected columns:** `listing_key`, `close_price`, `lot_acres`, `is_publishable`, `exclusion_reasons`
...
**Exports:** `getListingsForReconcile`, `getClosedListingKeysInWindow`
...
| `market_fact_sale` | `getOurDetachedClosings()` <br /> `lib/data/market-report/reconcile.ts` · `lib/data/proof/getProofBlock.int.test.ts` |
...
| `tc_events` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `stampedTermsPatch()`, `writeCycleTermsByPerson()`, `listDealEvents()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `MAX_TIEBREAK_ATTEMPTS()`, `wantsTiebreak()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `keepFileValue()`, `getTermsReviewQueue()`, `confirmTermReading()`, `listDocumentReview()`, `countDocumentReview()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/cycle-term-writes.ts` · `lib/data/tc/deal-events.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/document-review.ts` · `lib/data/tc/skyslope-intake.ts` |
