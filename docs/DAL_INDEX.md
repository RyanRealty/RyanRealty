...
**Generated:** 2026-09-24T17:35:01.805Z
...
| `tc_cycles` | `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `updateCyclePrices()`, `listDealAudits()`, `countDealContacts()`, `getCycleRawForDeal()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `getEnvelopeCycleKindAndDeal()`, `listUnassignedEnvelopeFields()`, `listEnvelopeDocumentFormVersions()`, `getFormVersionFieldMaps()`, `getListPriceByMlsNumber()`, `getFormSourcesForEnvelope()`, `listEnvelopeFormFreshness()`, `getPrincipalSignOffQueue()`, `getTcCycleRawById()`, `listDealOffers()`, `getDealOffer()`, `getLatestSaleCycle()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getCycleForCda()`, `listCycleIdsForDeal()`, `getLatestListingCycle()`, `listCycleDocumentCopies()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `listInFlightEnvelopes()`, `listDealMail()`, `listMailQueue()`, `countMailQueue()`, `listDealConversations()`, `getMailMessagesForAction()`, `getDealScopeRow()`, `getDocumentDealScope()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/oversight/health.ts` · `lib/data/tc/client-transactions.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/cycle-prices.ts` · `lib/data/tc/deal-audit.ts` · `lib/data/tc/deal-contact-reads.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/envelope-composer-reads.ts` · `lib/data/tc/envelope-form-sources.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/getTcCycleRawById.ts` · `lib/data/tc/listDealOffers.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/mail-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/skyslope-intake.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
...
| `tc_events` | `getClientIdentity()`, `hasClientDeals()`, `listClientDeals()`, `getClientDeal()`, `clientDocumentUrl()`, `getPortalSigningRecipient()`, `listDealEvents()`, `getDealParties()`, `inboundReferralFeePctForDeal()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `MAX_TERMS_ATTEMPTS()`, `termsFormsForDocument()`, `readDocumentTerms()`, `pendingTermsDocuments()`, `getCycleTerms()`, `applyCycleTerms()`, `acceptContractValue()`, `listDocumentReview()`, `countDocumentReview()`, `tryTakeSkySlopeIntakeLease()`, `releaseSkySlopeIntakeLease()`, `writeSkySlopeIntakeRunLog()`, `runSkySlopeVaultIntake()` <br /> `lib/data/tc/client-transactions.ts` · `lib/data/tc/deal-events.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/deal-terms.ts` · `lib/data/tc/document-review.ts` · `lib/data/tc/skyslope-intake.ts` |
