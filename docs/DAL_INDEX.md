...
**Generated:** 2026-08-23T20:35:38.898Z
...
**Tables:** `crm_timeline`, `crm_tasks`

**Selected columns:** `kind`, `ts`, `payload`, `title`, `name`, `type`, `origin`
...
**Selected columns:** `id`, `person_id`, `is_active`, `recipients`
...
**Exports:** `getHeroPhotosByListingKeys`, `getOpenHousesInRange`, `getListingDetailPhotos`, `getListingKeysForBrokerByLicense`, `getListingKeysForBrokerByEmail`, `getListingKeysByListAgentEmail`, `getListingDetailAgents`, `getOpenHouseById`, `getListingDetailOpenHouses`, `getListingDetailVideos`, `getListingKeysWithPriceChangeSince`, `seedListingDetailHistory`, `getListingDetailHistory`, `resolveCommunityChainBySlug`

**Tables:** `listing_photos`, `open_houses`, `listing_agents`, `listings`, `listing_videos`, `listing_history`, `status_history`, `price_history`, `communities`, `neighborhoods`, `cities`

**Selected columns:** `listing_key`, `photo_url`, `id`, `open_house_key`, `event_date`, `start_time`, `end_time`, `host_agent_name`, `remarks`, `rsvp_count`, `cdn_url`, `sort_order`, `caption`, `is_hero`, `ListingKey`, `agent_role`, `agent_name`, `agent_mls_id`, `agent_license`, `agent_email`, `agent_phone`, `office_name`, `office_mls_id`, `office_phone`, `OpenHouses`, `video_url`, `event`, `price`, `price_change`, `description`, `raw`, `old_status`, `new_status`, `changed_at`, `old_price`, `new_price`, `change_pct`, `OnMarketDate`, `ListPrice`, `name` (+3 more)
...
**Exports:** `staleReason`, `getMetric`

**Tables:** `market_metric`

**Selected columns:** `stat_id`, `geo_type`, `geo_slug`, `segment`, `period_end`, `window_months`, `definition_id`, `value`, `value_text`, `sample_n`, `method`, `excluded_n`, `complete_through`, `is_publishable`, `withheld_reason`, `is_floor`, `computed_at`
...
**Selected columns:** `stat_id`, `value`, `period_end`, `computed_at`, `complete_through`, `window_months`, `is_publishable`

---

### `lib/data/market-truth/public-segments.ts`

**Exports:** `PUBLIC_PLACE_SEGMENTS`, `publicSegmentNoun`, `publicSegmentBrowseHref`, `publicSegmentVerdictLabel`, `publicSegmentDisplayBits`, `publicSegmentItems`, `getPublicPlaceSegments`
...
**Exports:** `buildCityReportSnapshot`, `hasReportSignal`, `getCityReportSnapshot`, `getCityReportSnapshots`
...
**Cache keys:** `mega-menu-data-v2-mt-detached`
...
**Exports:** `getDealParties`, `ensureDealPartiesFromFile`, `peopleEmailsByNames`, `getDealsForPerson`, `getPartyNamesByDealIds`, `linkUniqueCycleParties`, `addPersonToDeal`, `removePersonFromDeal`, `createDealWithPeople`

**Tables:** `tc_deal_people`, `tc_deals`, `tc_cycles`, `tc_deal_contacts`, `crm_people`, `tc_events`, `tc_checklist_items`

**Selected columns:** `id`, `deal_id`, `person_id`, `role`, `crm_people(name)`, `broker_name`, `address`, `kind`, `raw`, `buyers`, `sellers`, `name`, `email`, `emails`, `tc_deals(id`, `property_key`, `stage)`
...
**Selected columns:** `id`, `name`, `form_number`, `blank_pdf_storage_path`, `library_id`, `code`

---

### `lib/data/tc/listing-action-reads.ts`

**Exports:** `getDealByPropertyKey`, `getDealById`, `listDealPropertyKeys`, `getLatestListingCycle`, `listChecklistItemCopies`, `listDealContactCopies`, `listDealContactKeys`

**Tables:** `tc_deals`, `tc_cycles`, `tc_checklist_items`, `tc_deal_contacts`

**Selected columns:** `property_key`, `id`, `mls_number`, `sellers`, `buyers`, `listing_price`, `checklist_type`, `status`, `listing_date`, `expiration_date`, `name`, `type_name`, `sort_order`, `role`, `company`, `email`, `phone`, `notes`
...
| `crm_broker_alerts` | `searchLegalCorpus()`, `latestCorpusVersion()`, `corpusCounts()`, `flagLawQuestionToMatt()`, `listPendingAlerts()`, `claimAlert()`, `markSent()`, `markFailure()`, `refuseAlert()`, `reclaimStaleSending()`, `insertBrokerSelfAlert()`, `recentHealthAlertExists()`, `insertHealthAlert()`, `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()` <br /> `lib/data/agent/legal.ts` · `lib/data/crm/brokerAlertDrain.ts` · `lib/data/crm/brokerSelfAlert.ts` · `lib/data/crm/healthAlertQueue.ts` · `lib/data/oversight/health.ts` |
...
| `listing_tile_mv` | `filtersToParamMap()`, `otherFilterChips()`, `enabledEventTypes()`, `summarizeAreaShapes()`, `activityLabel()`, `toClientPortalAlert()`, `collectAreaIds()`, `getClientPortalView()`, `rollupSavedHomeRows()`, `buildHomesPanelUnion()`, `buildSavedHomesIdentityOrFilter()`, `getContactSavedHomes()`, `getOwnedHomeMatches()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `platInventoryKey()`, `platCityAliases()`, `rowMatchesPlat()`, `isDisplayablePlatName()`, `registryChildPlats()`, `rollupPlatPublicInventory()`, `getRegistryPlatPublicInventory()`, `getPlatPublicInventory()`, `getListingTiles()`, `getTotalListingCount()`, `getListingTilesCount()`, `getCityListings()`, `getCommunityListings()`, `getZipListings()`, `getNeighborhoodListings()`, `getSearchMatrixInventory()`, `getSubdivisionLifetimeCounts()`, `getSubdivisionDescriptionKeys()`, `getMatrixNeighborhoods()`, `EMPTY_SITE_INDEX()`, `getSiteIndexLinks()`, `getDerivedPopularSearches()`, `getListingSitemapRows()`, `getIndexableSubdivisions()`, `isSubdivisionIndexable()`, `getSubdivisionBrowseSlugsByCity()` <br /> `lib/data/crm/getClientPortalView.ts` · `lib/data/crm/getContactSavedHomes.ts` · `lib/data/crm/getOwnedHome.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/geo/plat-public-inventory.ts` · `lib/data/listings/getListingTiles.ts` · `lib/data/listings/getSearchMatrixInventory.ts` · `lib/data/seo/getSiteIndexLinks.ts` · `lib/data/sitemap/getListingSitemapRows.ts` · `lib/data/subdivisions/getIndexableSubdivisions.ts` · `lib/data/subdivisions/getSubdivisionBrowseSlugsByCity.ts` |
...
| `market_metric` | `getCitySegmentBoard()`, `staleReason()`, `getMetric()`, `cityDetachedSlug()`, `getDetachedMarkets()`, `getDetachedInventories()`, `getDetachedOverlays()`, `getDetachedMarket()`, `getCityDetachedMarket()`, `getCityDetachedInventory()`, `getSellBendMarket()`, `applyDetachedOverlay()`, `withholdDetachedHeadlines()`, `overlayDetachedLayers()`, `overlayDetachedMarket()`, `getCityLeaderboard()`, `PUBLIC_PACE_WINDOW_MONTHS()`, `PUBLIC_PACE_STATS()`, `EMPTY_PUBLIC_PACE()`, `formatPaceShare()`, `formatPaceDelta()`, `publicPaceHasRow()`, `publicPaceItems()`, `getPublicDetachedPace()`, `PUBLIC_PLACE_SEGMENTS()`, `publicSegmentNoun()`, `publicSegmentBrowseHref()`, `publicSegmentVerdictLabel()`, `publicSegmentDisplayBits()`, `publicSegmentItems()`, `getPublicPlaceSegments()` <br /> `lib/data/market-truth/city-segments.ts` · `lib/data/market-truth/getMetric.ts` · `lib/data/market-truth/getSellBendMarket.ts` · `lib/data/market-truth/leaderboards.ts` · `lib/data/market-truth/public-pace.ts` · `lib/data/market-truth/public-segments.ts` |
...
| `tc_checklist_assignments` | `getPrincipalSignOffQueue()` <br /> `lib/data/tc/getPrincipalSignOffQueue.ts` |
| `tc_checklist_items` | `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `getDealParties()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getPrincipalSignOffQueue()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getLatestListingCycle()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()` <br /> `lib/data/oversight/health.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/listing-action-reads.ts` |
| `tc_commissions` | `collectCompanyScoreboardSignals()` <br /> `lib/data/loop/signals.ts` |
| `tc_cycles` | `getSyncFreshness()`, `getAlertQueueHealth()`, `getSignoffWaits()`, `liveDealCyclesFromBoard()`, `closingSearchHaystack()`, `closingMatchesQuery()`, `incompleteInFlight()`, `getClosingsBoard()`, `getLiveDealCycles()`, `countDealContacts()`, `getCycleRawForDeal()`, `getDealParties()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getPrincipalSignOffQueue()`, `listDealOffers()`, `getDealOffer()`, `getLatestSaleCycle()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getLatestListingCycle()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/oversight/health.ts` · `lib/data/tc/closings.ts` · `lib/data/tc/deal-contact-reads.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/listDealOffers.ts` · `lib/data/tc/listing-action-reads.ts` · `lib/data/tc/oref-packet-reads.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
| `tc_deal_contacts` | `countDealContacts()`, `getCycleRawForDeal()`, `getDealParties()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `getDealByPropertyKey()`, `getDealById()`, `listDealPropertyKeys()`, `getLatestListingCycle()`, `listChecklistItemCopies()`, `listDealContactCopies()`, `listDealContactKeys()` <br /> `lib/data/tc/deal-contact-reads.ts` · `lib/data/tc/deal-people.ts` · `lib/data/tc/listing-action-reads.ts` |
| `tc_deal_people` | `getDealParties()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()`, `stageReviewAsksForRecentCloses()` <br /> `lib/data/tc/deal-people.ts` · `lib/data/tc/stageReviewAsksForRecentCloses.ts` |
...
| `tc_documents` | `getPrincipalSignOffQueue()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()` <br /> `lib/data/tc/getPrincipalSignOffQueue.ts` · `lib/data/tc/oref-packet-reads.ts` |
...
| `tc_events` | `getDealParties()`, `ensureDealPartiesFromFile()`, `peopleEmailsByNames()`, `getDealsForPerson()`, `getPartyNamesByDealIds()`, `linkUniqueCycleParties()`, `addPersonToDeal()`, `removePersonFromDeal()`, `createDealWithPeople()` <br /> `lib/data/tc/deal-people.ts` |
...
| `tc_form_libraries` | `getTcFormLibraryBoard()`, `applyFormCatalogSnapshots()`, `listEnvelopeTemplates()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()` <br /> `lib/data/tc/form-catalog.ts` · `lib/data/tc/listEnvelopeTemplates.ts` · `lib/data/tc/oref-packet-reads.ts` |
| `tc_form_versions` | `getTcFormLibraryBoard()`, `applyFormCatalogSnapshots()`, `listEnvelopeTemplates()`, `loadPreferredOrefForm()`, `getOrefCycleForFill()`, `getOrefDealForFill()`, `getOrefFormVersionRow()`, `getOrefDocumentRow()`, `getMattMailboxPersonId()`, `getCycleDealId()`, `getOrefCycleForSeal()`, `getEnvelopeIdForDocument()` <br /> `lib/data/tc/form-catalog.ts` · `lib/data/tc/listEnvelopeTemplates.ts` · `lib/data/tc/oref-packet-reads.ts` |
| `tc_offers` | `listDealOffers()`, `getDealOffer()`, `getLatestSaleCycle()` <br /> `lib/data/tc/listDealOffers.ts` |
...
| `visitor_events` | `HOT_ANONYMOUS_SOURCE()`, `captureHotAnonymous()`, `topByCount()`, `pickLatestListingView()`, `deriveIntentSignals()`, `getContactBehaviorSummary()`, `getLookingAtNow()`, `getPropertiesReport()`, `buildSessionOrFilter()`, `resolveLeadSessionIds()`, `getViewedListingsForLead()`, `recordSaveListingEvent()`, `saveAnonymousPartialAddress()`, `JOIN_CONVERT_EVENT()`, `JOIN_PAGE_CATEGORY()`, `RECRUIT_JOIN_TAG()`, `JOIN_CONVERSION_SOURCE()`, `isJoinInquiry()`, `isJoinPageUrl()`, `summarizeJoinEvents()`, `readJoinConversionStats()`, `getJoinConversionStats()`, `recordJoinConversion()`, `tagRecruitJoin()`, `collectCompanyScoreboardSignals()`, `getDocEngagement()`, `getProspectEngagement()` <br /> `lib/data/crm/captureHotAnonymous.ts` · `lib/data/crm/getContactBehaviorSummary.ts` · `lib/data/crm/getLookingAtNow.ts` · `lib/data/crm/getPropertiesReport.ts` · `lib/data/crm/getViewedListings.ts` · `lib/data/crm/recordSaveListingEvent.ts` · `lib/data/leads/saveAnonymousPartialAddress.ts` · `lib/data/loop/join-conversion.ts` · `lib/data/loop/signals.ts` · `lib/data/prospecting/engagement.ts` |
