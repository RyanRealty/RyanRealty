...
## PAUSED — Matt directive 2026-06-11 (~13:00)

"Let's stop tc-builder and flyover videos until the v6 build is done." HOLD all
iterations: on each firing, check docs/EXPERIENCE_SYSTEM.md Family 2 — resume the
ladder ONLY when the v6 homepage build state reads shipped/approved (or Matt says
resume). Until then: finish any in-flight commit, then sleep long. The Oregon-law
work order and telephony items stay queued, untouched, first in line on resume.
