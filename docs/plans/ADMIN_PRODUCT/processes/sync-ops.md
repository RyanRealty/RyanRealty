...
- Concrete triggers (registered in vercel.json, verified this session):
  ...
  - Derived data: pg_cron `refresh_listing_tile_mv_30min` (:02/:32) + `refresh_dal_mvs_15min` (:05/:20/:35/:50; geo_snapshot, boundary_xref, listing_search, and neighborhood_year_pricing once `20260923014600` is applied) + `refresh_place_membership_15min` (:09/:24/:39/:54) once `20260923014500` is applied; until both migrations land the Vercel `refresh-mvs` route (hourly :08) runs neighborhood_year_pricing and the place_membership rebuild (audits DATA-2, COMP-3), `refresh-market-stats` daily + monthly recompute weekly, `refresh-subdivision-stats`, `refresh-similar-listings`, `market-history-snapshot` weekly (FRED/PMMS), `warm-sitemaps` hourly
