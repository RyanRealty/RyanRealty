...
- Concrete triggers (registered in vercel.json, verified this session):
  ...
  - Derived data: pg_cron `refresh_listing_tile_mv_30min` (:02/:32) + `refresh_dal_mvs_15min` (:05/:20/:35/:50; geo_snapshot, boundary_xref, listing_search, neighborhood_year_pricing via `20260923014600`) + `refresh_place_membership_15min` (:09/:24/:39/:54, `20260923014500`); the Vercel `refresh-mvs` route was deleted 2026-09-24 (audits DATA-2, COMP-3), `refresh-market-stats` daily + monthly recompute weekly, `refresh-subdivision-stats`, `refresh-similar-listings`, `market-history-snapshot` weekly (FRED/PMMS), `warm-sitemaps` hourly
