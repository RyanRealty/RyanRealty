...
| 4. Findings become work | `npx tsx scripts/fleet-intake.ts` — info→baseline recorded; minor/major/p0 append to one durable punch-list node (`version_gap=FLEET-PUNCH`, title `Fleet finding [p0\|major]: review punch list`) tagged `fleet:<fingerprint>` per line; `regress-G*` CHANGE findings still get their own node | intake script |
| 5. Loop fixes | when FLEET-PUNCH is next, the brief serves one surface-family punch-line slice (not the whole pile); accept = reproduce-or-reject each served line at 390+1280. Completing the punch list requires zero open lines. `regress-G*` stays its own node | normal loop cycle |
