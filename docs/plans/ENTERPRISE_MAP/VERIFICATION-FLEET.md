...
- ~~Scheduled unattended loop iterations (R-206)~~ **SHIPPED 2026-08-15** — the loop-sentinel cron (`/api/cron/loop-sentinel`, every 4h) launches a cloud loop iteration whenever the graph is dormant with eligible work. Kill switch: set env `LOOP_SENTINEL=off` (or pause the cron in Vercel).
