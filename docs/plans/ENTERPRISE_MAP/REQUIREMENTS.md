...
| R-206 | Unattended loop iterations: a sentinel watches for dormancy and launches "run the loop" itself; the loop only ever stops on genuine access gaps or the four approval classes — technical answers are decided, never waited on | Matt 2026-08-15 GO ("not paused for me to say run the loop") | VERIFIED | `/api/cron/loop-sentinel` every 4h: kill switch, fresh-activity standdown, 3h cooldown, dry mode; launches a cloud agent with the canonical loop prompt; CURSOR_API_KEY in prod env |
