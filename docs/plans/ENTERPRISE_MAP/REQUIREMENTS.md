...
| R-051 | Named stops stand: I6, Tremor npm, PropXYZ purchase, new Public Product OS, page-grade as ship gate | handoff 2026-08-15 | LOCKED | — |
...
| R-206 | Unattended loop iterations with no dormancy gaps: a sentinel relaunches the loop the moment the previous iteration actually ends (state-based, never timer-based); stops only for access gaps or the four approval classes | Matt 2026-08-15 GO + "a 1-hour node must not sit 3 hours" | VERIFIED | `/api/cron/loop-sentinel` every 10 min (deterministic, token-free): kill switch, fresh-activity standdown, Cursor-API busy check on the last agent's newest run, 15-min boot guard, 12/day cost cap, dry mode |
