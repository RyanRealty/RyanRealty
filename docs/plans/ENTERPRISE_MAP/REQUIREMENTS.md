...
| R-213 | One generative product: xAI only (image, video, voice, content text) via `XAI_API_KEY`. Social/video/VO/stills go through `lib/grok-*.ts`. Third-party gen vendors are inventoried and canceled. Executor reads https://docs.x.ai/overview before any generate call | Matt 2026-08-16 ADD; D10 | PARTIAL | G32 done — chokepoint + cancel list live (`lib/grok-voice.ts`, `ci:xai-stack`). Cancel-now: fal + Synthesia. Cancel-after-cutover: ElevenLabs VO, Replicate, OpenAI vision, Anthropic producers |
