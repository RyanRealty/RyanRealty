...
| R-219 | Analytics is page-tied: every public URL classifies to one `page_type`. Layout owns the hit. New pages inherit the map; do not add a per-page GTM tag. Hunt leftover Follow Up Boss names out of analytics events and cookies | Matt 2026-08-18 (analytics rework) | VERIFIED | `lib/analytics/page-type.ts` + `ci:page-analytics`; GA4 `page_type` / `crm_person_id`; G34 |
