...
**Max:** G35 · M7 (the tail pin — G56 fails if rows above these numbers vanish or the pin goes stale)
...
| G6 | SMS agent to its plan's definition of done. **BLOCKED 2026-08-16** — accept is a live marketing-line text → reply → APPROVE stamp; hard limit forbids outbound SMS to Matt. Graph `c8bbccaa` carries the reason. Unblock: Matt texts APPROVE or authorizes one smoke to his cell. Residual shipped 2026-08-16: superuser per-broker toggle at `/admin/crm/settings/brokers` (does not satisfy accept). | CAP-035 | broker-tools |
...
| G15 | **DONE 2026-08-16** — FILTER_COMPLETENESS accept ledger complete. 268 long-tail concepts (222 custom + 46 standard) dispositioned, unexplained = 0. Prod TTFB p75 `/homes-for-sale` 275ms / `/homes-for-sale/bend` 254ms (target 600ms, n=8). A5 gated (VOW live; sold browse still legacy RPC). A8 measured (TTFB recorded; client filter-paint / pan-pin RUM residual). Gate `ci:search-completeness-accept`. | REQUIREMENTS R-097…R-106 | public-ux |
| G16 | CMA/pricing production residual: corpus rebuild under live judge, resolver flags, one engine across CMA/BPO/expired-audit, comp geography contract, send-to-reply funnel, listing Transparent-CMA, seller CMA price-opinion spine (R-218) | REQUIREMENTS R-069, R-070, R-073, R-074, R-083, R-112, R-218 | sales-insights |
...
| G21 | Public IA/mobile residual: nav coverage, city section-order fan-out to neighborhood/community, KB density, duplicate DOM streaming waste, dead-end map cards, sub-city scoping, interstitial stacking, sticky broker bar. Punch 2026-08-16: hero count grain locked (`placeHeroLead` + `ci:place-hero-grain`; fleet 97c68da5). Residual still open. | REQUIREMENTS R-107, R-108, R-109 | public-ux |
...
| G23 | Email residue kill: stop FUB-smart-campaign archived nurture sends via connected Gmail; purge FUB vocabulary and dead keys. **Partial 2026-08-18:** analytics no longer sends `fub_person_id` / advertises `fub_cid`. Residual is Gmail/FUB-smart-campaign send paths + CRM column names. | REQUIREMENTS R-147 | nurture |
...
| G34 | **DONE 2026-08-18** — public analytics is page-tied. One URL map (`lib/analytics/page-type.ts`) stamps `page_type` for GTM/GA4/VisitTracker/V3SectionTracker. `ci:page-analytics` fails a new public first segment until it is classified. GA4 dimensions `page_type` + `crm_person_id` registered. Leftover `fub_person_id` dimension relabeled. Identity bridge uses `gtag('set')`. Live SHA `02948bcd` READY `dpl_8prn4iFqQ6KxEXuKKPoM17JGDVZp`. | REQUIREMENTS R-219; CAP-031 | leads |
| G35 | **DONE 2026-08-19** — redirect-only public aliases emit HTTP 308 from `next.config.ts`. Next 16 prerender turned `permanentRedirect()` into a 200 empty shell (`/motivated-sellers`, `/feed`). Gate `ci:redirect-only`. Founding fleet 57eefae9 / df8ca55f. | REQUIREMENTS R-220 | public-ux |
