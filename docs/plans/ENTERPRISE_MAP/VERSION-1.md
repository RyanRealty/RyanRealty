...
| G11 | **PARK 2026-08-17 (Matt)** — Meta audience hold retired. Matt: stop checking Meta. The 7-day / 2026-08-22 calendar accept was the only thing holding INT-007 at FIX, and the streak was already 55 consecutive UTC days (last LIVE 2026-08-16T09:01:26Z, CRM received 13980), so the date was the sole unmet condition. INT-007 flipped **FIX→KEEP** on that evidence. Hold apparatus removed: `lib/data/loop/meta-audience-hold.ts`, `ci:meta-audience-hold`, `evalMetaAudienceHold`, the loop-g11 scripts, and the hold reads on `/admin/audiences` + `/admin/analytics/meta-health`. **Kept:** the 36h `evalAudienceSync` watchdog, which still turns red if the daily audience cron goes dark. Row parked, not deleted — the manifest may not shrink. | INT-007 | factory |
