...
| INT-036 | AdSense | `NEXT_PUBLIC_ADSENSE_CLIENT_ID` | `components/AdUnit.tsx` only (the sitewide load in `GoogleAnalytics.tsx` was removed 2026-09-23) | prod JS had `adsbygoogle` + `pub-592866…` on every route (2026-08-16); 2026-09-22 live probes: Auto-ads auctions on `/`, a listing and `/cities/bend` at 1440 and 375, **4 of 4 unfilled** (0x0), no fill observed | **Optional** | **green** | **KEEP (narrowed)** | Monetization narrowed to one explicit slot on `/tools/appreciation`, loaded by `AdUnit` only with marketing consent. **Why (UXLIVE-5, 2026-09-23):** Auto ads on brokerage pages cost trust and page weight on pages meant to convert (portals and competing brokerages buy real-estate intent, and Auto ads pick their own placement), and no fill was observed, so it earned nothing. `/activity` (the other slot) now 301s to `/housing-market`. Open for Matt: turn Auto ads off in the AdSense account too, since the tools page still loads the script. |
...
| INT-036 | prod JS pub-592866 + adsbygoogle 2026-08-16; sitewide load removed 2026-09-23 (UXLIVE-5), AdUnit-only on /tools/appreciation | n/a | optional live | n/a | Ads |
