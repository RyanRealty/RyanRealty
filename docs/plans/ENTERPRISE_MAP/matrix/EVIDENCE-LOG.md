...
| Server | Spans from 18:55Z, the first production build carrying the DSN | 28,763 spans sent in the hour to 19:57Z at 10% sampling (Sentry's count() showed 287,630: it scales each sampled span by 1/rate; count() 447,140 against count_sample() 44,714, read 2026-09-25), mostly `/subdivisions/[slug]`, `/communities/[slug]`, `/cities/[slug]`; tracing made opt-in (`SENTRY_TRACES_SAMPLE_RATE`) |
| Tracing rate | 1% from 2026-09-25 (Matt, after the cost research) | `SENTRY_TRACES_SAMPLE_RATE=0.01` on Vercel production; about 2.07M spans a month at the busiest measured hour. Every Sentry plan includes 5M spans; beyond that Team pay-as-you-go is $0.0000020 a span, and data past the included volume and any pay-as-you-go budget is dropped, not billed (docs.sentry.io/pricing, read 2026-09-25) |
