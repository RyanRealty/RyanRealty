...
| FAC-009 | Migrations discipline | `supabase/migrations` + `db:push` / `db:guard` | Count **461**; recent include the report price-bands ClosePrice migration in git (applied to hosted 2026-09-25) | F7 deliberately not in migrations (if still open) | **OPEN** hosted apply for new migrations |
