...
4. ~~Top bar~~ ✅ done (pass 7) — `TopBarScope` pins the "Everyone ▾" scope switcher in the mobile top bar on `/admin/crm` (mounts `BrokerScopeSheet`, Suspense-isolated, CRM-gated). Bell intentionally skipped (no distinct notifications center; would duplicate the Inbox/Activity bottom tabs).
