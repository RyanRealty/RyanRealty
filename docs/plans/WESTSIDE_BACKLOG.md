...
| 9 | **Review-ask consistency** — DESCOPED per Matt 2026-07-29 ("i don't need an engine, i already have an email template"). Remaining wiring only: on deal close, stage Matt's existing template as a ready-to-send CRM draft on the contact with `GBP_REVIEW_URL` (direct write-review link, now in `lib/brand/contact.ts`), so the ask never depends on memory. Matt sends. We are 5.0×25; nearest target Bend Premier's 78; Ladd Group 266. | Places API counts 2026-07-29 | Consistency, not automation, closes the review gap | S | SHIP-NOW (small) |
