# Current — 2026-09-21 06:25Z (Matt's five p0s all built; CI unblocked repo-wide; every check green locally)

Surface: Claude cloud `session_01BAjTFBeaGSMToksvaqRqip`, branch `claude/run-loop-14np9i`, tip `75b21c53f`, draft PR #351 (description rewritten 06:20Z — the original is superseded and two of its claims are corrected there). PR only — Cos Mini lands. Do not merge. HOLD owner email.

**The single most important thing in this block.** Seven `lib/cma/**` test files were red on `main`, and `.husky/pre-commit` runs the full unscoped `npm run test:unit`, so **every commit in the repository was blocked** and CI had been halting before the build for days. Nothing downstream had run: not the build, not route-smoke, payload, tap-targets, content-floor or a11y. Fixed in `ef9dd4eac`. Everything below about "pre-existing debt that only became visible" follows from that one fact, so do not read those as regressions from this branch.

## Matt's 2026-09-21 list — all five built

- **SITE-151 concessions — LANDED** `b1c55f366`. Builder + published offer now sit with each home. 17 of 38 communities have an identified builder; 10 rows carry a financing answer (8 a real program, 2 an explicit "none published, reviewed 2026-09-16"); 7 show builder only. **Compliance, verified structurally:** the code never reads `listing_private` / `private_data` / `PrivateRemarks` at all. Private remarks carry agents' personal mobile numbers and are MLS agent-only; they may inform, never publish. A test now fails if any concession string references private remarks or a phone number.
- **SITE-152 maps + coverage — LANDED** `6ea7dee64`, **half of Matt's complaint answered; the other half is now SITE-159.** There is exactly ONE map on the page and there never was a second (one `V3Atlas` import at page.tsx:93, one usage at :369, confirmed independently by the orchestrator). What reads as a second map is V3Atlas's own chip list — the real tap target, because cluster pins render under 20px — stacking below the canvas as a field of bordered pills with no separating label (`V3Atlas.css:1599` is a bare flex-wrap row with a 2xs top margin; `:1627` gives every chip its own 1px `--v3-edge` border). That chip list is load-bearing accessibility inside a SHARED primitive (city / neighborhood / subdivision / community / place-type / zip / team), so a route-scoped node correctly refused to restyle it. **Filed as SITE-159 (`bdcc996e`), open, which owns that shared-primitive change.** Coverage was already complete (all 38 render).
- **SITE-153 dog floater — LANDED** `97d3276f4`. Six doors in Matt's order, real flip motion, reduced-motion respected. **Open question for Matt:** it stayed bottom-right on research (moving a persistent contact affordance to an unconventional corner measurably hurts discovery) and was instead made solid navy and raised 4rem; it had been cream-on-cream beside the cookie chip, overlapping the listing filmstrip at 1440. If Matt meant put the dog IN the header, that pairs with SITE-155. **Real bug fixed underneath:** click-the-dog-to-close never worked — Radix's default modal Dialog sets `pointer-events: none` on the body and `aria-hidden` on every sibling of the portal, and the trigger lives outside `DialogContent`, so the second tap was unclickable and the trigger was hidden from assistive tech. `modal={false}`. The old gate asserted that behavior worked and had never checked in a browser.
- **SITE-154 listing videos — LANDED** `59306105f` (+ `3a0875dd0` test fixture). We were fetching every video Spark has and throwing it away: `lib/sync/deltaSync.ts` requested `Videos` in its expand on every scheduled run and never wrote `listing_videos`; `lib/listing-mapper.ts` touched the payload once to set `hasVirtualTour` for a quality score, then discarded the URLs. The only writer was a manual admin action — hence **8 rows across 596,823 listings**. Now wired into the scheduled sync with a bounded backfill. **Correction to an earlier claim in this file:** the orchestrator told Matt "we fetch every video and throw it away, so no listing has one." The first half is right, the conclusion was wrong — `youtube.com/embed/HuSxin6MetA` is live on a production listing, verified independently. The home Matt named has no video even at source; it has a Zillow 3D capture in `virtual_tour_url`, which is the 3D tab he was already looking at.
- **SITE-155 chrome — THREE OF FOUR LANDED** `4d40b083d` + `1527ad190`. "Work with us" removed (the 2026-09-19 CTA-lock reversal; the dog takes that job). Avatar-only, no name, `referrerPolicy="no-referrer"` so the Google profile image loads. Header clips at every width, not only under 640px: a CLOSED mega panel's `visibility: hidden` box still widened `scrollWidth` between roughly 860 and 1200px — measured at `/cities/bend` at 1024px, panel right edge 1155px against a 1024px viewport. Search collapses to an icon through the whole 1280-class bar (56.25–84.99rem), matching the CSS range so JS and CSS cannot drift. **The fourth defect, the scroll jump, is NOT the search box and is filed as SITE-158 (in progress).** It is `app/loading.tsx`: a ~2,840–2,910px five-section fake-homepage skeleton that streams first for EVERY route before a shorter route skeleton replaces it. Scroll during that window and the document collapses, so the browser clamps `scrollY` toward 0. Measured: `/homes-for-sale/bend` 1400 → 1012 → 10; `/cities/bend` 1400 → 66; `document.activeElement` stayed `body` throughout, ruling out a focus steal.

## Also landed, none of it from Matt's list

`75b21c53f` **ci:a11y** — `AvatarFallback` carried `delayMs={0}`; any defined `delayMs` makes Radix's `canRender` start `false`, so the node is absent from SSR and the first client render and arrives from a `setTimeout`. In that gap it carries only un-overridden shadcn defaults: 1.39:1. Neither avatar ever sets an `AvatarImage`, so the delay guarded nothing. Contrast now sits on the element as `bg-navy text-cream`. Side effect: `/reviews` had 25+ reviewer initials rendering fully invisible by the same flaw · `083fb9563` listing crumb readable at every phone width · `365e7b3b9` neighbourhood place pages stop scrolling sideways (culprit was the search chip row, not the map; `<body>` overflow-x does not clip because of the CSS2.1 html/body propagation rule) · `364ffbd76` `scripts/measure-route-fit.mjs` · `8b1ba182f` communities hero photos were truncated at PostgREST's 1,000-row cap so only 6 of 16 reached the page · `40f1fa904` the `/team` 28x28 carousel arrow, the only unexcused tap target on the public site · `f60b8704f` Bend Oktoberfest stopped advertising a past date · `92e026b8b` three content floors re-seeded.

**That last one deserves care if you revisit it.** Two of the three floors were seeded against our own wordmark: `logo-horizontal-navy-transparent.png` is natively 2271x454, and the old floor of 2043 is that times the gate's 0.9 tolerance, on routes that carry no photographic hero by design. The third was seeded at 256px, a width its card has never rendered at. New floors are seeded against real inventory photos and still fail the banned 320x240 thumb. Reasoning is inline in each parity.json. **`ci:route-content-floor` is NOT a sandbox flake** — an earlier note in this file said so, a clean CI runner reproduced it, and the public correction is on PR #351.

## Environment, for whoever runs next here

The box OOM-kills the largest process once a few lanes hold dev servers. `next build` dies; `tsc` comes back "Killed" and `ci:commit-compiles` then reports "COMMIT IS NOT SELF-CONTAINED", which is NOT a type error — check `free -m`, stop the dev servers, re-run that one gate. It happened again at 06:05Z and passed on the re-run with memory free. A killed dev server leaves `.next/dev/lock`; delete that one file by name (a repo hook refuses recursive and glob deletes). `vitest` is the real runner: `node --test` on a vitest-authored file reports a vacuous pass and is not signal, and `vitest.config.ts` sweeps every `scripts/__tests__/**/*.test.mjs` into the mandatory gates project, so author those in vitest. Disk fills at roughly 1.5GB per worktree; `git worktree unlock` then `remove --force` on finished lanes.

To run a DAL function under plain `tsx`, stub `server-only` and `next/cache` in the CJS loader before importing — see the header of `scripts/_ws4-verify-email-delivery.mts`. A scratch script must live inside the repo (`scratchpad/`, gitignored) or module resolution for `@/` fails.

## Open, for Matt

1. **Dog in the header, or bottom-right as shipped?**
2. **New-construction snapshot is five days stale and the gap is widening.** Verified today through the DAL with `bendNewConSearchFilter()`: live Active Bend new-construction is **214 de-duplicated / 216 raw MV head-count**, against the **185** stamped `2026-09-16 PT` in `BEND_NEW_CON_HEADLINE`. This is **not** a published error — `app/new-construction/page.tsx:328` prints the LIVE count and falls back to 185 only if the read fails, and the eyebrow says "researched 2026-09-16 · live counts today". But the median (`$699,900`) and the price band (`$185K–$5.3M`) in the tiles beside that live count are still snapshot values, and inventory is up ~16% in five days. Re-research is a node, not a side effect, because adding communities means per-community builder research.
3. **CORRECTION — Pronghorn is not mistagged.** An earlier entry in this file guessed it was "a resort community mistagged `City='Bend'`, same pattern as Caldera Springs". That is wrong. `data/resort-communities.json` records Pronghorn / Juniper Preserve as `city: "Bend"`, `city_slug: "bend"` — the resort's own footer address is 65600 Pronghorn Club Drive, Bend, Oregon 97701. `City='Bend'` is correct and the count is not inflated. What IS true: 2 Pronghorn rows appear in the live Bend new-construction set while the curated 38-community table does not list that community, so those homes show as map dots with no row. Minor coverage gap, folded into item 2.
...
- **Read this first if you pick up SITE-137 or SITE-138.** Both nodes name ONE example fixture and **both fixtures read clean on production**. A one-shape check closes them as stale. They are not stale. The counter-query (six widths x four listings x five place pages) is on each node's evidence, and is now one command: `node scripts/measure-route-fit.mjs --preset listing-crumb|place-overflow`.
- **SITE-137** (`083fb9563`). `828 NW Florida Avenue` fits (right 352); every longer path did not — Horizon 535, Night Owl 532, Applewood 584, at 360/375/390/393/412/430, `text-overflow:clip`, no ellipsis, and the page does not scroll, so the address was unreachable. `485c1dba`/`ab130e38` pinned every overlay-compact rung to `max-content` with `overflow:visible`, which only holds while the whole path fits. Now: the list clips its own box, ancestors `flex 0 50 auto` ellipsize first, the address rung `flex 0 1 auto` holds until it alone cannot fit. Both levels need `min-width:0`. `scripts/lib/listing-fold-density.mjs` had been ENFORCING the broken shape; its assertions are inverted.
- **SITE-138** (`365e7b3b9`). `/cities/bend` is clean at every width — the overflow is on the NEIGHBOURHOOD routes (`old-bend`, `awbrey-butte`: +29 at 360, +14 at 375). **The map was not the culprit**: the escaping node is the embedded search's `.srch-chip-actions` row below the map. `<body>`'s `overflow-x-hidden` does not clip it because the CSS2.1 html/body propagation rule hands a plain body overflow to the viewport; `/cities/bend` only escapes via `.city-page` on its `<main>`. Neighborhood `<main>` now carries `.nbh-page`. The **clipped-chip half was already satisfied** — zero clipped edges measured on `.rr-map-mark` / `[data-price-pill]` / `[data-subject-ring-label]` — so nothing was invented for it. SITE-127 hover verified intact at 1440.
- **THE BIG ONE — `ef9dd4eac`.** Seven `lib/cma/**` test files were red on clean `f86ade9d5`, so `.husky/pre-commit`'s unscoped `npm run test:unit` **blocked every commit in the repo**, and CI on `main` had been stopping at that step — skipping build, route-smoke, payload, tap-targets and content-floor **for days**. Root cause for four: the comp-matrix arc row joined `entry.endLabel` instead of `entry.outcome`, so a subject with no ask printed nothing and a client was never told their home is not listed. Three others asserted contracts Matt changed after they were written (commits cited inline). `cover-value.ts` now falls back to the tiers when Recommended sits outside the printed band.
- **Two gate findings that are NOT this branch's, deliberately not laundered.** (1) `ci:tap-targets` fails on a 28x28 `button[name="next slide"]` at `/team`, stable across two runs, in since the 2026-09-16 team carousel — invisible until now because CI never reached the step. NOT baselined (that list is shrink-only) and `/team` not restyled (it is another node's page). Proposed patch: `min-h-11 min-w-11` on the two carousel controls in `app/team/_v3/TeamClosings.tsx`, scoped there so the listing hero is unaffected. **This needs a node.** (2) `ci:route-content-floor` does not give a stable verdict in this sandbox: run 1 flagged `/oregon/medford` + `/communities/tetherow/types/single-family`, run 2 flagged those plus `/search` on a different metric. The gate counts only images that finished loading and this sandbox drops CDN egress. None of the three routes is in the diff.
- **Also landed:** `scripts/measure-route-fit.mjs` + vitest test. Note for tooling lanes: `vitest.config.ts` sweeps every `scripts/__tests__/**/*.test.mjs` into the mandatory `gates` project, where a `node:test`-authored file prints a real passing TAP run and is THEN marked "No test suite found". Author those in vitest. `node --test` on a vitest-authored file reports a vacuous pass — it is not signal.
- **Open finding for whoever owns `V3Atlas.client.tsx`:** `/cities/bend`'s overflow magnitude is unstable run to run (0, 1976, 0 across three identical measurements), all culprits tracing to the inline SVG road paths under a deliberate `overflow:visible`. Looks like a render-timing race.
- Verified: `test:unit` 1122 files / 11957 tests green, `ci:gates` 115/115, `next build` pass, `ci:route-smoke` + `ci:page-payload` pass.
- Skills read: `.claude/skills/site-queue/SKILL.md` (full), `CLAUDE.md`, `docs/plans/ENTERPRISE_MAP/SESSION_HANDOFF.md`.
