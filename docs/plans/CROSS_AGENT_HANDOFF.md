...
| **`main` @ commit** | `f0e49e2` — pushed to origin/main, `Approved-by: matt`. 6 commits this session: AI-citability layer · G34 gate · robots.txt crawler allow-list · homepage+city parity contracts · handoff refresh · neighborhood Dataset/FAQ. |
...
| **NEXT (fresh session, queue)** | AI-citability is DONE (do not redo). Remaining Wave 3 work toward "a working website": 1. **Route rebuilds** onto site-v2 blocks: `/zip/[zip]`, `/search`, `/sell`, `/about`, `/team` (the neighborhood page `/cities/[slug]/[neighborhoodSlug]` still has a LEGACY structure under its now-correct JSON-LD — a full site-v2 rebuild is still owed there, but its AI citability is complete). 2. Optional: `llms.txt` dynamic-stats enrich (low value — only Claude+Perplexity consume it). **AI-citability pattern to REUSE for any new page:** `buildMarketFaq(name, pulseOrFallback)` → `<MetadataBlock schemas=[breadcrumb, place, dataset]>` + `<FAQBlock items={faqs}>`; G34 (`scripts/check-ai-structured-data.mjs`) enforces it. Approach: verified data only (trace to `market_pulse_live` / `geo_snapshot_mv`), gate every rule, dev-server curl to confirm JSON-LD in SSR before showing Matt. ⚠️ Working tree carries OTHER sessions' uncommitted LP-form `ci:design-tokens` violations — do not `chmod +x .husky/pre-push` until those clear (it gates on the working tree and would block all pushes). |
