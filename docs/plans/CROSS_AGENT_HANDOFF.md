# Current — 2026-09-21 (SITE-162 Tetherow first look)

Surface: Grok Build `grok-build-01a0c44c-162`. Worktree `wt/site-162-20260921` on SITE-156 leftover `6ae9769c8`. Cos Mini-lands. Do not merge. HOLD owner email. Never `npm run push` from this worktree.

- **SITE-162** Tetherow first look. Node `f2c635c2-bb92-48c9-a939-97a26f0023bb`. Community fold is now V3PlaceLook (Google + one ring + priced photo cards), same lock as neighborhood. Looked `/communities/tetherow` 1440+375: two priced cards in the first viewport (`$2.2M` 19372 Seaton Loop, `$2.9M` 19492 Bainbridge Court, beds/baths/sqft); 375 chip soup gone (clusters `10`/`4`, 0 measured overlaps). Caldera same template, not regressed. Named-peer shots in `look-pass-site162-tetherow-2026-09-21/` (tetherow.com golf+lodge vs our live MLS houses). Receipt-bound `desktop.png`/`mobile375.png`/`value-open-*` left at HEAD (SITE-94). Rebaseline is not done. SITE-156 Caldera leftover stack kept (hit-only plats, named amenities, empty Typical-price omitted, sticky off, empty placeholders).
- **SITE-156** still on this branch (`6ae9769c8`). Compare after push: `wt/site-162-20260921`.
