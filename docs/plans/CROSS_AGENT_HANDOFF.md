# Current — 2026-09-18 (SITE-128 Tip Ready #5 — other-subdivs rail + SITE-127 $ consistency)

Surface: Cursor cloud `bc-c0264151-2f08-51ef-9d67-c6a31d02ed15`. Tip SHA `b6ecd4c5f` on `cursor/site-128-other-subdivs-pin-ed15` / draft PR #314. Tipped against latest `origin/main` `a873fe9ea` (first-look + map hierarchy + amenity + naming/crumbs). Bend @375 paint tip `f7aa8fc7e` / PR #312 is Tip Ready awaiting Mini-land — Cos Mini-lands that Bend fix first. This tip does not stack on `f7aa8fc7e`. PR only — Cos Mini lands. Do not merge. HOLD owner email. SITE-128 claim stays public-look. SITE-127 claimed `cursor-grok-4.6-2026-09-18`.

- **Keep-exploring.** `otherCommunitySubdivs` — sibling subdivs in the same community. Name-only cards. Drops self, permit-glued slugs, withheld MLS tokens. No sales sort, no city dump. Listing `#other-subdivs` after similar homes (empty on a city frame). Subdivision `#other-subdivs` from parent community/neighborhood GIS plats + registry child aliases, minus GIS/resort nearby already listed. Community/neighborhood `#child-places` stay name-only.
- **SITE-127 $.** Atlas cluster pills print `735K` / `735K+` / `$1.5M` via `formatAtlasClusterPin`. No mix of bare counts vs $ pills. Count stays on `data-atlas-cluster-n`. Hover still names homes + range.
- **Prior locks.** first-look refuse, hierarchy lock, amenity layers, naming/crumbs untouched. No geom invent.
