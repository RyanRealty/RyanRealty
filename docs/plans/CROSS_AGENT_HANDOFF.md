...
| Field | Value |
|--------|--------|
| **Surface** | **Claude Code (Opus 4.8, 2026-05-29)** — Ryan Realty website rebuild · Wave 3. SHIPPED this session: all 27 community + neighborhood pages content-rich, subdivision pages, and the spatial boundary-map fix. Next: render remaining LP-depth fields + homepage `parity.json`. |
| **`main` @ commit** | `8329bd0` — pushed to origin/main, `Approved-by: matt`. 55 files. Hosted DB migrations `20260529010000`–`050000` applied (matching repo). |
| **Working tree** | Clean of MY work (all committed in 8329bd0). The remaining dirty/untracked files are OTHER sessions' (lead-capture, contact forms, tracking, LP forms, `.gitignore`, meta-marketing, `_712-*` scripts, FUB docs, etc.) — leave them. Note: `.husky/pre-push` lost its executable bit (push warned + skipped it; `chmod +x` to restore). |
| **Task focus (SHIPPED — do NOT redo)** | (1) **Rich content, 27/27, gate G33 100%.** Every resort/planned community + Bend neighborhood has a verified, primary-source-cited `data/resort-community-<slug>.json` (overview prose, at-a-glance facts, drive times, amenities; citations embedded as the audit trail). Shared renderer `components/site/CommunityRichContent.tsx` used by `/communities/[slug]` + `/cities/[slug]/[neighborhoodSlug]`. Loader `lib/resort-community-content.ts`. Gate `scripts/check-community-content.mjs` + `data/community-content-baseline.json` (empty = locked at 100%). Directives D95/D96. (2) **Subdivisions:** `/communities/[slug]` shows a "Subdivisions within" section + broken-out map polygons, each linking to `app/subdivisions/[slug]/page.tsx`. Membership via `community_subdivisions` RPC (majority-overlap + area-guard, NOT centroid). D94. (3) **Spatial:** in-boundary listings from precomputed `listing_boundary_xref_mv` via `listings_in_boundary` (killed the request-time ST_Within anon-3s-timeout); resort polygons corrected to authoritative county-GIS plat unions (Tetherow 5,718ac→699ac); DALs throw-not-swallow to avoid empty-map cache poison. D93. (4) Homes-only filter, 6-communities+view-all, ExitIntentPopup removed. |
| **NEXT (fresh session, queue)** | 1. **Render the remaining LP-depth** in `CommunityRichContent` — resort configs already carry `course_specs`, `course_rankings` (only one row shown), `membership_tiers`, `builders`, `signature_hole`; the renderer doesn't show them yet. 2. **Homepage `parity.json`** + remaining Wave 3 route rebuilds. 3. 2 pre-existing `ci:design-tokens` failures (Buyer/Seller LP forms raw primitives) — not mine; fix or baseline separately. Approach: verified primary-source research (cite, never invent), gate every new rule, verify programmatically + visually before showing Matt. Memory: `reference_community_content_system.md`, `reference_spatial_in_boundary_mv.md`. |
