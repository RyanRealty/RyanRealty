...
**Self-starter (same day — Matt GO on R-206: "not paused for me to say run the loop"):** `/api/cron/loop-sentinel` every 4h (vercel.json, G53 green): dormancy check (no in_progress node fresh <3h) + open work exists + 3h cooldown (sync_logs `loop_sentinel:launch`) + kill switch `LOOP_SENTINEL=off` → POST api.cursor.com/v1/agents (Basic auth CURSOR_API_KEY — added to Vercel prod env this session) launching a cloud agent with the canonical grind-until-blocked prompt (hard limits: no sends/posts/spend/OAuth/SkySlope). Dry mode `?dry=1`. Lib: `lib/data/loop/sentinel.ts`. R-206 VERIFIED. **Concurrent-session note:** the sentinel stands down when any session has a fresh claim; human sessions and sentinel agents share the graph's claim mutex.
