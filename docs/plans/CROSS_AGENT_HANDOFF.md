# Current — 2026-09-23 (Vault mail index, client transaction page, misfile found — PR, production run waits on Matt)

Surface: Claude cloud session, branch `claude/transaction-management-optimization-6jdt16`, draft PR (not main). Nothing written to production: the auto-mode classifier refused production data writes and a Google Vault scope probe, so the migration, the misfile correction, the history backfill and the live alias test all wait on Matt's go-ahead. Everything below was built, unit-tested and read-only-audited against the real mailboxes.

- **Found live: the 2026-08-23 mail filer misfiled 3,185 of its 3,295 filings (read-only audit, `docs/audits/TC_MAIL_AUDIT_2026-09-23.md`).** A SkySlope-imported contact put `matt@ryan-realty.com` on 56111 School House Rd (closed) as "other agent", so every email in Matt's inbox matched it (3,077 filings, 88 documents, checklist links on a closed file); shared vendor contacts (TC firm, Western Title, First American) put mail for one property onto another (Tumalo 77, Simpson 15, Beaumont 6, Impala 3, Sedalia 7). Still running every 15 minutes in production until this PR deploys. Fastest stop without a deploy: null the email on that one contact row (`tc_deal_contacts` School House, role other_agent); the refused write is in the PR description.
- **Also found: the production filer never read a PDF.** `readPdfPagesText` handed a Node Buffer to pdfjs, which rejects it; every auto-filed document was classified `unknown`. Fixed with a copy (test proves red → green).
- **Built: the Vault mail index.** `lib/tc/mail-rules.ts` (pure rules, 42 tests incl. every live misfile), `lib/tc/mail-index.ts` (index/sweeps/rematch/auto-open), `tc_mail_messages` + `tc_offers` source columns (migration `20260923180000_tc_mail_index.sql`, not applied), wired into `/api/cron/crm-gmail-sync`, new daily `/api/cron/tc-mail-sweep` (registered). Rules doc: `docs/TC_MAIL_FILING_RULES.md`. Offers on our listings are recorded whether or not anyone replied, with "Not yet sent to seller" flagged (OAR 863-015-0250(1), 863-015-0135(2), verified 2026-09-23, added to `docs/TC_OREGON_COMPLIANCE.md`).
- **Read-only audit** (`npx tsx scripts/tc-mail-backfill.ts audit --since 2026-08-22`): 5,338 messages; the rules file 372 across 19 deals (closed files get their post-close and forwarded mail), queue 53 and leave none ambiguous; 52 of the 53 are one real deal with no file: **909 NW Delaware Ave** (offer → escrow opened → inspections → title → "CLOSING TODAY" → final settlement statement). The daily sweep auto-opens a file when queued mail proves a deal is under way (escrow opened / settlement / executed agreement); offers alone never open one.
- **Built: surfaces.** Deal page: Email section, Client conversations (CRM calls/texts/notes for the file's clients, broker-only), offer email trail + "Not yet sent to seller", Share with client per document. `/admin/closings/mail` queue (brokers: own mailbox; principal: all), board link, Today card, nav child. Client page `/account/transactions` (+ `[dealId]`): confirmed-email identity → CRM person → `tc_deal_people`; milestones, what's next (Sign now mints a fresh link), key dates, signed/shared documents, team; never call notes, filed mail, other offers, commission, principal review (`lib/tc/client-portal.ts`, tests). Checked read-only against the test files for Marketing Test Lead and Vault Test Buyer; fixed a leak (buyer saw the seller's listing agreement go out) before shipping.
- **Order to run once Matt says go:** merge + deploy → `apply_migration 20260923180000_tc_mail_index` → `npx tsx scripts/tc-mail-backfill.ts reconcile --since 2026-08-22` → `sweep-deals` (all history) → `sweep-transactions` → `npx tsx scripts/tc-alias-e2e.ts run` (11-step listing → lowball → counter → accept → escrow → close → post-close with the canon aliases; sends email between our own mailboxes).
- **Google Vault**: the service account can list matter "COMPLIANCE EMAILS" (ediscovery.readonly). The index uses the Gmail API (live mailboxes); deleted mail only exists in Vault, which needs an export and the full `ediscovery` scope, which is an OAuth grant for Matt to decide.
- Skills read: `.claude/skills/tc-builder/SKILL.md`, CLAUDE.md §0/§1/§3/§7/§8.
