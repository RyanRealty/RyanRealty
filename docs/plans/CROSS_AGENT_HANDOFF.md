# Current — 2026-09-21 (Matt's five p0s: four landed, chrome outstanding; CI unblocked repo-wide)

Surface: Claude cloud `session_01BAjTFBeaGSMToksvaqRqip`, branch `claude/run-loop-14np9i`, tip `6ea7dee64`, draft PR #351. PR only — Cos Mini lands. Do not merge. HOLD owner email.

**The single most important thing in this block.** Seven `lib/cma/**` test files were red on `main`, and `.husky/pre-commit` runs the full unscoped `npm run test:unit`, so **every commit in the repository was blocked** and CI had been halting before the build for days. Nothing downstream had run: not the build, not route-smoke, payload, tap-targets or content-floor. Fixed in `ef9dd4eac`. Everything below about "pre-existing debt that only became visible" follows from that one fact, so do not read those as regressions from this branch.

## Matt's 2026-09-21 list
...
- **SITE-152 maps + coverage — LANDED** `6ea7dee64`, **but Matt's complaint is only half answered.** There is exactly ONE map on the page and there never was a second (one `V3Atlas` import at page.tsx:93, one usage at :369, confirmed independently by the orchestrator). What reads as a second map is V3Atlas's own chip list, the real tap target because cluster pins render under 20px, stacking below the canvas as its own bordered box at 375. That chip list is load-bearing accessibility inside a SHARED primitive (city/subdivision/community/team/zip), so a route-scoped node correctly refused to restyle it. **"The maps are confusing" still needs that shared-primitive change.** Coverage was already complete (all 38 render); the page now states its rule with counts derived from the same arrays it maps over, plus a FAQ entry in the existing FAQPage JSON-LD.
...
- **SITE-154 listing videos — NOT STARTED, but ROOT-CAUSED. Read this before writing any UI.** We fetch every video Spark has and throw it away. `lib/sync/deltaSync.ts:71` requests `Videos` in `DELTA_SYNC.EXPAND` on every scheduled run; `grep -i video` over that whole file returns only that line, so the sync never writes `listing_videos`. `lib/listing-mapper.ts:361` touches the payload once, to set `hasVirtualTour` for a 5-point quality score, then discards the URLs. The only writer is `replaceListingVideosForKey` (`lib/data/sync/syncWrites.ts:381`), called only from `app/actions/sync-spark.ts:89`, a manual admin action. Hence **8 rows across 596,823 listings**. The fix is a sync change plus a backfill, not a player. The home Matt named has no video even at source; it has a Zillow 3D capture in `virtual_tour_url`, which is the 3D tab he was already looking at.
- **SITE-155 chrome — OUTSTANDING.** Lane was running for 2.5h with only `V3Chrome.tsx` modified and no writes in 90 minutes; asked for status. Four defects: scroll jump on load, remove "Work with us" (the 2026-09-19 CTA-lock reversal), Google avatar instead of the name, responsive with nothing clipped.

## Also landed, none of it from Matt's list

`083fb9563` listing crumb readable at every phone width · `365e7b3b9` neighbourhood place pages stop scrolling sideways (culprit was the search chip row, not the map; `<body>` overflow-x does not clip because of the CSS2.1 html/body propagation rule) · `364ffbd76` `scripts/measure-route-fit.mjs` · `8b1ba182f` communities hero photos were truncated at PostgREST's 1,000-row cap so only 6 of 16 reached the page · `40f1fa904` the `/team` 28x28 carousel arrow, the only unexcused tap target on the public site · `f60b8704f` Bend Oktoberfest stopped advertising a past date · `92e026b8b` three content floors re-seeded.

**That last one deserves care if you revisit it.** Two of the three floors were seeded against our own wordmark: `logo-horizontal-navy-transparent.png` is natively 2271x454, and the old floor of 2043 is that times the gate's 0.9 tolerance, on routes that carry no photographic hero by design. The third was seeded at 256px, a width its card has never rendered at (the token has had one value since 2026-09-02). New floors are seeded against real inventory photos and still fail the banned 320x240 thumb. Reasoning is inline in each parity.json.

## Environment, for whoever runs next here

The box OOM-kills the largest process once a few lanes hold dev servers. `next build` dies; `tsc` comes back "Killed" and `ci:commit-compiles` then reports "COMMIT IS NOT SELF-CONTAINED", which is NOT a type error — stop the dev servers and re-run. A killed dev server leaves `.next/dev/lock`; delete that one file by name (a repo hook refuses recursive and glob deletes). `vitest` is the real runner: `node --test` on a vitest-authored file reports a vacuous pass and is not signal, and `vitest.config.ts` sweeps every `scripts/__tests__/**/*.test.mjs` into the mandatory gates project, so author those in vitest.

Note for the next orchestrator: five pushes tonight each cancelled the previous CI run, so no full run completed. The checks are fixed; the board has just never been allowed to settle.

## Open, for Matt

1. Dog in the header, or bottom-right as shipped?
2. The chip list needs separating from the map canvas — shared primitive, needs its own node.
3. Live new-construction total is 211 de-duplicated vs the 185 in the 2026-09-16 snapshot, with ~7 new one-listing subdivisions plus Pronghorn (7 listings, likely a resort community mistagged `City='Bend'`, same pattern as the already-excluded Caldera Springs). Adding them means per-community builder research, so it is a node, not a side effect.
