...
| Field | Value |
|--------|--------|
| **Surface** | **Claude Code (Opus 4.8, 2026-05-29)** — Ryan Realty website rebuild · Wave 3. `/communities/[slug]` REBUILT to site-v2 composition + verified (programmatic + visual). Root-caused & fixed the empty-map-pins/empty-cards bug: `listings_in_boundary` was doing request-time `ST_Within` (~10s cold) that blew the anon 3s timeout. Replaced with a precomputed `listing_boundary_xref_mv`. **Draft is UNCOMMITTED awaiting Matt's "ship it"** (he gated commit on cards working — they now do). Next after approval: richer community content + homepage `parity.json`. |
| **`main` @ commit** | `6bcfd27` (unchanged — nothing committed this session yet). Hosted DB IS ahead: migrations `20260529010000_listings_onmarket_point_gist`, `20260529020000_listing_boundary_xref_mv` (MV + RPC + refresh fn), `20260529030000_drop_broad_point_gist` applied. Repo migration files written to match. |
| **Working tree (this session's uncommitted draft — stage these on approval)** | `app/communities/[slug]/page.tsx` (601→~235, site-v2 composition), `lib/data/geo/getGeoBoundaryMapData.ts` (cache key v2), `app/api/cron/refresh-mvs/route.ts` (+xref refresh), 3 new `supabase/migrations/20260529*.sql`, `docs/DATABASE_FOR_AI_AGENTS.md` (§4d + MV rows), `docs/DESIGN_DIRECTIVES.md` (D93), `docs/DATABASE_SCHEMA_SNAPSHOT.md` + `docs/DAL_INDEX.md` (regenerated). **DO NOT stage** the pre-existing non-mine churn (lead-capture, contact forms, tracking, LP forms, `.gitignore`) or the 3 pre-existing `ci:design-tokens` failures (ad-roi `h-[40rem]`, Buyer/Seller LP forms raw primitives). |
| **Task focus** | **DONE: every community + neighborhood page is content-rich (G33 27/27), AND resort communities now link to per-subdivision pages.** Newest: built `app/subdivisions/[slug]/page.tsx` (boundary map + homes for sale via the gated DAL, graceful empty state); the "Subdivisions within" cards + map polygons on `/communities/[slug]` link to it. Fixed `community_subdivisions` RPC to majority-overlap + area-guard (was centroid-in-polygon, which wrongly pulled in a 1,072-acre Broken Top plat); Tetherow now lists 22 plats all 100% inside its boundary. Widened `getGeoBoundaryMapData` GeoType to include 'subdivision'. Verified: /subdivisions/tetherow-phase-1 (25 homes), golf-homes-at-tetherow (3), tetherow-phase-3 (empty state). Gates green (G31/G33/page-dal/seo-routes/static-params; design-tokens flags only 2 pre-existing LP forms, not mine). All 14 resort/planned communities + all 13 Bend neighborhoods have verified, cited `data/resort-community-<slug>.json` configs (researched via primary-source subagents, brand-voice clean, valid JSON). Shared renderer `components/site/CommunityRichContent.tsx` shows overview prose + at-a-glance facts (hidden when a geo has no resort facts) + drive times + amenities; used by BOTH `app/communities/[slug]` and `app/cities/[slug]/[neighborhoodSlug]` (neighborhood page wired this turn, keyed by `${citySlug}-${neighborhoodSlug}`; redundant "homes on the map" heading removed there too). Verified: awbrey-butte + broken-top/sunriver/pronghorn/northwest-crossing render. Gates green: G33 27/27, G32, G25, G1. **Earlier turns (same uncommitted draft):** spatial MV card/pin fix, Tetherow polygon plat-union, subdivisions broken-out map + section (D94), homes-only filter, 6-communities+view-all, ExitIntentPopup removed, resort rich content (D95/D96). This turn: built the hard richness gate G33 (`scripts/check-community-content.mjs`, D96) + coverage baseline (`data/community-content-baseline.json`); researched 13 communities via verified, cited subagents and authored `data/resort-community-<slug>.json` for broken-top, pronghorn, eagle-crest, sunriver, caldera-springs, black-butte-ranch, brasada-ranch, northwest-crossing, crosswater, awbrey-glen, widgi-creek, vandevert-ranch, three-rivers (+ tetherow already). All brand-voice clean, valid JSON, primary-source citations embedded as the audit trail. Pages render overview prose + at-a-glance facts + drive times + amenities (verified broken-top/sunriver/pronghorn/northwest-crossing). ⚠️ WAVE 2 — the 13 Bend neighborhoods (`bend-*`) are still in the G33 backlog: they render at `/cities/[slug]/[neighborhoodSlug]` (a DIFFERENT page than /communities) and need (a) that page enriched, (b) a neighborhood-shaped content config (character/parks/schools/typical-homes, NOT golf/HOA), (c) verified research per neighborhood. **Prior turns (same uncommitted draft):** Tetherow LP-depth, subdivisions broken-out map + section (D94), polygon plat-union fix, card MV timeout, homes-only filter, 6-communities+view-all, ExitIntentPopup removed. NEWEST: rich content (overview prose, at-a-glance facts card [founded/acres/architect/HOA/recognition], drive times, amenities) from `data/resort-community-[slug].json` via the server-only `getResortCommunityContent` loader (`lib/resort-community-content.ts`); enforced by G32 + D95. Removed the global "Before you go" ExitIntentPopup from `app/layout.tsx`. ⚠️ ONLY Tetherow has an authored content JSON — the other 13 resort communities render sparse until verified content configs are authored (cannot invent founded/architect/HOA). Remaining LP sections not yet ported: course specs, membership tiers, builders, signature hole. **Earlier:** subdivisions broken out on map + section (D94, gated RPC), polygon plat-union fix (5,718ac→699ac), card MV timeout fix, homes-only filter, 6-main-communities+view-all. NEW this turn: removed redundant "homes on the map" heading (D94); subdivisions broken out as SEPARATE polygons on the map + a "Subdivisions within Tetherow" section (21 plats + home counts) via the gated `community_subdivisions` RPC → `getCommunitySubdivisions` DAL (mig `20260529050000`); enforced by G32 (now requires the import) + D94. ⚠️ Subdivision cards are NON-clickable for now — the dedicated subdivision detail page (resolve `geo_type='subdivision'` slug on `/communities/[slug]` with spatial homes + market context) is the next focused build. Earlier this turn: card timeout (MV), homes-only filter, 6-main-communities+view-all, AND the polygon. **POLYGON ROOT CAUSE:** the `boundaries` resort rows are crude "Ryan Realty spatial discovery v6" hulls (Tetherow = 10-pt, **5,718 acres**, swallowed Broken Top/Sunrise Village → 149 wrong homes). FIX: replaced with the authoritative **ST_Union of Deschutes County GIS subdivision plats** (Tetherow = 269-pt, **699 acres**, 29 real homes). Migration `20260529040000`. Map fitBounds verified zoom 13 centered on the polygon; stroke bumped to weight 3. **⚠️ PENDING: the OTHER 13 resort communities still have the bad spatial-discovery hulls** — apply the same plat-union fix per community, but each needs verification for false-match traps (e.g. "Tetherow Crossing" near Redmond at lat 44.31 is NOT Tetherow; the union excludes `%crossing%` + lat≥44.1). Awaiting Matt approval to commit + confirm plat-union is the "polygon in supabase" he meant. Then homepage `parity.json`. |
