...
# Current — 2026-08-16 (cursor-loop-chain) — Southern Crossing 1/23/3 finding rejected

**Surface:** Cursor cloud `bc-88ef6b93` (`cursor/loop-chain-2026-08-16t23-20-dd33`). **Time:** 2026-08-16 ~23:35 UTC. Brief served fleet finding `08704f1f` (public-ux / fleet `8a95d715ef63989d964b9d643d2938f4`), not G16/G32. One node only. No product change. **`main` @** `1faedbd9b` + this stamp. Production already **READY** on the shipped neighborhood inventory class (`9cac09b1` / plat pulse `3f34bf653`). `npm run ci:gates` 224/224. No hosted migration. Loop stays **ARMED**. Node **done** (rejected: does not reproduce). Finding `8a95d715` status rejected. No public-ux ledger insert (open window `2a5054ac`).

**Done**
- Fleet finding [p0]: Southern Crossing index tile 1 active vs place hero/map 23 homes / 23 ACTIVE vs FAQ 3. FIRST STEP reproduce: it does not. Class already shipped (`getNeighborhoodPublicInventory`).
- Production 2026-08-16T23:22Z: `/neighborhoods` tile **3 Active $920,000**, `/cities/bend` row **3 Active $920,000**, place hero **3 homes for sale in Southern Crossing** + **Median list $920,000**, hydrated map badge **3**, FAQ / Dataset Active Listings **3**, `#homes` list **3** unique inventory hrefs (Roosevelt $989,000 · Bryanwood $920,000 · Tall Tree $675,000). 0 of index 1 / place 23. Probe `scripts/probe-southern-crossing-count-prod.mjs` exit 0.
- Second query shape (`listing_boundary_xref_mv` geo_slug=`bend-southern-crossing`): SFR + PUBLIC_ACTIVE = 3. All-types Active in the polygon = 23 (3 SFR + 15 townhouse + 3 condo + 2 duplex). Pulse `active_count` = 3. That is the stale 23, not a live split.
- Screenshots `/opt/cursor/artifacts/sc_count_{place,index_tile,place_homes}_{1280,390}.png`.
- R-024 coverage note only (still LOCKED). Do not mark G27 done.

**Also on origin/main (other sessions, do not steal / do not undo):** Awbrey 52/63/62 reject `1faedbd9b` (node `5688e089` **done**, does not reproduce). Ridge plat parent-pulse `3f34bf653` / stamp `d688cd4ef` (node `a8726dc3` **done**). Ridge plat SFR count `846193510` / stamp `7bf89b6ab` (node `390ea7a4` **done**). Awbrey index-vs-place reject `6f334748e` / stamp `09de2a587` (node `2d90a914` **done**, does not reproduce). listing down-payment `aba8c2222` / handoff `31e81df7b` (node `ee37b3a4` **done**). place list medians `eff056fb2`. page-grade process KILLED `9afe74a6a` (do not run `/page-grade`). NWX v8e logo-in-frame killed. Tetherow one-annual HOA. MOS withhold. Counted-set list. G33 `/admin/loop`. G32 seeded. G6 toggle. G6 accept stays **blocked**.
