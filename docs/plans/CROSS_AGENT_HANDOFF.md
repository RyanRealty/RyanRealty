...
- **Blocker, and not only for story films:** the xAI team hit its prepaid credit / spending limit at the end of production (`403 permission-denied`, `team_blocked: true` on key "RyanRealtyApp"). The same team serves CRM reply intent (`lib/crm/reply-intent.ts`), the broker SMS agent (`lib/agent/runtime.ts`), the CMA pipeline and the marketing inbox parser; if production uses this key they fail until the balance is topped up. Matt said the piece billed ~$30 (rate card explains ~$8; the rest is judge reasoning tokens and/or resolution-tiered video).
