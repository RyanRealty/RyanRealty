...
- **Blocker for the next piece:** the xAI team hit its credit / monthly spending limit at the end of production (`403 permission-denied` on `/images/edits` and `/v1/models`, so `ci:grok-models` fails on the account, not the code). Matt must raise it before any further generation.
