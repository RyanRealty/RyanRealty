...
| Field | Value |
|--------|--------|
| **Surface** | **Claude Code (Opus 4.8, 2026-05-29)** — Ryan Realty website rebuild · Wave 3. SHIPPED this session: the **AI-agent citability (GEO) layer** — machine-readable JSON-LD on every key surface so Claude/ChatGPT/Perplexity/Google AI Overviews surface + cite Ryan Realty. Next: neighborhood-page market Dataset/FAQ (its Wave 3 rebuild) + remaining Wave 3 routes. |
| **`main` @ commit** | `e12e4bb` — pushed to origin/main, `Approved-by: matt`. 4 commits this session: AI-citability layer · G34 gate · robots.txt crawler allow-list · homepage+city parity contracts. |
...
| **NEXT (fresh session, queue)** | 1. **Neighborhood page** `/cities/[slug]/[neighborhoodSlug]` — still legacy 516-line page (hand-coded Place+breadcrumb passes G34's presence check). Add market `Dataset` + `FAQBlock` via `buildMarketFaq`, migrate hand-coded JSON-LD to `MetadataBlock`, AND do its Wave 3 structural rebuild together. ⚠️ VERIFY `getNeighborhoodBySlug.medianPrice` is a LIST price (not sale) before labeling it "median list price" in the FAQ — the helper hardcodes that label. 2. Remaining Wave 3 routes: `/zip/[zip]`, `/search`, `/sell`, `/about`, `/team`. 3. Optional: `llms.txt` dynamic-stats enrich (low value — only Claude+Perplexity consume it). Pattern to reuse: `buildMarketFaq(name, pulseOrFallback)` → `<MetadataBlock schemas=[breadcrumb, place, dataset]>` + `<FAQBlock items={faqs}>`. Approach: verified data only (trace to `market_pulse_live`), gate every rule, dev-server curl to confirm JSON-LD in SSR before showing Matt. |
