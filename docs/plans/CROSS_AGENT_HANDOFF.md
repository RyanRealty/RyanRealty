...
- **Done:** Mini `f55e24ddc` score 47 tells in progress: official compact Card (no lead stretch), catalog data-window hover notes, official Empty + empty/feed-miss demoStates, H1 = `buildOregonCityTitle`. `--ship` stays exit 1 until Mini writes `demoMatch: true`.
- **Next:** Recapture fold + figures-open + listings + empty + feed-miss. Leave SITE-105 `in_progress`.
