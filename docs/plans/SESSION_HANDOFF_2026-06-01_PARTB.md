...
5. **Design-system migration.** ✅ **Search cards DONE 2026-06-02:** `components/search/SearchResults.tsx` now renders the canonical `components/site/ListingCard` (was a bespoke inline card) — consistent site-wide look + now shows sqft. Verified the field flows: `ListingTileRow` gained `TotalLivingAreaSqFt?` (already populated at runtime by getListings/getViewportListings mappers + the search_listings_advanced RPC), hover-sync preserved via a `data-listing-key` wrapper div, tsc + ci:gates green. (`/listings` card migration is moot — route deleted in item 4.) **STILL OPEN — Tetherow LP:** `app/lp/tetherow/page.tsx` (1709 lines) has 16 hex literals + 2 retired fonts (Playfair/Inter), 0 arbitrary brackets. The design-token gate scans the WHOLE file, so this is a full color-mapping pass (each hex → navy `#102742`/cream `#faf8f4`/`rgba(16,39,66,*)` per DS v2; fonts → `var(--font-amboqia)`/`var(--font-geist-sans)`) — needs visual judgment per color, a focused job.
