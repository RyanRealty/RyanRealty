...
5. **Design-system migration.** ✅ **Search cards DONE 2026-06-02:** `components/search/SearchResults.tsx` now renders the canonical `components/site/ListingCard` (was a bespoke inline card) — consistent site-wide look + now shows sqft. Verified the field flows: `ListingTileRow` gained `TotalLivingAreaSqFt?` (already populated at runtime by getListings/getViewportListings mappers + the search_listings_advanced RPC), hover-sync preserved via a `data-listing-key` wrapper div, tsc + ci:gates green. (`/listings` card migration is moot — route deleted in item 4.) **STILL OPEN — Tetherow LP** (`app/lp/tetherow/page.tsx`, 1709 lines, 0 arbitrary brackets). Gate only scans it when next edited (not failing now). It's NOT mechanical — the hex are semantic state colors on a client-facing LP, so Draft-First applies (show Matt the rendered result before push). Exact map (8 distinct hex + 2 fonts):
