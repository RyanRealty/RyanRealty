...
|---|---|---|---|
| Homes | `/homes-for-sale` (one map+list; opens split on desktop and on the list on phones with the map one tap away, framed on all of Central Oregon, newest listed first (on-market date, not the MLS edit time): Matt 2026-09-23, `lib/search/search-opening.ts`, `lib/search/search-sort-order.ts`) | Open houses, price drops, our listings, sold as a **filter**, invest | Luxury as a filter of search: the chrome links the indexable preset `/homes-for-sale/bend/luxury`, and `/luxury-homes-bend` 301s there. Compare as a mode of search. Video tours stay as `/videos` until they live on the listing. Listing alerts: on-page Sheet, not `/lp/…` |
...
**`/about` (Matt 2026-09-21 lock `about-matt-2026-09-21`, SITE-163; extended 2026-09-23).** Above the proof, unchanged: faces at display scale with the one boutique sentence (AboutFirm), firm Google reviews (V3Proof), firm closings (FirmClosings), Call/Text/Email/Schedule with live hours (AboutReach + V3OnDuty), the office and firm OREA (AboutOffice), the one-line inquiry (AboutInquiry). **Matt 2026-09-23, the About-page AEO playbook, added BELOW the proof** (photos, reviews and closings stay on top), in this order: What Ryan Realty does (each service an H3, one door each; never commercial or property management), What makes Ryan Realty different (our own checkable facts with their figures), Who Ryan Realty works with (clients by need, never a protected class), Where we work (Atlas), How Ryan Realty works (channels, live office hours, one broker through closing, and the reply-time promise **"same business day"**, Matt's own commitment, stated plainly), The team behind Ryan Realty (origin, the licensed-broker count from the live roster, a door to each broker's `/team` page, the firm's profiles), Key facts about Ryan Realty (one crawlable `<dl>`), Frequently asked questions (H3 questions; FAQPage is the same array). **No competitor is named** anywhere on the page and nothing is a comparison (NAR Code Article 15, Oregon advertising rules). **No broker roster fold**, no three equal broker cards, no KPI grid. Key facts carry no Notable clients (client privacy), no Competitors, and no Contract terms (none confirmed). Held by `scripts/lib/about-lock.mjs` beat 9 and `design_system/ryan-realty/ui_kits/about/parity.json`. **Matt 2026-09-24:** Ryan Realty was founded in **2014** (Ryan Realty LLC; the Bend office opened June 2023), so the site-wide Organization `foundingDate` is 2014; and the differentiators show recorded closings as a **count only**, with no drawn strip of the closings.
