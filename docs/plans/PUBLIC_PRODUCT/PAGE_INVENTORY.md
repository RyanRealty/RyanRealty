...
| # | Section | Notes |
|---|---|---|
| 1 | Map + list | Bound both ways. Atlas language if we can; if tiles stay, restyle chrome to v3 (no Google Draw/Roboto). The bare URL opens **split** on desktop and on the **list** on phones (map one tap away, not mounted until then), framed on all of Central Oregon (`CENTRAL_OREGON_BOUNDS`, derived from the served places' recorded boundaries), newest listed first, by the on-market date (Matt 2026-09-23; it replaced UXLIVE-4's list default of 2026-09-22). The regional frame counts the service-area set, not the pixel viewport, and serves its first 48 cards in the HTML with the pins read after hydration. `?view=list|split|map` still work, canonical to the bare URL. Every regional door (chrome, homepage, `/cities`) links the clean `/homes-for-sale`. |
