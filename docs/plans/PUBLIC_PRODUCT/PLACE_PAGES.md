...
**The homes under the map are dials (Matt 2026-09-23).** `PlaceSubdivisionHomes layout="dial"`: each buyer group in the current map selection is one `V3ListingDial`, the same primitive the plat pages use, so a reader looking at River West's multifamily homes sees one of them large and flips through the rest on the dial to its left, with "01 / 02" at the dial's head. Choosing a subdivision on the map still decides what is in the dials (each re-opens on the first home of the new selection). The neighborhood band is 90rem, so the dial lays its card on its side there: the dial, the photograph against it, the copy in a column on the right (the product-gallery order). The dial stands on the LEFT (2026-09-23) because the site's Jax button sits at the right edge, mid-screen, and covered a right-hand dial wherever the band runs to the viewport's edge. City and community pages keep the carousel (`PlaceSubdivisionHomes` default).
...
**One dial per property type (Matt 2026-09-23: "we need to see carousels of all available property types if there are any; we haven't been doing commercial and multi family"; then, the same day, "an alternative to a carousel ... a primary card ... a smaller dial with thumbnails of the other photos ... some kind of indicator of how many total cards are in the dial").** The inventory is `V3PlaceInventory layout="dial"`: for each type with active stock, in the order single-family, multi-family, townhomes and condos, land, commercial, one `V3ListingDial`. One listing shows large (its lead photograph, then the rail card's copy: ask, facts, address, a door to the listing), the rest of that type are thumbnails on a dial to its left with no scrollbar, and "03 / 12" with a filling rule heads the dial. A thumbnail the dial's edge cuts through shows its photograph but not its caption, so no price is ever shown cut off. The dial turns by thumbnail, previous/next, arrow keys, Home/End, and a swipe on the photograph on a phone, where the dial becomes a strip under the card. A type with one listing is the card alone, with no dial and no count. An empty type is omitted. Every listing is still an `<a href>` in the served HTML (the cards not showing carry `hidden`), and every listing the ledger would list is in the dial (no photo or no price does not drop it; "Price not published" is the unpriced ask). A commercial lease (MLS PropertyType G) is not for sale, so it is never in a dial or its count. `layout="rails"` (one card carousel per type) and the default ledger rows stay available.

**The card plays its reel after the photograph (SITE-194, Matt 2026-09-24: "as we toggle through or navigate those cards, have the primary photo come in first; after a second or two, play the video associated with it if there is one").** On every `V3ListingDial`, the photograph paints first. Once the card has been on screen in a visible tab for a second and a half, the dial asks `GET /api/listings/[listingKey]/card-video` (`getListingCardVideo`: the same walkthrough reel the listing page leads with, through the cached videos read, never a 3D tour, and only a host that can play silently with no chrome: a direct file, Vimeo, YouTube, Cloudflare Stream) and fades the reel in over the photograph, muted and looping, inside the photograph's own box so nothing moves. A turn unmounts it at once; one reel plays on a page at a time; a reader who asked for reduced motion or Save-Data gets a "Play video" control instead of autoplay. No page adds a per-card video read to its server render. A page may pass `hasVideo: false` on a row it knows has no reel to skip the request. **The rail's side:** `railPosition` puts the dial left of the card (the default, for the Jax reason above), right of it, or under it as a strip on every screen; the three are compared on the internal preview `/dev/public-v3/dial-rail`.
