...
|---|---|---|
| Calculator defaults (rate, tax %, insurance %) | `public.app_config` keys `mortgage_rate` / `default_tax_rate_pct` / `insurance_rate_pct`, read through the DAL's `getCalculatorDefaults` (6h `unstable_cache`, tag `app_config`), with hardcoded floor fallbacks and a fraction-vs-percent sanity clamp | `lib/data/config.ts:37-77,79-83`; table documented at `docs/DATABASE_FOR_AI_AGENTS.md:258` |
...
5. **Defaults trace to app_config (§0)**:
   `select key, value from app_config where key in ('mortgage_rate','default_tax_rate_pct','insurance_rate_pct');`
   → compare against the rate the calculator renders by default; they must agree after the
   percent normalization in `lib/data/config.ts:64-76`. If the row is absent, the rendered
   default must equal the documented fallback (7 / 0.75 / 0.30, `:31-35`).
