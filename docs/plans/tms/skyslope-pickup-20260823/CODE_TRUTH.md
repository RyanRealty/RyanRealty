...
| `/admin/signing` | EXISTS; on Closings rail **this branch** | Brokers: `transactions.view`. `esign.send` unparked 2026-08-23. |
| `/admin/closings` | EXISTS | `tc_deals` board. **New file** (Seller listing / Buyer sale). Active-listing lens with MLS + expiration. Search. Incomplete-checklist lane. Parties from cycle names when `tc_deal_people` is empty. Brokers see their files; Matt sees all. Title/escrow/TC backfill from SkySlope `raw` on first deal open. |
...
| FUB/CRM → deal log | PARTIAL (this branch) | FUB dead. CRM Gmail person timeline now also files onto the matching Vault deal. Unique cycle-name CRM links (button + unique-only matcher) so auto-file can use `tc_deal_people` instead of address fallback. |
...
Shipped on this branch: live Forms recipient roles, Closings rail Signing/Sign-off, new-deal checklist seed, Form-library envelope compose, Gmail/Twilio auto-file onto deal log + matching checklist, `action_required` column + composer picker, Twilio MMS PDF fetch, 48h envelope reminders, listing expiration on Closings, Forms Use on deal, Closings **New file**, DigiSign Full Name + Time, envelope Edit message.

Still missing (not product clones):

1. 14-column Manage Listings grid (`/admin/listings` stays MLS). Kebab actions live on the deal.
2. Google Calendar **write** until Workspace DWD includes `calendar` (CRM calendar already has the dates).
