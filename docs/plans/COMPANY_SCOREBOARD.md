...
| Loop sentinel | **SELF-RUNNING** as of 2026-08-15: `/api/cron/loop-sentinel` (every 4h) launches a cloud loop iteration when the graph is dormant with open work. Guards: kill switch (`LOOP_SENTINEL=off`), fresh-activity standdown, 3h cooldown. Nobody says "Run the loop" anymore — saying it still works. | R-206 VERIFIED |
