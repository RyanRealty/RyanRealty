...
| Loop sentinel | **DISARMED** (R-211) until Matt says "arm the loop". Machinery: one-node-per-session + zero-gap handoff + orphan auto-release + live status at `/admin/loop`. Heartbeat every 10 min; kill switch / standdown / 12-per-day cap always on. | R-206 VERIFIED · R-211 LOCKED |
