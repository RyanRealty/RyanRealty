...
| Loop sentinel | **SELF-RUNNING** as of 2026-08-15: `/api/cron/loop-sentinel` heartbeats every 10 min (token-free code) and relaunches the moment the previous iteration ends — state-based busy check, max dormancy ≈ 10 min, never a blind timer. Guards: kill switch (`LOOP_SENTINEL=off`), fresh-activity standdown, 15-min boot guard, 12/day cost cap. Nobody says "Run the loop" anymore — saying it still works. | R-206 VERIFIED |
