...
| F5 | SPEC §1.4 | Pulse 488 / 3.54 seller vs legacy RPC 794 / 4.5 balanced | **confirmed** | Pulse 488 / 3.54. Legacy RPC still `LIKE '%active%'` (admits AUC). Cache `end_of_period_inventory` **452**. “Three defs inside the cache layer” is **overstated**: 488 is pulse, 794 is legacy RPC, 452 is cache eopi. | major | One `active_count`. |
