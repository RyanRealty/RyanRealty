...
**Deliverability:**
- **9.1 Email auth (HIGHEST leverage).** Resend domain `mail.ryan-realty.com` (Matt confirms **DNS now set up** — verify SPF/DKIM live + DMARC ramping). *Remaining code-side:* **remove the `onboarding@resend.dev` sandbox fallback** in `lib/resend.ts` for prod (fail loud if `RESEND_FROM` unset), and confirm the root-domain DMARC for Gmail-as-broker is enforcing. *FLAG (Matt — Cloudflare): confirm DKIM CNAME + SPF `include:amazonses.com` + return-path + `_dmarc` ramp `p=none→quarantine→reject` with `rua`.*
- **9.2 Bounce/complaint → suppression — ✅ DONE (commit `4b55bf6a`).** Proper Svix HMAC verification + hard-bounce/complaint → `addSuppression({channel:'email'})` across all sibling rows. *Confirm a real Resend test event in the dashboard once deployed.*
...
**9.E — Email inbox-placement & anti-spam (STANDING REQUIREMENT — every automated/bulk email the CRM sends must satisfy this; enforce with a gate).** Matt directive: *our emails must be optimized, not go undelivered, and not look like spam.* Concretely:
- **9.E.1 Authenticate (covered by 9.1):** SPF + DKIM + DMARC aligned on a consistent verified `From` + a real (monitored) `Reply-To` — the #1 inbox factor.
- **9.E.2 Multipart, not HTML-only.** Every send includes a **plain-text alternative** (HTML-only is a strong spam signal) and a balanced **text-to-image ratio** (never image-only); all images hosted + `alt`-texted, inline CSS, no `<script>`, no remote-loaded fonts.
- **9.E.3 CAN-SPAM body requirements:** a visible unsubscribe link in the body (not just the header), the brokerage **physical mailing address** in the footer, an honest non-deceptive subject + a real `preheader`, and the broker/brokerage clearly identified.
- **9.E.4 Content spam-score linter (the gate — `ci:email-quality`):** scan every template/render for spam triggers — ALL-CAPS subject, excessive `!`/`$$$`, spam-word list (free/guaranteed/act-now/winner…), URL shorteners, link domains that don't match `ryan-realty.com`, missing plain-text part, missing unsubscribe, missing physical address, broken HTML. Fail the build on a high score. Run a real **seed/spam-score test** (mail-tester/GlockApps) on each new template before it ships.
- **9.E.5 Reputation & list hygiene:** warm up volume, **segment the cold 15K book from warm transactional** so a complaint spike can't poison CMA delivery; a **sunset policy** (stop emailing chronic non-openers); send from a consistent domain/identity; monitor **Google Postmaster Tools** + the 9.5 health board (bounce <2%, complaint <0.1%, spam-rate). Spam-trap avoidance: never email purchased/scraped lists; the suppression chokepoint (9.2) + engagement targeting keep the list clean.
- **9.E.6 Engagement signals:** meaningful subject + preheader A/B where useful, send-time within the recipient's window, personalization from `crm_people` (name/area), and a clear single primary CTA — Meta/Gmail reputation rewards opens/replies, punishes deletes-without-open.
- **9.E.7 Per-send preflight:** a shared `prepareDeliverableEmail()` helper that EVERY automated path (CMA, sequence, alert, newsletter) routes through, applying 9.E.1–9.E.4 (multipart + List-Unsubscribe + physical address + spam-lint) so no path can ship a non-optimized email. *This is the enforcement point — make it impossible to send a spammy email, not merely discouraged.*
