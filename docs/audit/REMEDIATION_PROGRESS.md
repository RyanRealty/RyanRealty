...
| 3 | 3.x | Governance + tests + env | 🔶 orphan gates 24→7 ✅ · doc drift ✅ (orphan-count 24→7 synced in CLAUDE.md + MECHANICAL_GATES.md) · tests +24 files / 757 ✅ · **3.2 critical paths — 4/5 fully covered: sync-cursor ✅ · market-formula ✅ · auth-guards ✅ (pure isValidCronAuth extracted from the CRON_SECRET bypass surface + open-redirect safeRedirect) · FUB lead-create ✅ (sendEvent fetch-mocked: fail-closed-no-key, /v1/events payload contract + Basic auth, conditional-field omission, 204-ok, parsed-error, network-error; crm/mirror stubbed; + isPlaceholderFubEmail); DAL-fn partial (service-area/getListingVideos)** · shared gate-lib: walkFiles + 4 gates migrated; rest NOT drop-in, left as-is ✅ · **3.3 dual-name env collapse: `getMetaPageToken()` (META_PAGE_ACCESS_TOKEN ?? META_PAGE_TOKEN) + `ci:meta-token` gate (baseline 8, dogfoods walkFiles), meta-health page migrated** 🔶; build-failing-Zod-over-159-keys: deferred (needs `next build`) |
