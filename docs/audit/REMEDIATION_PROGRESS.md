...
| 3 | 3.x | Governance + tests + env | 🔶 orphan gates 24→7 ✅ · doc drift ✅ (orphan-count 24→7 synced in CLAUDE.md + MECHANICAL_GATES.md) · tests +22 files / 745 (+ inventory-filters, search-filters dedup, walkFiles, report-year-compare YoY, market-faq, meta-env, json-ld offer-honesty, year-series, cma-map-url) ✅ · shared gate-lib: walkFiles + 4 gates migrated; rest NOT drop-in, left as-is ✅ · **3.3 dual-name env collapse: `getMetaPageToken()` (META_PAGE_ACCESS_TOKEN ?? META_PAGE_TOKEN) + `ci:meta-token` gate (baseline 8, dogfoods walkFiles), meta-health page migrated** 🔶; build-failing-Zod-over-159-keys: deferred (needs `next build`) |
