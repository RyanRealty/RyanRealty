...
- **needs a running app + carries deploy-brick risk:** Zod Stage 2 throw — the build half is now verifiable, but the runtime-boot throw (instrumentation.ts) can 500 the site if a required var is absent at runtime, and confirming which of ~159 vars are truly required needs the live Vercel env. Marginal value (the vars it would guard are obviously-required), real downside. Flagged for an explicit Matt sign-off.
