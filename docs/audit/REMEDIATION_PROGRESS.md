...
After this pass: meta-gate **94 ci:\* gates wired, 0 orphaned**, orphan gate-FILE backlog **2** (`bundle-budget`, `tool-discipline`). `ci:process-canon` is **GREEN** on `main` (the workflow re-traced it: all 33 plan docs registered incl. `PAGE_REVIEW_REDESIGN_RUNBOOK.md`; the earlier "RED" notes were stale).

**Deferred — needs a build env, render-verify, live-path care, creds, or a Matt decision (NOT blind-pushed, per policy):**
- **next build:** Zod Stage 2 throw (instrumentation.ts; over-marking a runtime var bricks deploy) · `server-only` markers on lib/data (RSC client-graph poisoning is build-only-detectable; `service.ts` has 616 importers) · the actual 2.2 file splits (`'use server'` manifest + barrel poisoning) · `bundle-budget` re-baseline (reads `.next/static`).
...
- **owner decision:** `tool-discipline` orphan gate — Layer 1 (skill-existence) is DEAD (`tool-mastery`/`viral-playbook` SKILL.md both gone, always-fails); Layer 2 is a sound inline-ElevenLabs/Replicate-call ratchet worth salvaging. Salvage-vs-delete enables/forfeits enforcement on the FROZEN marketing/video producer layer (G45) — flagged for Matt, not acted on.
