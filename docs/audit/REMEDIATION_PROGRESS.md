...
### Deferred-items DEEP pass — 7 more shipped (2026-06-22, exhaustive classify→safe-subset)

A 12-agent classification (safely-completable-now vs build/creds/owner-blocked) drove a second pass. Seven more shipped, each additive/reversible, tsc + named-gate + vitest verified (+ a negative-test fixture per new gate). Tests 780 → 821.

- **DAL boundary Part C** — extend the cached-write classifier to resolve a NAMED fetch-fn reference (`unstable_cache(fetchFn,…)`) within the same file and scan its body; UNRESOLVED refs (cross-file, a function PARAMETER, expression-bodied arrow) never flag. Clean on the real tree; 10-case negative test. (commit `54cb059a`)
- **DAL 2.1 ratchet** (`ci:dal-actions-reads`) — the safe slice of narrowing the `app/actions` read exemption: a read-vs-write classifier that freezes the raw `.from().select` read count at **266** (49 files); writes unaffected, no code moved. The actual read relocation stays the deferred remainder. (commit `e07fcd6e`)
- **2.2 god-file freeze** (`ci:file-size-budget`) — no-code-touch ratchet: every app/+lib file ≥600 LOC (50 files) frozen at current size + the `lib/data/index.ts` barrel frozen at 132 exports; no new god-file. The actual splits stay deferred (next build). (commit `ba29b7b7`)
- **Zod Stage-2 prerequisite** — export the schema + the three lists and add `lib/env.test.ts`: the partition coverage guard (every `EnvSchema` key classified into exactly one list, no overlap/orphan) + validate behavior pins. The build-failing throw stays deferred. (commit `64ab4d6b`)
- **FUB merge guardrail** — add canonical `fubAuthHeaderTrimmed()` (byte-identical to the 3 clients' trimmed header) + a characterization test locking the trim discrepancy vs untrimmed `fubAuthHeader`. No call site touched; the 3-client merge stays deferred. (commit `45aa7423`)
- **Date 1.4 byte-identical subset** — migrate the only two provably byte-identical inline date formatters (`components/ListingTile.tsx`, `app/admin/console/leads/page.tsx`) to `formatDate`, locked by an equivalence test; date gate **86 → 83**. (commit `8547a714`)
- **legacy-redirects static redesign + wire** — make the gate static-by-default (structural + committed-fixture coverage, 152/152) so it joins `ci:gates`; live sitemap / HEAD smoke kept as opt-in deploy flags. **Orphan backlog 3 → 2.** (commit `9b3b495a`)
- **tool-discipline (G36): retire dead Layer 1, salvage + wire Layer 2** — the gate ran nowhere and always-failed because Layer 1 asserted two now-deleted skill files exist. Retired Layer 1; kept + activated Layer 2, the sound inline-ElevenLabs/Replicate-call ratchet (a live CLAUDE.md rule), grandfathering the current 27. Layer 2 was already clean (0 NEW) so wiring it doesn't redden `ci:gates`; it does NOT touch the G45 producer freeze. Refactored for a 6-case negative test. **Orphan backlog 2 → 1.** (commit `4ed86fd7`)

After this pass: meta-gate **95 ci:\* gates wired, 0 orphaned**, orphan gate-FILE backlog **1**. `ci:process-canon` is **GREEN** on `main`. Tests 780 → 827.

### Build-enabled pass — `next build` runs on macOS after all (2026-06-22)

The "dev/build is Windows-only" assumption was wrong: `next build` runs clean on this macOS session (compile ~20s, full static generation of 302 pages, exit 0). That unblocked the build-gated items, each verified by an actual production build before push:

- **`bundle-budget` (G10) fixed + wired — orphan backlog 1 → 0.** The audit's "runs nowhere" was half the story: CI calls `npm run ci:bundle-budget` post-build, but that npm script never existed, so the CI step failed on a missing script. Added the script, re-baselined against the real build (3.62 → 6.13 MB / 179 chunks), raised the per-chunk ceiling 600 → 850 KB (the Next 16 Turbopack framework chunk is ~767 KB, normal), and removed it from the orphan baseline. Stays OUT of the static `ci:gates` chain (needs `.next/static`); runs in CI post-build + manually. **The audit's 28-orphan gate finding is now fully closed (28 → 0).** (commit `e667ecf6`)
- **`server-only` markers on 8 server modules (p2.2) — build-verified.** Each marker poisons the build if a client component imports the module as a value; a real build is the only way to prove it's safe. Verified via full production builds: `lib/supabase/service.ts` (security-critical — service-role key; 616 importers), `lib/data/index.ts` (the whole DAL public surface — 91 importers), + `lib/crm/{twilio,suppressions,broker-alerts}.ts`, `lib/email-tracking.ts`, `lib/data/sync/syncWrites.ts`, `lib/data/admin/syncCounts.ts`. Added a vitest `server-only`/`client-only` stub alias (those Next-bundler packages don't resolve under plain Node). No client component can now pull the server DAL — or the service-role client — into a client bundle. (commits `daef0e5c`, `011cc9f8`)

- **Zod env Stage 2 (runtime-boot fail-fast) — DONE, build-safe-verified.** `assertRuntimeEnv()` throws at server boot (instrumentation.ts register(), nodejs runtime) if a required var is missing, naming it. Gated to SKIP the `next build` phase (NEXT_PHASE), so a missing runtime-only var fails the running server loudly but can never brick a deploy's build. Proven: a full build with `SPARK_API_KEY` emptied still compiles + static-generates 302 pages (exit 0, zero env throws in the log). No-op in healthy prod. (commit `28b1682c`) The broader ~159-key build-FAILING throw stays out (needs a required-vs-optional sign-off against the real Vercel build env).

**STILL DEFERRED — genuinely gated (NOT blind-pushed, per policy):**
- **separate large refactor (build-verifiable, big):** the actual 2.2 file splits (syncWrites.ts ~70 fns, app/actions/listings.ts 2,670 lines, the DAL barrel) — the ratchet freeze is shipped; the splits are their own initiative · DAL `app/actions` read relocation into lib/data (ratchet shipped; moves remain) · `experience/` per-symbol consumption trace · additional `server-only` markers on the remaining lib/data leaves (cheap now — add, build, keep if green — but diminishing returns after the barrel + service.ts).
- **live-path + build:** FUB 3-client merge (mutates live auth bytes) — guardrail in place.
- **render-verify:** currency baseline shrink · the remaining 83 inline date sites.

**Legacy note (superseded):** earlier entries below calling `ci:process-canon` RED and listing 7 orphan gates are pre-this-pass; the current truth is GREEN process-canon + a 2-file orphan backlog.
...
- **`ci:process-canon` (G44) — RESOLVED / GREEN (verified 2026-06-22).** This note (originally: G44 RED because `PAGE_REVIEW_REDESIGN_RUNBOOK.md` was unregistered) is STALE. The doc was since registered at `docs/DEVELOPMENT_PROCESS.md:97`; `node scripts/check-process-canon.mjs` prints "in sync" and exits 0, all 33 plan docs registered, the 3 entry-point pointers match canon v1.1.0 — confirmed against committed HEAD. The static chain is not blocked by G44.
