...
| 3 | 3.x | Governance + tests + env | 🔶 orphan gates 24→7 ✅ · doc drift ✅ (orphan-count 24→7 synced in CLAUDE.md + MECHANICAL_GATES.md) · tests +23 files / 749 ✅ · **3.2 critical paths: sync-cursor ✅ + market-formula ✅ + auth-guards ✅ (extracted isValidCronAuth from isAuthorizedAdminOrCron — the CRON_SECRET bearer/bypass surface — into a pure testable unit + open-redirect safeRedirect); DAL-fn partial (service-area/getListingVideos); lead-create = pure subsets done (phone/fub-env/email-body), full path needs FUB-mock (deferred)** · shared gate-lib: walkFiles + 4 gates migrated; rest NOT drop-in, left as-is ✅ · **3.3 dual-name env collapse: `getMetaPageToken()` (META_PAGE_ACCESS_TOKEN ?? META_PAGE_TOKEN) + `ci:meta-token` gate (baseline 8, dogfoods walkFiles), meta-health page migrated** 🔶; build-failing-Zod-over-159-keys: deferred (needs `next build`) |
