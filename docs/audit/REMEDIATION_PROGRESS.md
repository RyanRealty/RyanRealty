...
After this pass: meta-gate **95 ci:\* gates wired, 0 orphaned**, orphan gate-FILE backlog **1** (`bundle-budget` only — it reads `.next/static`, genuinely needs a real `next build`). `ci:process-canon` is **GREEN** on `main` (the workflow re-traced it: all 33 plan docs registered incl. `PAGE_REVIEW_REDESIGN_RUNBOOK.md`; the earlier "RED" notes were stale). Tests 780 → 827.

**SAFELY-COMPLETABLE SET EXHAUSTED.** Everything below genuinely needs a build env, creds, render-verify, or a larger refactor — none is blind-pushable from this session. NOT a backlog of "easy wins left undone"; each is gated by something this macOS session cannot do.

**Deferred — needs a build env, render-verify, live-path care, creds, or a larger refactor (NOT blind-pushed, per policy):**
- **next build:** Zod Stage 2 throw (instrumentation.ts; over-marking a runtime var bricks deploy) · `server-only` markers on lib/data (RSC client-graph poisoning is build-only-detectable; `service.ts` has 616 importers) · the actual 2.2 file splits (`'use server'` manifest + barrel poisoning) · `bundle-budget` re-baseline + wire (reads `.next/static`).
- **live-path + next build:** FUB 3-client merge (mutates live lead-create/attribution auth bytes) — guardrail now in place.
- **render-verify:** currency baseline shrink (every baseline site formats exact prices; `formatPrice` rounds to $1k → output-changing) · the remaining 83 inline date sites (timezone/format-shape/locale divergence).
- **large refactor:** DAL `app/actions` read relocation into lib/data (ratchet shipped; the moves remain) · `experience/` per-symbol consumption trace.
