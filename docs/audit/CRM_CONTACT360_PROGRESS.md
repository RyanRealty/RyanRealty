...
**To run the live push (the ONE step the agent cannot do):** set `META_AUDIENCE_PUSH_ENABLED=true` in Vercel production env, redeploy, then the agent triggers `/api/cron/meta-audience-sync` (Bearer CRON_SECRET) — the push runs server-side. CONSENT-COMPLETENESS FOLLOW-UP: if the daily cron stays live (auto-refresh), apply the held removal-queue migration so opt-outs propagate to Meta removal (additive uploads alone never remove an opted-out person).
