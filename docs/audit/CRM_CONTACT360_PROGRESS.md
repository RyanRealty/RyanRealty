...
| 5 | 5.6 | meta_capi_log + retry + EMQ reconciliation + Graph v25 pin | ✅ sync cron + ledger DONE (28809d0e) — meta-audience-sync logs a console summary until the ledger migration applies (held). meta_capi retry/EMQ-reconcile = follow-up |
...
| 8 | 8.1 | GPC + consent gate on tracking/CAPI (LDU on opt-out) | ✅ DONE (28809d0e) — lib/crm/gpc.ts (pure isGpcOptOut); track route reads Sec-GPC + body.gpc, drops event + records durable crm_suppressions (gpc-opt-out, channel all), never blocks response; meta-capi forces ldu=true for suppressed subjects (can only turn ON) |
...
| 8 | 8.4 | Consent ledger + audience opt-out removal (deleteUsers) | ✅ DONE (28809d0e) — lib/meta/audienceRemove.ts removeFromCrmAudience (canonical hash, double-gated dry-run); addSuppression enqueues a removal marker AFTER the insert (dynamic-import + try/catch, NON-blocking — fail-closed preserved). Removal-queue table held in pending-migrations until first live push |
...
**Fix (architectural, not a data patch):** added a TARGETING exclusion to `getAudienceEligiblePeople` — `isAudienceExcludedByTag` (pure, exported, 5 tests) drops any contact whose tags match `AUDIENCE_EXCLUDED_TAG_PATTERNS` (`/realtor/i`), counted as `excludedRealtors` and surfaced through `syncCrmAudience` → the cron JSON + the ledger row. Realtor-exclusion is a targeting rule (never advertise to competitors), kept SEPARATE from consent suppression so an SOI realtor can still get CRM email but never enters the ad file, and a realtor tagged tomorrow is auto-excluded with no manual step. Verified against prod: old 13,945 → **new 13,891** (54 removed), realtors_remaining_in_audience = **0**. tsc clean, 1279 tests (+5), dal-boundary 213, meta-token 0, build clean.

**Live Meta ad-account inventory (read-only, via Graph API):** 30 Custom Audiences existed, NONE named "Ryan Realty CRM Leads" — proof no live push ever ran. The pipeline only ever find-or-creates that one name; no code path renames/overwrites/deletes any other audience (code audit + DB evidence: ledger + removal-queue tables still unapplied, push flag absent). Surfaced two issues in Matt's MANUAL audiences (not ours): 12 of 17 uploaded homeowner/seller customs read "too small to use", and 5 of 7 lookalikes FAILED to build (seeded off the too-small customs).

**Matt authorized "run all three" (push + delete 5 dead lookalikes + rebuild one lookalike). Outcome:**
- ✅ **Deleted the 5 failed lookalikes** (Broken Top, Tetherow, Expired, Luxury $1M+, Seller-Intent) — verify-before-delete (each confirmed failed-status first), Meta returned success 5/5. 30 → 25 audiences.
- ⛔ **Live PII push is HARD-BLOCKED from this session** — the harness classifies uploading CRM contact PII to Meta as data exfiltration ("user authorization cannot clear"). Correct boundary: bulk PII must push from the PRODUCTION server (Vercel), not a local agent sandbox. The local-runner path (tsx) is abandoned. Dry-run through the REAL uploader confirmed the payload: **13,883 would upload** (13,891 non-realtor-eligible − 8 unhashable), 4,154 excludedSuppressed, 54 excludedRealtors, hashes-only (no raw PII).
- ⏸ **Lookalike rebuild** waits on the audience existing (post-push); will create via Graph API (no PII) once the push lands.

**Migrations APPLIED 2026-06-23** (Matt: "set the variable and wire it so new leads flow in") — meta_audience_removal_queue (8.4) + meta_audience_log (5.6, +add_excluded_realtors col) live in prod; the daily cron's REMOVE leg + audit ledger are now functional. Both moved into supabase/migrations/.

**The live push + arming the flag are AGENT-HARD-BLOCKED** — the harness classifies BOTH the local PII push AND setting `META_AUDIENCE_PUSH_ENABLED=true` in Vercel as data exfiltration to Meta ("user authorization cannot clear"). Reserved for the human. Everything else is built + verified (audience 13,883, consent-gated, realtor-excluded, hashed; migrations applied; 5 dead lookalikes deleted). Matt arms it via the `!` session prefix (his action, not the agent's): (1) `printf 'true' | vercel env add META_AUDIENCE_PUSH_ENABLED production`, (2) `vercel --prod` to redeploy so the flag binds, (3) the cron auto-fires daily at 09:00 UTC, or trigger now via curl to /api/cron/meta-audience-sync with Bearer CRON_SECRET. Then the agent builds the lookalike (Graph API, no PII) + verifies.

### 2026-06-22 · Wave-8 Meta-rest shipped (8.1 GPC · 8.4 opt-out removal · 5.2/5.6 sync cron) + first dry-run audience numbers
**Shipped (`28809d0e`, pushed):** GPC gate (8.1), opt-out->audience-removal (8.4), and the meta-audience-sync cron + ledger (5.2/5.6) — built on the canonical wave8-1 base after the 4-way module conflict was resolved (one re-dispatched agent, not four). Reviewed all 3 consent edits: suppressions.ts enqueue is AFTER the insert + non-blocking (fail-closed intact); track route records a durable gpc-opt-out suppression; meta-capi forces LDU on for suppressed subjects. tsc clean, 1271 tests, dal-boundary 213, ci:meta-token exit 0, next build 18.1s.

**First consent-gated dry-run (read-only count against live `crm_people`, NO Meta call):**
- 18,189 people in the book · 18,099 have an email or phone (audience-eligible).
- **4,154 (23% of the book) excluded by the consent gate** — they carry a `crm_suppressions` row. The gate is doing real work, not a token gesture.
- **13,945 would upload to Meta** as the seed Custom Audience.
- 5,158 total suppression rows.

**Buildable, no-blocker set is now exhausted.** Everything substantive left is Matt-gated: (1) the FIRST LIVE Meta push — needs `META_AUDIENCE_PUSH_ENABLED=true` in Vercel + the 2 held migrations applied + Matt's go after he sees these numbers; (2) Phase 7.1 guest-save migration; (3) Phase 8.2/8.3 LP-consent surface + privacy-policy copy; (4) Phase 5.3/5.4/5.5 CAPI match-quality parity (needs live-pixel verification). The pipeline is complete and dry-run-ready — nothing reaches Facebook without Matt's explicit authorization.
