...
**Live Meta ad-account inventory (read-only, via Graph API):** 30 Custom Audiences exist, NONE named "Ryan Realty CRM Leads" — proof no live push ever ran. The pipeline only ever find-or-creates that one name; no code path renames/overwrites/deletes any other audience (code audit + DB evidence: ledger + removal-queue tables still unapplied, push flag absent). Surfaced two issues in Matt's MANUAL audiences (not ours): 12 of 17 uploaded homeowner/seller customs read "too small to use", and 5 of 7 lookalikes FAILED to build (seeded off the too-small customs). Matt authorized deleting the 5 dead lookalikes + running the live push + rebuilding one lookalike — IN FLIGHT (the 5-audience delete was correctly blocked by the safety classifier pending an explicit per-target confirm; the live push is paused behind this realtor fix).
