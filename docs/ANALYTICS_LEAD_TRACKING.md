...
**Purpose:** tie an individual identity (a name in the in-house CRM, `public.crm_people`) to anonymous, cross-source (Google / Facebook), and repeat-visitor behavior, so you can see exactly what a lead did before and after they identified themselves.
...
|---|---|---|
| **In-app admin dashboards** + **in-house CRM** (`/admin/crm`) | The real **name, email, phone**, full session timeline | "Who is this person and exactly what did they look at?" |
| **Google Analytics 4** | Pseudonymous `fub_person_id` (legacy GA4 dimension name for the CRM person id) + behavior, **no names/emails** (GA4 ToS forbids PII) | Aggregate funnels, source attribution, cohorts, and **ad-targeting audiences** |

The join key between them is the **CRM person id** (`crm_people.id`; GA4 still labels the dimension `fub_person_id`). In GA4 you get a *number*; you turn it into a *name* in `/admin/crm` or `/admin/people/[id]`.
...
- **CRM person id** — the **cross-surface identity join** (`crm_people.id`; some tables and the GA4 dimension still use the legacy column name `fub_person_id`). Resolved from an email (form submit, OAuth) or carried directly in an email-click link (`?_fuid=`). This is the only identifier that ties a person together across WordPress, the Vercel app, Google, Facebook, and email.

### 1.2 Cross-origin caveat (important, and a common misconception)

`localStorage` is **origin-scoped**. The `rr_session_id` minted on `ryan-realty.com` (WordPress) is a **different value** than the one on `ryanrealty.vercel.app`. They do not and cannot match across origins. So `rr_session_id` is **not** a cross-surface key — it only links anonymous-to-known within a single origin. The cross-surface link is always the CRM person id.

### 1.3 The five identify paths

A visitor becomes "known" (gets a CRM person id) via any of these. Each one now also stitches their prior anonymous session:
...
1. Marks the session identified: `identified_at`, `fub_person_id` (legacy column; stores `crm_people.id`), `identified_email`, `identified_via`.
2. Replays prior `visitor_events` into the CRM as **Viewed Property** / **Viewed Page** events (only the high-signal categories: `listing_detail`, `seller_intent`, `buyer_intent`, `area_guide`, `financial_tools`, `search` — blog/home/about noise is skipped).
3. Marks each replayed event `pushed_to_fub_at` (legacy column; idempotent — re-running never double-fires).
4. Writes `events_backfilled_at` / `events_backfilled_count` on the session.
5. Writes **one** chronological summary note on the CRM person: the browsing window, first-touch source/campaign, listings viewed, and pages, in real chronological order.

The first-touch UTMs captured at session start ride along on every backfilled event, so the lead's CRM record reflects the **original source**, not the moment of sign-in.
...
- **`/admin/visitors/live`** — live visitor feed; identified rows link straight to the CRM person.
- **`/admin/visitors/[sessionId]`** — one session's full timeline, with the resolved CRM person + email.
- **`/admin/people/[personId]`** — one person's cross-session view.
- **`/admin/fub-attribution`** — source/medium attribution per lead (route name is legacy).
...
### 3.2 In the in-house CRM

Open the person at `/admin/crm` or `/admin/people/[id]`. After identification you'll see the replayed **Viewed Property / Viewed Page** events plus the single **"Anonymous browsing history backfilled"** note with the true chronology, first-touch source, and listings viewed.

### 3.3 In GA4 (pseudonymous)

GA4 shows the **`fub_person_id`** event-scoped dimension on identified events (legacy name; it is the CRM person id). To name it, open `/admin/people/[id]`. Repeat visitors who later identify light up retroactively (within origin), because backfill attaches the id to their prior session.
...
- Read: each row is one identified person. Copy the id → `/admin/people/[id]` for the name. Sort by Conversions to surface hottest leads.
...
- **GA4 PII rule:** never push name/email into GA4. Only the pseudonymous `fub_person_id` (legacy dimension name for the CRM person id) and hashed `user_id` may flow. The name lives in `crm_people` and the in-app admin.
