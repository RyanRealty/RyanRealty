...
| `yoy_inventory_change_pct` | numeric(8,2) | YoY % change in active inventory at period_end (snapshot at end of window) | Snapshot active count at period_end vs same date prior year | no prior → NULL | -80 to +500 |
...
| `mom_inventory_change_pct` | numeric(8,2) | MoM % change in active inventory snapshot | Snapshot active count at period_end vs same date prior month | no prior → NULL | -50 to +50 |
