...
- [ ] MV refresh crons (`/api/cron/refresh-market-stats`, `/api/cron/sync-delta`, and the pg_cron MV jobs `refresh_listing_tile_mv_30min` + `refresh_dal_mvs_15min`) green for 7 consecutive days post-deploy (Vercel cron logs; `cron.job_run_details` for pg_cron). *(2026-09-24, audits DATA-2 + COMP-3: `/api/cron/refresh-mvs` is deleted. Migrations `20260923014600` and `20260923014500` moved its last two duties onto pg_cron: `refresh_neighborhood_year_pricing_mv` inside `refresh_dal_mvs_15min`, and `refresh_place_membership_changed()` as `refresh_place_membership_15min`. Both stamp `mv_refresh_state`.)* *(refresh-mvs cron was failing in production 2026-05-22 → 2026-05-23 with HTTP 500 — both RPCs hit Postgres default statement_timeout at ~20s. Fixed 2026-05-23 by adding `SET statement_timeout TO '300s'` inside both refresh functions (migration `20260523054800_mv_refresh_statement_timeout.sql`). Verified post-fix with auth'd curl: HTTP 200, listing_tile_mv refreshed in 122s, geo_snapshot_mv in 34s, total 156s. 7-day green clock effectively restarts 2026-05-23. Box closes 2026-05-30 if cron stays green that whole window.)*
