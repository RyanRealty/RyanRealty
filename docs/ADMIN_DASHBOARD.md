...
The dashboard at **/admin** is a single-page operational view: sync health, GA4, Meta + CRM marketing performance, lead/visit intelligence, and operational panels for notifications, content status, site performance, and financial metrics.
...
### Marketing command center (Meta + GA4 + CRM)
- **Unified panel** with one marketing snapshot for the last 30 days:
  ...
  - CRM contact quality (`crm_people` counts and Facebook-sourced contacts)
...
- **My Leads governance**: `/admin/crm` shows the assigned pipeline snapshot, excludes likely realtor contacts from targeting, and shows stage distribution.
- **Outreach automation playbook**: Stage-based text/email/call sequences live in `/admin/crm/sequences`.
...
- **Recent activity**: Last 50 visits with time, path, and whether the visit had a user (identified). Hot leads and engagement scoring use `crm_people` at `/admin/crm`.
...
- **CRM depth**: Contact counts come from `crm_people`. Sequences and tasks are edited at `/admin/crm`.
...
| `components/admin/DashboardMarketingCommandCenterPanel.tsx` | Unified Meta + GA4 + CRM marketing KPI panel |
