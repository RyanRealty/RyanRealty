...
| D93 | In-boundary listings (map pins AND homes-for-sale cards) MUST come from the precomputed `listing_boundary_xref_mv` via the `listings_in_boundary` RPC. Request-time `ST_Within` against the 589K-row `listings` table is BANNED: a wide-bbox polygon (e.g. Tetherow) makes the spatial scan ~10s cold, which blows the anon role's 3s `statement_timeout` and silently returns empty pins + empty cards while the polygon outline still renders. The spatial join runs once at MV refresh (`/api/cron/refresh-mvs`, ≤15 min), so request time is a sub-10ms indexed lookup. Overlapping `geo_type='neighborhood'` polygons (Bend district + resort community) each get their own xref row, which is why resort listings resolve here but NOT via `listing_tile_mv.boundary_neighborhood`. | 2026-05-29 owner-review (community cards empty) | `supabase/migrations/20260529020000_listing_boundary_xref_mv.sql`, `lib/data/geo/getGeoBoundaryMapData.ts`, `app/api/cron/refresh-mvs/route.ts` | enforced | G31 (app layer: page must import `getGeoBoundaryMapData`; DB layer documented in `docs/DATABASE_FOR_AI_AGENTS.md` §4d) | The RPC body reads the MV — verified 5/5 anon calls return 166 Tetherow pins with zero timeouts after the change (was 2/3 timeouts request-time). | 2026-05-29 |
