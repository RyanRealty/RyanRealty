...
| ID | Directive | Source | Surfaces | Status | Gate | Cascade path | Opened |
|---|---|---|---|---|---|---|---|
| D23 | Shadows must use `--shadow-sm` / `--shadow-md` / `--shadow-lg`. No `rgba(0,0,0,...)` shadows except photo overlay scrims. | SKILL.md + MANIFEST.md "Shadows" | All elevated elements | enforced | G28 | `lint-design-tokens.js` DISALLOWED_BLACK_SHADOW regex catches `box-shadow: ... rgba(0,0,0,...)`. | 2026-05-28 |
| D24 | Inline `box-shadow` with literal `rgba(0,0,0,...)` on non-photo surfaces is a violation. | `colors_and_type.css` | Cards, dropdowns | enforced | G28 | Same as D23. | 2026-05-28 |
...
| D72 | No decorative gradients. Only navy protection overlay on hero + Ken Burns scrim. | SKILL.md + EXECUTION_PLAN §9 L3 | All pages | enforced | G27 | `lint-design-tokens.js` DISALLOWED_LINEAR_GRADIENT regex + ALLOWED_GRADIENT_PATTERNS exception for navy scrims and photo bottom-to-transparent. | 2026-05-28 |
...
| D74 | Menu / nav text is too small at 14px. Bump to 16–17px per mockup. | 2026-05-28 chat | `components/site/SiteHeader.tsx`, every mockup nav | deferred (target: Wave 3 same-day fix) | NONE | Update `<SiteHeader>` primitive font-size. Update mockup CSS to match. Single-file change cascades. | 2026-05-28 |
| D75 | Photo gallery nav must include thumbnail strip, dot indicator, swipe on mobile, keyboard arrows, visible photo counter (`3 of 47`). Current Lightbox lacks thumbnails + dots + swipe. | 2026-05-28 chat | `components/site/listing-detail/ListingHero.tsx` Lightbox | deferred (target: Wave 3 same-day fix) | NONE | Lift the Lightbox into a `<PhotoGalleryLightbox>` primitive. | 2026-05-28 |
| D76 | Show ALL listing price/status history events. Current PropertyHistory filters out Photo + empty FieldChange — Matt wants them visible (or an expandable toggle with default-expanded). | 2026-05-28 chat | `components/site/listing-detail/PropertyHistory.tsx` | deferred (target: Wave 3 same-day fix) | NONE | Add `mode: 'all' \| 'meaningful-only'` prop, default `all`. Page passes `all`. | 2026-05-28 |
| D77 | Listing detail must BEAT Zillow Showcase. Missing today: ClimateRiskBlock, VacationRentalPotential, TransparentCMASummary, plus richer schools data (ratings, distance). | 2026-05-28 chat + EXECUTION_PLAN §8 | `app/listing/[listingKey]/page.tsx`, `design_system/ryan-realty/ui_kits/listing-detail/parity.json` | deferred (target: Wave 4) | G6 (via parity.json) | Build three new components, add to `parity.json` as `blocking: true`. G6 fails on listing-detail until they're imported. | 2026-05-28 |
