...
6. **Matt's contract-terms queue** (`getTermsReviewQueue`, `lib/tc/terms/review.ts`): live files only, soonest closing first; the typed-value conflicts and the terms still split after the third read (another buyer's offer on the same property is left out). Picking a reading (`deal_terms_reading_confirmed`) makes it the agreed reading of that form and writes the file's terms again. Linked from `/admin/sign-off` and from the file's "From the contract" panel.
