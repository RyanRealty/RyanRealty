...
8. **Typed text never loses a word** (`lib/tc/text-areas.ts` `fitTextToBox`): the sealer shrinks a value until it fits its box (down to 6 pt) and prints every line; the signer's editor and the composer show whether it fits and refuse text that cannot fit at 6 pt. The signing page draws the same lines at the same size and baselines (`components/tc/pdf-sign/PrintedLines.tsx`, an SVG in the box's own points), each line held to its printed width, so a phone font wider than Helvetica never cuts a letter off at the box edge. The browser measures with `lib/tc/helvetica-metrics.ts` (generated by `scripts/gen-helvetica-metrics.mjs` from pdf-lib's font data, held to it by `text-areas.test.ts`), never `@pdf-lib/standard-fonts` itself (all fourteen fonts plus pako pushed static JS past `ci:bundle-budget`). A character Helvetica cannot encode shows and prints as "?".
...
10. **A couple sharing one email address** (Matt 2026-09-25). Each signer still gets their own email and their own link. When another signer on the envelope uses the same address, the invite's subject starts with the signer's name, the button reads "Review and sign as <name>", and the email says the other signer gets a separate email at the same address; the consent screen says whose link it is; the composer notes the shared address. When the first finishes, the page offers **Continue as <next signer>**: it opens that signer's own emailed link (reused, never re-minted), only while their turn is open and for 30 minutes after the first finished (an old or forwarded link cannot hand it out later); event `envelope_signer_handoff`. The completed copy goes once to the address, greeting everyone who reads it (`sharedAddressCosigners`, `namesInSentence` in `lib/tc/signing.ts`).

End-to-end test, 2026-09-24 (local build against production data, alias test deal): a 002 composed with every field kind, sent to the two test inboxes, opened from the emailed link on an iPhone 13 profile, consented, signed by the buyer (date and time pickers, a linked choose-one pair, a handed-over value changed), the seller emailed on the buyer finishing, signed, sealed with the certificate. Shared address, 2026-09-25: two buyers at admin@ each got their own named invite; the first finished, continued as the second on the same phone (the second's emailed link, same token), then the seller; sealed with three signers. Resend's delivery events (`email_events`) report every completed copy delivered, to marketing@ as well; that the marketing@ alias does not show attachment mail in the matt@ mailbox is on the receiving side, not the send.
