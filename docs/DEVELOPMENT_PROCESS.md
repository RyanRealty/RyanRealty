...
| `PUBLIC_PRODUCT/` | **live** — Public Product OS: the process→data→IA→visual→roll rebuild of the public site (admin-OS machinery replicated: filesystem memory, design amnesia, Matt locks, cut-list, ratchet gates; dual page objectives visitor+machine+exits). Constitution: `docs/plans/PUBLIC_PRODUCT/PUBLIC-PRODUCT-OS.md`. Bootstrap: `docs/plans/PUBLIC_PRODUCT/SESSION_BOOT.md`. **Destination** for public UX; sole process authority for the public site. Every file within is covered by this row. |
