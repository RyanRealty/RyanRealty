...
| `VISIBILITY_2026-09-22/` | **live** (2026-09-23) — the visibility-then-conversion pass: verified audit evidence, the fix brief, fifteen fix-package specs with their WIP branches, and the resume procedure (`README.md`, `resume-workflow.txt`). Every file within is covered by this row. |
...
| `PLACE_MEMBERSHIP_MISSION_2026-08-26.md` | **live** (2026-08-26) — goal of record for the place-membership mission Matt opened off the resort audit (`238c2f31`): the MLS-alias → county-plat map (repo item C-21, a NAME-MAPPING job — the fuzzy prefix rule is forbidden), the missing master-planned communities, the `/communities` junk-slug hole, the four wrong/missing boundary polygons, and the Pronghorn → Juniper Preserve rename. The closing review pass is measured against this file. |
| `SOCIAL_MEASUREMENT/` | **live** (2026-08-26) — goal of record for the social measurement mission (Matt 2026-08-26: "measure all of my social feeds", then "accurately track all traffic"). Records the four systems that reported healthy while emitting fabricated numbers — Grok image-to-video, Instagram, the Facebook page/post feeds, YouTube video scope — the consent ceiling that caps attribution, and the silent-zero guard that makes the class visible at session boot. The closing review pass is measured against this file. Every file within is covered by this row. |
| `CMA_FUNNEL_MISSION_2026-09-07.md` | **live** (2026-09-07) — the CMA funnel mission: the goal the final review pass is measured against (lanes + auto-send, document look, tracking to leads, engine health), the verified starting state, and the per-stream ledger. `CMA_STATE_OF_THE_WORLD.md` still wins on engine defects; the code outranks both. |
| `CMA_REIMAGINED_2026-09-07.md` | **live** (2026-09-07) — the blueprint for the rebuilt seller document, opened on Matt's verdict ("no one says band. fix the layout. the chart makes no sense. everything should be linking back to my website. it does not flow and looks awful. reimagine it"). Names the reader, the one register, the banned words, chapters 0 to 7 with the data contract each one renders from, and the tracked-link rule. It REPLACES the chapter order in `CMA_PRICE_OPINION_SPINE.md` for the seller document and closes `CMA_DOC_PUNCHLIST_2026-09-07.md`; the spine's voice and no-pitch rules, `lib/pricing/` ownership of every number, and the ORS 696 disclosure still bind. |
| `CMA_DOC_PUNCHLIST_2026-09-07.md` | **live** (2026-09-07) — the CMA document work order behind the funnel mission: P1–P10, each verified by the look-pass tool (`scripts/cma-lookpass.ts`) before Matt sees a URL. Closed items stay listed with the shot that proved them. |
| `CMA_STATE_OF_THE_WORLD.md` | **live** (2026-08-27) — the CMA reconciliation: what is BUILT and verified (with the stale claims in older CMA docs corrected), the eight real defects D1–D8 ranked by cost, and the locked build order Matt set (quality first, delivery after). Opened on Matt's directive to "unfuck" the tangle of stale CMA plans. When this file disagrees with an older CMA plan doc, this file wins; the code outranks both. |
...
| `VOICE-CANON-2026-08-05.md` | **retired 2026-09-07 (Matt): canon and gates removed; VOICE.md is now a one-page voice doc with no mechanical rules.** |
