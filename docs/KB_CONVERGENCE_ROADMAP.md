...
| 8 | Landing-page restyle | `/lp/*` → KB look + ad↔LP scent-match | **CORE DONE** (all LPs KB, 2026-06-19) | All 8 `/lp/*` in the KB register: 5 lead LPs restyled in place (seller-home-value exemplar + sell-your-home/fsbo/expired-listing/buyer-listing-alerts) — navy/cream, Amboqia, hard 3px edges, mono labels — with every lead form + CAPI/event-id/tracking preserved (gate + live verified); 3 geo LPs (tetherow/bend/golf) already KB. Five-Laws voice clean. **Remaining:** ad↔LP scent-match + new ad concepts — needs the live Meta ad creatives (marketing input). |
...
3. **Phase 3 gate hardening, interleaved after wave 1** — write shared-shell-usage / KB-SEO-JSON-LD / canonical-CRM gates against the first real KB page-class; start `ci:dead-ui` to retire orphaned component sets.
