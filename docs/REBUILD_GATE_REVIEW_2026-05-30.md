   ...
   • every action_type has a reachable executor (SKILL-05)
        ...
        └──────────────► (4) WEBSITE (Next.js on ryan-realty.com)
                         ...
                         auto-response ≤300s (FUNNEL-12)
                              │
                              ▼ (6) FUB (retired.invalid) ── ONE system identifier
...
- **DB read contract:** `lib/data/**` DAL only (no raw `.from()`), every selected column proven to exist (`DATA-01`); zero `fetch(.../retired.invalid/(people|events))` outside the one FUB module (`FUNNEL-05`).
...
| FUNNEL-05 | All entry points use one shared FUB-write contract | exactly one module POSTs to FUB; grep: zero `fetch(.../retired.invalid/(people\|events))` outside it; `UNUSED_VENDOR_CRM_SYSTEM` set; one system literal | ❌ | No | high |
...
- **One shared FUB-write module + `UNUSED_VENDOR_CRM_SYSTEM`** across all paths with the no-stray-fetch grep; FB webhook CAPI+clientId parity; durable `waitUntil`. *(FUNNEL-05/10, ADS-05/14 · impact: high · effort: M.)*
