...
| **What asking price may a listing print?** | `publishListingAsk` / `publishListingDrop` in `lib/listing/publish-listing-ask.ts`. Exact whole-dollar `ListPrice` at or above `MIN_PUBLIC_SALE_ASK` ($1,000). Withhold below that floor (Spark type-G lease crumbs $1.08 / $1.20 / $2.40 and $0). Do not print $0 as a home price. Drop is exact original minus exact ask. H1, cards, JSON-LD, and payment share that ask. | `ListPrice`, `OriginalListPrice` | n/a (pure) |
| **What beds / baths may a listing print?** | `publishListingRooms` in `lib/listing/publish-listing-rooms.ts`. Keep living area. Withhold beds+baths when 8+ beds at `< 150` sqft/bed or 8+ baths at `< 80` sqft/bath (7800 Rogue River 23/22/1,000). Hero, Facts, cards, JSON-LD, and listing-alert meta share that set. | `BedroomsTotal`, `BathroomsTotal`, `TotalLivingAreaSqFt` | n/a (pure) |
