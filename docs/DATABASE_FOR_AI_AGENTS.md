...
| Table | Rows | Purpose |
|---|---|---|
| `public.boundaries` | 3,537 | Polygons. `geo_type ∈ {city, neighborhood, subdivision, school_district, park, school, zip}`. **11 cities** (TIGER/Line, including Powell Butte CCD), **28 neighborhoods** (14 Bend districts + 14 resort communities), **3,427 subdivisions** (3,223 Deschutes County GIS plats + 204 Crook County GIS Subdivisions, LandGroup/7, SITE-58), **6 school districts** (Oregon Dept. of Education, W2.7), **42 schools** (Deschutes County GIS attendance, BoundaryFD/19; Crook/Jefferson/Culver/Gilchrist attendance unpublished — two-shape SITE-66), **18 parks** (6 Oregon State Parks + 11 county GIS + 1 OSM remainder), **10 ZIPs** (Census TIGER/Line 2024 ZCTA5, the CANONICAL_ZIPS set). PostGIS geometry in `polygon` (MULTIPOLYGON, SRID 4326). **Trails are NOT a `boundaries` geo_type** — see `public.trail_lines`. Jefferson County publishes no recorded-plat layer. |
...
it rot silently. **Jefferson publishes no public taxlot layer** — re-verified SITE-66, two query
shapes (`https://maps.co.jefferson.or.us/arcgis/rest/services` HTTPS fetch failed; HTTP 404).
