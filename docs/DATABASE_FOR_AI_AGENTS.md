...
| **May a plat page print YTD sold next to a year table?** | `publishPlatYtdStats` in `lib/market/publish-plat-year-sales.ts`. Year table is `getSubdivisionSalesHistory`. Withhold the `market_stats_cache` YTD strip when sold count or nearest-thousand median contradicts the current-year row. | year `closedCount`, `medianClosePrice` | n/a (pure) |
| **What active count may `/communities` print for one name?** | `publishCommunityIndexCount` in `lib/market/publish-community-index-count.ts`. Snapshot `activeSfrCount` wins. Featured and A-to-Z share that number for the same community name. | `activeSfrCount`, index tile count | ≤ 15 min |
