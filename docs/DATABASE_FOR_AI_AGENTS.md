...
| **`public.listing_boundary_xref_mv`** ⭐ | ~9.3K | **Precomputed listing→boundary spatial join** (mig `20260529020000`). ONE row per (boundary, on-market listing inside it): `(geo_type, geo_slug, listing_key, lat, lng, list_price, standard_status, property_type)`. Overlapping polygons (e.g. a Bend district AND the Tetherow resort, both `geo_type='neighborhood'`) each get their own row, so resort listings are correctly attributed even though their MLS `SubdivisionName` aliases collapse the `listing_tile_mv.boundary_neighborhood` column to the Bend district. **This is what the `listings_in_boundary` RPC reads** — a trivial indexed lookup, NOT a request-time `ST_Within`. Refreshed CONCURRENTLY by pg_cron job `refresh_dal_mvs_15min` (:05/:20/:35/:50, advisory lock 7107). The boundary-map pins + "homes for sale" cards on every city/neighborhood/community page come from here via the `getGeoBoundaryMapData` DAL. |
| `public.place_membership` | ~2.69M | **Market Truth membership**: one row per (listing, geo_type, geo_slug) for listed AND sold listings; `is_primary` = the smallest containing polygon (cities by MLS city text). Every `market_metric` cell joins it. Written by `place_membership_rebuild_keys(text[])`; kept current by pg_cron job `refresh_place_membership_15min` (:09/:24/:39/:54) calling `refresh_place_membership_changed()`, which rebuilds any listing whose `ModificationTimestamp` is newer than its rows. Freshness stamp: `mv_refresh_state` row `place_membership`. After a boundary edit, run the keyset sweep `refresh_place_membership(p_after, p_limit)` to the end. Service-role only (mig `20260823001500`, `20260923014500`). |
