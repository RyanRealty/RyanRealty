...
| `public.place_membership` | ~2.69M | **Market Truth membership**: one row per (listing, geo_type, geo_slug) for listed AND sold listings; `is_primary` = the smallest containing polygon (cities by MLS city text). Every `market_metric` cell joins it. Kept current by rebuilding any listing whose `ModificationTimestamp` is newer than its rows (or that has none). **Today (2026-09-23):** hourly at :08 by `/api/cron/refresh-mvs` through `lib/data/market-truth/refreshPlaceMembership.ts`, which rebuilds candidate runs with the keyset RPC `refresh_place_membership(p_after, p_limit)`; freshness = newest `computed_at`. **Once migration `20260923014500` is applied:** pg_cron job `refresh_place_membership_15min` (:09/:24/:39/:54) calls `refresh_place_membership_changed()` (rules in `place_membership_rebuild_keys(text[])`, advisory lock 7109, stamp `mv_refresh_state` row `place_membership`), and the route just calls that function. After a boundary edit, run the keyset sweep `refresh_place_membership(p_after, p_limit)` to the end. Service-role only (mig `20260823001500`, `20260923014500`). |
