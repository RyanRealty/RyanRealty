...
| **What asking price may a listing print?** | `publishListingAsk` / `publishListingDrop` in `lib/listing/publish-listing-ask.ts`. Exact whole-dollar `ListPrice`. Drop is exact original minus exact ask. H1, drop line, JSON-LD, cards, and payment share that ask. Withhold when thousand-round would print `$0` (sub-$500 commercial placeholders). | `ListPrice`, `OriginalListPrice` | n/a (pure) |
| **Which bedroom / bathroom counts may a listing print?** | `publishListingRooms` in `lib/listing/publish-listing-rooms.ts`. Positive counts only. Eight-plus rooms need living area density (100 sqft/bed, 80 sqft/bath). Sixteen-plus without living area withhold. Hero, facts, cards, and JSON-LD share that pair. | `BedroomsTotal`, `BathroomsTotal`, `TotalLivingAreaSqFt` | n/a (pure) |
