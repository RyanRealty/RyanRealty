...
| **Schools covering a community or neighborhood** | `get_place_schools(geo_type, geo_slug)` via `getPlaceSchools`. Attendance polygons in `boundaries` where `geo_type='school'` (Deschutes County GIS). Keep a school only when its share of the place is ≥ 0.05. Names and levels come from `data/co-schools.ts`. Do not use `get_subdivision_schools` (one MLS modal per level) for this list, and do not print the share. Crook, Jefferson, Culver, and Gilchrist attendance is unpublished, so an empty result is omitted. | `geo_slug`, `geo_label`, `share` | polygons are manual; DAL cache 6h |
| **Homes inside a boundary** (map pins + "homes for sale" cards on city/neighborhood/community pages) | `listing_boundary_xref_mv WHERE geo_type=<type> AND geo_slug=<slug> AND standard_status='Active'` — via the `listings_in_boundary` RPC, wrapped by the `getGeoBoundaryMapData` DAL. **NEVER `ST_Within` against `listings` at request time** (it blows the anon 3s timeout — see §4d). | `listing_key`, `lat`, `lng`, `list_price` | ≤ 15 min (pg_cron job `refresh_dal_mvs_15min`, :05/:20/:35/:50) |
...
| **Which URL opens the regional homes set the homepage names?** | `publishRegionalSearchHref` in `lib/search/publish-regional-search-href.ts` → the bare `/homes-for-sale`. Since 2026-09-23 it opens split (desktop) / list (phones) on `CENTRAL_OREGON_BOUNDS`, and its regional frame counts the service-area set with no bbox (`getViewportSearch` option `frame: 'region'`, rules in `lib/search/search-opening.ts`), the same population `?view=list` counts. No view injects a city. | `listing_search_mv` via `searchListingsAll` | n/a (pure) |
...
| **`public.listing_tile_mv`** / **`public.listing_search_mv`** ⭐ | ~597K / ~9.3K | **Serving views over two TABLES maintained incrementally** (migrations `20260924163000` + `20260924173000`; they were matviews refreshed whole every 30 and 15 minutes until 2026-09-24). `listing_tile_mv_src` holds one row per `listings` row, `listing_search_mv_src` one per on-market listing; each row is defined once, in the views `listing_tile_row_def` / `listing_search_row_def`. Statement-level triggers on `listings`, `listing_feature_flags` and `listing_private` queue the changed `"ListNumber"`s in `listing_mv_queue`; pg_cron `listing-mv-drain` (every minute, advisory lock 7110) recomputes exactly those rows and stamps `mv_refresh_state` for both; `listing-mv-reconcile-nightly` (11:40Z, lock 7111) compares every row with its definition, re-queues drift and logs counts in `listing_mv_reconcile_log`; rows a key could not apply land in `listing_mv_errors`. **Freshness: within about a minute of the sync.** `refresh_listing_tile_mv()` / `refresh_listing_search_mv()` still exist and now drain. Never `REFRESH MATERIALIZED VIEW` either source: they are tables. |
| **`public.listing_boundary_xref_mv`** ⭐ | ~9.3K | **Precomputed listing→boundary spatial join** (mig `20260529020000`). ONE row per (boundary, on-market listing inside it): `(geo_type, geo_slug, listing_key, lat, lng, list_price, standard_status, property_type)`. Overlapping polygons (e.g. a Bend district AND the Tetherow resort, both `geo_type='neighborhood'`) each get their own row, so resort listings are correctly attributed even though their MLS `SubdivisionName` aliases collapse the `listing_tile_mv.boundary_neighborhood` column to the Bend district. **This is what the `listings_in_boundary` RPC reads** — a trivial indexed lookup, NOT a request-time `ST_Within`. Refreshed CONCURRENTLY by pg_cron job `refresh_dal_mvs_15min` (:05/:20/:35/:50; advisory lock 7107 once migration `20260923014600` is applied). The boundary-map pins + "homes for sale" cards on every city/neighborhood/community page come from here via the `getGeoBoundaryMapData` DAL. |
| `public.place_membership` | ~2.69M | **Market Truth membership**: one row per (listing, geo_type, geo_slug) for listed AND sold listings; `is_primary` = the smallest containing polygon (cities by MLS city text). Every `market_metric` cell joins it. Kept current by rebuilding any listing whose `ModificationTimestamp` is newer than its rows (or that has none). pg_cron job `refresh_place_membership_15min` (:09/:24/:39/:54) calls `refresh_place_membership_changed()` (rules in `place_membership_rebuild_keys(text[])`, advisory lock 7109, stamp `mv_refresh_state` row `place_membership`); the interim Vercel route `/api/cron/refresh-mvs` was deleted 2026-09-24 once that stamp existed. After a boundary edit, run the keyset sweep `refresh_place_membership(p_after, p_limit)` to the end. Service-role only (mig `20260823001500`, `20260923014500`). |
...
| **Brasada Ranch** | `brasada-ranch` | Powell Butte | 1 (single MLS name) | 25 | Corrected 2026-08-26. Powell Butte View REMOVED — 1/148 inside (0.7%), scattered rural parcels 6–9km off the ranch. SITE-58 (2026-09-09) ingested Crook County GIS Subdivisions LandGroup/7 OBJECTID 186 as `geo_type='subdivision'` / `geo_slug='brasada-ranch'` (888 acres). The neighborhood row remains the community outline. |
| **Widgi Creek** | `widgi-creek` | Bend | 4 (+ PointsWest + Elkai Woods + Milepost 1) | 6 | Outline rebuilt 2026-09-25 (migration `20260925031047`): the ST_Union of its nine recorded plats, Points West, Milepost 1 and Elkai Woods Townhomes Phases I-VI (45.4 acres); PointsWest 136/136, Elkai Woods 58/58, Milepost 1 34/34 inside. The county INN OF 7TH MOUNTAIN community polygon it used to hold is now `inn-of-the-7th-mountain` (minus these plats, 271.2 acres). OPEN: the MLS files the Seventh Mountain Golf Village plat both as '7th Mtn Golf Village' (an Inn alias) and as 'Widgi Creek' (113 of 148 geocoded 'Widgi Creek' listings sit on it); which community owns that plat is Matt's call. |
...
The 3,427 subdivision polygons in `boundaries` exist for property-level tagging (`listings.boundary_subdivision`) and for the parent-community polygon derivation. **They are NOT the primary aggregation taxonomy** — per the Spark MLS evidence ([SPARK_API_REFERENCE.md](SPARK_API_REFERENCE.md)), the MLS field is the universal community name and the county GIS plats are a separate ontology (legal-plat records). Deschutes is BoundaryFD/4 (3,223). Crook is LandGroup/7 (204, SITE-58). Jefferson publishes no recorded-plat layer.
