...
- **How:** pull through Matt's SkySlope Forms session (`reference_skyslope_forms_api`: token in `sessionStorage['com.skyslope.id.tokens']`, base `forms.skyslope.com/library/api/`, `api-version=2.0`, `GET /libraries` + `/form-versions?libraryId=`; lib ids OREF 1340 / ODS 1528 / OR 1837/3208). Download blanks, store in `tc-documents` Storage, extract AcroForm field maps where present. **Needs Matt's live Forms session.**
- **Data model:** populate `tc_form_libraries` + `tc_form_versions` (`blank_pdf_storage_path`, `field_map`, `signer_profile`, `effective_date`).
- **UI:** `/admin/forms` → add library filter + per-form "Add to deal"; a deal-side "Add form" picker.
- **Compliance/licensing:** OREF blanks are copyrighted — load under Matt's member access only, never redistribute. The engine is generic.
- **Acceptance:** browse libraries filtered by ODS/OREF/OR; add a form to a deal → it becomes an envelope-able document.
- **Depends-on:** Matt's live SkySlope Forms session.

#### T2.2 — Template field-mapper + auto-fill from deal/MLS data  ⟵ the differentiator
- **Goal:** place fill + signature fields ONCE per `tc_form_versions` row (tagged with data binding + signer role); "Write an Offer" instantiates a filled, verified OREF template (not a blank upload). Beats SkySlope by merging our richer deal + MLS data.
- **Data model:** `tc_form_versions.field_map` jsonb — array of `{ key, type ('text'|'signature'|'initials'|'date'|'checkbox'), page, x, y, w, h, binding (path into deal/MLS data, e.g. 'sale_price', 'buyer.0.name'), signer_role, required }`. Reuse the geometry convention from `lib/tc/signing.ts`.
- **Server actions:** extend `tc-envelopes.ts` — `createEnvelopeFromTemplate(cycleId, formVersionIds)` renders the blank, merges deal data into `binding` fields (pdf-lib text draw), then places signature fields from the map. Reuse `lib/tc/seal-pdf.ts` drawing.
- **UI:** a field-mapper admin tool (place fields once per form version) + the existing composer for per-envelope tweaks.
- **Acceptance:** pick an OREF sale agreement template on a deal → the rendered PDF has the address/price/parties/dates filled and signature fields placed for buyer+seller; send + sign works end to end.
- **Depends-on:** T2.1 (real blanks).
