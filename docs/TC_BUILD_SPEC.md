...
- **Goal:** detect when a library form we hold has been revised at the source (a newer published version exists) and we're using the old one — then flag it and offer a one-click update. This is what SkySlope does ("you're on an old version"). Also flag **new** published forms we do not hold, and forms the source retired.
- **How it works (catalog check SHIPPED 2026-08-14):** a metadata-only list of current published versions (`status === 'Published' && id === publishedVersionId`) for OREF `1340`, ODS `1528`, Oregon Realtors `1837`. Diff in `lib/tc/form-catalog-diff.ts` (match `source_form_id`, then form number). Persist `tc_form_catalog_items` + `tc_form_catalog_checks`. Held rows get `update_available` + `pending_source_version_id`. UI: `/admin/forms` filters + badges. OREF packet warns when the preferred 001 is stale.
- **Trigger (honest constraint):** the recheck needs SkySlope auth. Until Partnership-API credentials exist, run the console script from the signed-in Forms tab, then paste JSON on `/admin/forms`. Optional `POST /api/admin/forms/catalog-check` with `TC_FORMS_INGEST_SECRET`. Upgrade path: weekly cron once partner creds exist.
- **Update flow (not yet):** `updateFormVersion(formVersionId)` — pull the new blank via the existing ingest loader → insert a NEW `tc_form_versions` row → set the old row's `retired_at` + `superseded_by` → **carry the field map forward ONLY if the layout is unchanged**. Until that lands, ingest the new `sourceVersionId` the same way as a first load.
- **Compliance:** placing a signature on a stale/wrong form layout is the risk this prevents. Using a superseded form warns, and does not silently proceed.
- **Acceptance (catalog half):** with a form whose source `publishedVersionId` is newer than ours, the check flags it; new source forms appear as "New at source"; the packet warns on outdated 001.
