...
| **Meta campaign actively running** *(iter 2)* | +5 / 0 | spend > 0 AND impressions > 0 |
| **GA4 traffic above noise floor** *(iter 2)* | +5 / 0 | sessions ≥ 100 in 30d |

**Verdict thresholds:** `>=75 strong`, `50–74 needs_attention`, `<50 at_risk`.

**Bend market context auto-recommendation** *(iter 2)*: when `bendMarketContext.monthsOfSupply ≤ 4.0` (seller's market), the report card automatically appends a `[SCALE][MEDIUM]` recommendation telling the agent this is the right time to push seller-acquisition spend.
...
Sorted by ROI, highest-impact first. Items shipped in iteration 2 are crossed out.

1. ~~**Improve `applied_count` in FUB outreach.**~~ Resolved 2026-05-11 with granular tracking. `applied_count_breakdown` now ships in the packet with `state_updated_count`, `note_added_count`, and `at_least_one_applied_count`. We now know that when a FUB contact is updatable, both attempts succeed; the gap (47/150 in the latest run) is contacts where FUB rejects both calls (likely due to invalid person ids in the cache or rate-limited tier).
...
3. ~~**Add Bend market context to the weekly packet.**~~ Resolved 2026-05-11. Every packet now includes `bend_market_context` (active, median list, MoS, sold 30d, median close 90d, market health label) pulled from `market_pulse_live`. Auto-recommendation fires when MoS ≤ 4.0.
4. ~~**Add CAPI `value` to every Lead event for Meta bid optimization.**~~ Resolved 2026-05-11. Contact form sends $200 / $300 / $500 by inquiry type, valuation form sends $500. Meta uses this to push budget toward higher-intent conversions.
5. ~~**Weekly digest email + at_risk alert.**~~ Resolved 2026-05-11. After every marketing cron run, `MARKETING_DIGEST_EMAIL` (or `ADMIN_EMAIL`) receives a brand-styled HTML digest with score, verdict, recommendations, Bend context, and the agent pickup prompt. Subject line escalates to `[ALERT]` when the verdict drops to `at_risk`.
6. **Get a Meta campaign running.** Latest packet recommends `[FIX][HIGH] No active Meta campaign this cycle` — Meta Ads reports zero spend / zero impressions in the last 30 days. Optimization can't optimize what isn't running. Action lives in Meta Ads Manager.
7. **Restore `fub_contacts_cache` mirror table.** The dashboard pipeline snapshot pays a FUB API cost on every render right now; only the weekly cron should pay that. Spec a small sync (FUB people → Supabase) on a 30-min cron.
8. **Investigate the `103 of 150 outreach attempts that get rejected by FUB`.** State + note both fail with the exact same count (47/150 succeed = 103 fail both). Add per-contact error capture in the execution items so we know whether it's invalid person ids in the snapshot, FUB rate limiting, or contacts in restricted states.
9. **Decide on a strict broker user map.** Setting `FOLLOWUPBOSS_BROKER_USER_MAP=matt-ryan:<userId>` would let `fetchMyLeadsFromFubLive` skip the email lookup and shave a network call per cron.
10. **Resolve the Bend Policy Pulse 3-part series state mismatch.** Files `bend_policy_pulse_part{1,2,3}.mp4` are in `public/v5_library/` but the scorecard says they're drafts awaiting review. Tracked separately from this pipeline but called out here for handoff completeness.
