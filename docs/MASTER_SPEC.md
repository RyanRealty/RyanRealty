...
| CRM (in-house `public.crm_people`) | `/Users/matthewryan/RyanRealty/lib/crm/send-event.ts` + `/Users/matthewryan/RyanRealty/lib/crm/send-event.ts` for retired-vendor history |
...
Note: buyer lead cycle is longer than seller lead cycle (6–18 months from first contact to close for relocators). Lead scoring decay and engagement scoring in the CRM are critical to maintaining pipeline quality over that window. [discovery 01: Lead Routing; field research (`engagement_streak_days`, `last_active_date`) archived at `lib/crm/send-event.ts`]
...
13. **No 404 on sold listings, and no noindex either — the policy is stated once, in §4.9 "Listing not found / sold / withdrawn".** This item is a pointer, not a second copy: an off-market detail URL returns HTTP 200 with the honest off-market state and stays `index, follow` (Matt 2026-09-08, SITE-32). Do not restate the rule here — the one thing this URL class has never survived is two written policies. The only clause that is local to URL design: delete a listing URL only if the listing was never legitimately published (e.g. test data).
...
- **Matt ruled 2026-09-08 (SITE-32): off-market listing URLs stay INDEXED, `index, follow`, carrying the honest state above. This paragraph is the one surviving policy.** The competing recommendation in `docs/plans/data-architecture-plan.md` Part J §1 — keep the page but `noindex` it, `Offer.availability: SoldOut`, optionally 410 after 12 months — was deleted the same day; neither had ever been implemented, and two written policies for one surface is how a page ends up with none. A Closed detail page showing `ClosePrice` is **not** a VOW-only sold surface under ODS §5-4 A.4 for indexing purposes, so G54 (`ci:ods-compliance`, which pins A.4 for the search presets) is deliberately not extended to detail pages. Measured over GSC 2026-06-08..2026-09-05 across 14,508 listing-detail URLs: off-market is 6,611 URLs and 26,123 impressions (46% of listing-detail impressions) — Closed 3,207 URLs/12,125 impressions, Pending 1,431/6,105, Expired 790/3,226, Canceled 748/2,860, Withdrawn 435/1,807 — at a CTR that straddles or beats Active, roughly 324–577 clicks per 90 days. An address query has no Active substitute, so a `noindex` deletes those clicks rather than redistributing them. These are residual index, not sitemap rows: `lib/data/sitemap/getListingSitemapRows.ts` submits Active/AUC only, and that stays. Held mechanically by `scripts/check-listing-offmarket-index.mjs` (`ci:listing-offmarket-index`), which fails if either listing-detail metadata chokepoint starts emitting `noindex` for anything but the two sanctioned cases: the refusal path (`getListingDetail` → null, i.e. IDX opt-out or Coming Soon) and SITE-33's out-of-area cities.
