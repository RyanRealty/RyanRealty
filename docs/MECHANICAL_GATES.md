...
| **G36** | **AI-tool discipline + skill auto-load contract** — (Layer 1, hard) `automation_skills/content_engine/SKILL.md` AND `marketing_brain_skills/producers/TEMPLATE.md` must both reference the `tool-mastery` + `viral-playbook` skills (so they actually auto-load at produce-time) and both skill files must exist; (Layer 2, ratcheted) no NEW inline ElevenLabs / Replicate-video API calls — VO routes through `scripts/_voice_lib.py` / `lib/voice/`, Replicate video through the shared helper. Grandfathered set in `scripts/.tool-discipline-baseline.json` may only shrink. | `scripts/check-tool-discipline.mjs` (`npm run ci:tool-discipline`, in `ci:gates`) | CLAUDE.md "Marketing Brain Architecture" + organic-growth engine 2026-05-29 |
