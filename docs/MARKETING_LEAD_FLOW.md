...
**Live contract (2026-08-18):** Capture is **`sendEvent()`** in **`lib/crm/send-event.ts`** → **`ensureNativeLead()`**. Follow Up Boss is decommissioned. There is no People API fallback that creates a live vendor contact. Leftover `FOLLOWUPBOSS_*` env names in Vercel do nothing.
