...
screenshot it at 390px and show Matt — pixels, not claims.

## Why we can beat a thin activity list

We have `fetchLiveVisitors` / `fetchLiveSummary` — real engagement scores +
intent tags (the 890 "mortgage_curious" lead, hot/active badges). Every place
a vendor CRM would show a flat list, we show who's hot right now.

## The mobile patterns to match

1. **Lead detail is TABBED, not one long scroll.** Dark header (avatar + name +
   status) then horizontal tabs. Tab set: Info · Comms · Homes · Notes ·
   Calendar · Automations. Each tab is one focused phone screen. OUR mapping →
...
3. **People has two segments: All Lists / Stages.** All Lists = smart lists with
   LIVE counts (e.g. "All Recent Online Activity 20", "All Expireds 657").
   Stages = stage buckets with counts (Seller Prospect 7.5k, Lead). Maps onto
   our stage filter chips + (future) saved/smart lists.
4. **Home = a segmented activity feed** (Everyone: New Leads / Emails /
...
7. **The global "+" quick-action menu (FAB) — Matt flagged this as "very
   ...
   workflow" and "Start a CMA") and it surfaces the
...
Matt flagged mobile as still clunky. Root cause: the 2026-06-16 pass shipped
the tabbed detail + FAB + segments but **never a bottom tab bar**, so
navigation lived in a horizontal-scroll strip + hamburger. Fixed:
...
- ✅ **Shared mobile kit** — `components/admin/shared/mobile/CrmMobileKit.tsx`:
  ...
  for the look so screens can't drift. Token-pure except the brand-external
  avatar/action colors (tracked in `.design-token-lint-ignore`).
- ✅ **People list** — rows (avatar · name · source · date · chevron),
...
- ✅ **Tasks** — agenda: `CrmSegmented` view switch + date-grouped
...
- ✅ **People — Stages + All Lists** — the leads list gains a Stages / All lists
  toggle. Stages: per-stage counts (e.g. Seller Prospect 7.5k) + book total.
...
- ✅ **"Online now" live list** (our edge) — All Lists leads with a live
  ...
  linking to the live-visitor surface.
...
**The contract is built.** Several patterns beaten with live web-intent
(Website segment leads by engagement, "Online now" list, the context-aware "+").
Remaining ideas are genuinely new scope, not parity:
...
- This doc is the saved target.
- Mobile views must use the console kit (`ConsoleSection` etc.) — gated by
  `ci:console-kit`.
- Every surface: build → screenshot at 390px → show
