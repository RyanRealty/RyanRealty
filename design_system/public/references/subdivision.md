# References — `subdivision`

Table mark: **59** (`design_system/public/taste-table.json`, instrument v1-2026-09-08, evaluated 2026-09-08).
Route: `app/subdivisions/[slug]/page.tsx` · sample URL: `/subdivisions/ridge-at-eagle-crest`.

Cite this file from the SITE node brief. Hand it to the evaluator so `beats` is judged against a named page, not an invented compliment.

## What the table says is dull

The hero's second and third lines — the raw MLS-source sentence and the templated "Eagle Crest, the resort Ridge at Eagle Crest sits inside" caption — read as QA output stapled to the display headline rather than composed copy; it's the flattest object in either shot because it breaks the editorial register the Amboqia headline just set, immediately, one line down.

## External pages to beat (one sentence each)

1. **Eagle Crest resort site** (https://www.eagle-crest.com/) — Subject owns setting photography and story; strip MLS jargon from the hero and put trace on a source chip.
2. **Zillow subdivision / building pages** (https://www.zillow.com/) — Place claim in plain language; inventory map without methodology sentences in the H1 band.
3. **V3Quiet + V3SourceLine (shipped)** (components/site/v3/V3SourceLine.tsx) — Authored claim sentence per plat; trace moves to a tap/hover source chip, never the primary subhead.

## Adapt from shipped code (do not generate from adjectives)

- `components/site/v3/V3Quiet.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."
- `components/site/v3/V3Atlas.client.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."
- `components/site/v3/V3SourceLine.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."

## House form to reach for

Per TASTE.md preference order, replace the dull display with: **hero figure claim sentence; source as chip (not jargon subhead)**.

Banned fallback: another stacked Quiet → figure-row → ledger. If the section is data, pick a form from: hero figure · stat tile with sparkline · emphasis line with a scrubber · horizontal bar · dot strip · slope · small multiples · beeswarm · map with data-encoded cells · table.

## Variants rule

A NEW data-display section on this class ships as two or three named variants behind one prop. Capture with:

```bash
node scripts/take-route-shots.mjs --variants a,b,c subdivision <url>
```

Matt picks from the decision sheet; losers are deleted in the commit that records the pick.
