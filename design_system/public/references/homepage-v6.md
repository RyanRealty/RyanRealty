# References — `homepage-v6`

Table mark: **56** (`design_system/public/taste-table.json`, instrument v1-2026-09-08, evaluated 2026-09-08).
Route: `app/page.tsx` · sample URL: `/`.

Cite this file from the SITE node brief. Hand it to the evaluator so `beats` is judged against a named page, not an invented compliment.

## What the table says is dull

The hero: a beautiful photo and clean typography, but it is exactly the full-bleed-photo-plus-search-bar every competing real estate site opens with — a person would keep scrolling past it looking for something distinctive rather than stop here.

## External pages to beat (one sentence each)

1. **Redfin homepage** (https://www.redfin.com/) — Search hero carries a live market or inventory signal, not only a photo bar.
2. **Stripe homepage restraint** (https://stripe.com/) — One composed object in the hero that rewards looking past the primary field.
3. **V3Stage + V3Pulse (shipped)** (components/site/v3/V3Stage.tsx) — Stage can host a live pulse figure; fold a moving market signal into the photo hero.

## Adapt from shipped code (do not generate from adjectives)

- `components/site/v3/V3Stage.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."
- `components/site/v3/V3Pulse.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."
- `components/site/v3/V3Ask.client.tsx` — open the file; adapt its markup/CSS rather than restating "quiet / editorial / expensive."

## House form to reach for

Per TASTE.md preference order, replace the dull display with: **hero figure / pulse inside Stage beside the ask**.

Banned fallback: another stacked Quiet → figure-row → ledger. If the section is data, pick a form from: hero figure · stat tile with sparkline · emphasis line with a scrubber · horizontal bar · dot strip · slope · small multiples · beeswarm · map with data-encoded cells · table.

## Variants rule

A NEW data-display section on this class ships as two or three named variants behind one prop. Capture with:

```bash
node scripts/take-route-shots.mjs --variants a,b,c homepage-v6 <url>
```

Matt picks from the decision sheet; losers are deleted in the commit that records the pick.
