...
2. **Field** — live inventory as a spatial surface: map + list in one frame, hover/tap
   bound both ways, counts honest to the viewport. Opens Homes. Embeds in City,
...
4. **Stage** — full-bleed media (owned video/photo) carrying one line of type and one
   ...
   asset exists. Never over a number. Opens Listing. Opens City. Opens Master-plan when an owned
   place photo exists.
...
| City | `/cities/[slug]`, `/zip/[zip]` | Stage (live place photo + H1 + type-and-go search), then Field of this city's houses. | Child neighborhoods and master-plans are doors below the fold. Chart Room is Time/Relate/Rank. |
...
- **2026-08-28** — City grain opens Stage (hero_image_url first, H1, type-and-go
  search with one Type control), then Field. Chart Room is V3Chart Time/Relate/Rank
  mid-page. Buyer voice on the face.
