-- refresh_market_fact_sale: a bounded window reads only its own closings.
--
-- With p_until set (a historical rebuild, one window at a time) the function
-- filtered on "CloseDate"::date, which no index can serve, so every call read
-- all ~385,000 closed listings: on 2026-09-25 one half-year call spent 28 s in
-- that scan alone (98,011 blocks read from disk for 5,690 rows) and several
-- whole-year and half-year calls ran past the 120 s statement limit.
--
-- The window now also bounds the raw timestamp, with a day of slack either side
-- so no session time zone can move a closing across it, and the scan uses the
-- close-date index. The ::date filters still decide which closings belong to
-- the window, so no row is added or dropped. Without p_until (the daily
-- modified-since refresh) the query is unchanged. The body is otherwise
-- 20260925060000_market_fact_sale_absent_from_mls.sql.
...
BEGIN
  ...
  WITH src AS (
    ...
    WHERE l."StandardStatus" = 'Closed'
      ...
      -- A bounded window also bounds the raw timestamp (a day of slack for the
      -- session time zone), so the close-date index can serve the scan.
      AND (p_until IS NULL OR l."CloseDate" < (p_until + 1)::timestamptz)
      AND (p_until IS NULL OR p_since IS NULL OR l."CloseDate" >= (p_since - 1)::timestamptz)
  ),
  ranked AS (
    SELECT
      s.*,
