-- PERF fix for p_neighborhood_slug in search_listings_advanced.
--
-- The 20260707100000 version expressed the neighborhood scope as correlated
-- EXISTS subqueries inside an OR chain. The planner could not fold that into
-- an index path, so every neighborhood-scoped call seq-scanned all ~589K
-- listings (~3.8s) and blew the 3s statement timeout on the app's PostgREST
-- role — the alert engine then silently skipped the search (0 results).
--
-- Fix: resolve the neighborhood scope BEFORE the main query.
--   * bend-* district slugs → array of listing keys from listing_boundary_xref_mv
--     (small, on-market only), matched with l."ListingKey" = ANY(...) on the PK.
--   * resort slugs → array of SubdivisionName alias labels from
--     neighborhood_subdivisions, matched with l."SubdivisionName" = ANY(...)
--     via idx_listings_subdivision_name.
-- Custom plans constant-fold the p_neighborhood_slug arms, leaving one
-- indexable condition (verified: tetherow 3.8s → ~50ms).
--
-- WHY the field exists at all: default neighborhood saved searches (one per CRM neighborhood list —
-- Matt directive 2026-07-06) need the alert engine to match listings by
-- canonical neighborhood slug ('bend-river-west', 'tetherow', ...). The saved
-- filter model previously had no neighborhood dimension: boundary_neighborhood
-- on listings/listing_tile_mv holds only the Bend district LABEL and is blank
-- for resort communities, so it cannot serve both. listing_boundary_xref_mv
-- (the precomputed listing→boundary spatial join, refreshed by
-- /api/cron/refresh-mvs) covers every neighborhood boundary — Bend districts
-- AND resort communities — with an indexed (geo_type, geo_slug, listing_key)
-- lookup, so the new predicate is a cheap semijoin, not a spatial scan.
--
-- TWO MATCH STRATEGIES (verified against live data 2026-07-06):
--   * bend-* district slugs → polygon xref. The Bend district boundaries are
--     official City of Bend GIS Neighborhood Districts and the xref counts are
--     accurate. (The xref MV holds ON-MARKET rows only — Active / Coming Soon /
--     Active Under Contract / Pending — which is exactly what the alert engine
--     needs.)
--   * resort-community slugs (tetherow, broken-top, ...) → SubdivisionName
--     alias match via neighborhood_subdivisions, the canonical registry per
--     docs/DATABASE_FOR_AI_AGENTS.md §3a. The 'spatial discovery v6' polygons
--     for these communities OVER-MATCH badly (broken-top polygon: 346 active
--     incl. Tetherow + Awbrey Butte homes vs 21 true alias matches; NWX: 221
--     vs 22) and must not feed listing emails (§0 data accuracy).
--
-- Same consolidation pattern as 20260705120000: drop every overload, recreate
-- ONE function, so PostgREST never sees an ambiguous candidate set.
...
CREATE FUNCTION search_listings_advanced(
  ...
  p_neighborhood_slug text DEFAULT NULL,  -- bend-* → listing_boundary_xref_mv; resort slugs → neighborhood_subdivisions aliases
...
AS $$
DECLARE
  v_nbhd_keys text[] := NULL;   -- bend-* districts: on-market listing keys from the polygon xref
  v_sub_labels text[] := NULL;  -- resort communities: canonical SubdivisionName aliases
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      -- Neighborhood scope (precomputed above so the planner gets a plain
      -- indexable ANY() instead of a correlated EXISTS it can only seq-scan).
...
COMMENT ON FUNCTION search_listings_advanced IS 'Advanced listing search: flat + details jsonb + amenities. Single authoritative overload. Supports off-market statuses (expired/withdrawn/canceled/off_market/active_or_offmarket), multi-city (p_cities), either-or view match (p_view_contains_any vs view_description), off-market recency window (p_off_market_within_days), exclude-sold-since (p_exclude_sold_since), and neighborhood scope (p_neighborhood_slug: bend-* districts via listing_boundary_xref_mv polygon keys, resort communities via neighborhood_subdivisions alias labels — both precomputed into arrays for index use). Used by /listings and /search.';
