-- Market Truth — zip grain + leftover honest stats on compute_market_metrics_shadow.
-- Neighborhood, subdivision, and county grains stay unpublished (REGISTRY §4, F18).
-- Zip membership is PostalCode city_text (not polygons / ST_Within).
-- 91 service-area zips > 25: compute the 10 canonical public zips only (timeout cap).
-- commercial_lease (G) is rent, not a sale, and stays out.
-- 20260823140000 remains YoY+mix; this replaces the function.
--
-- Skipped (not faked):
--   median_concession_reported — no honest concession_reported flag in this job.
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      THEN 'zip_not_on_public_allowlist'
