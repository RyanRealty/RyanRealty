-- Add p_neighborhood_slug to search_listings_advanced.
--
-- WHY: default neighborhood saved searches (one per CRM neighborhood list —
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      -- Neighborhood scope. bend-* districts: official City of Bend GIS polygon
      -- via the precomputed xref MV. Resort communities: canonical
      -- SubdivisionName alias registry (the v6 discovery polygons over-match).
      AND (p_neighborhood_slug IS NULL OR p_neighborhood_slug = ''
        OR (p_neighborhood_slug LIKE 'bend-%' AND EXISTS (
              SELECT 1 FROM listing_boundary_xref_mv x
              WHERE x.geo_type = 'neighborhood'
                AND x.geo_slug = p_neighborhood_slug
                AND x.listing_key = l."ListingKey"))
        OR (p_neighborhood_slug NOT LIKE 'bend-%' AND EXISTS (
              SELECT 1 FROM neighborhood_subdivisions ns
              WHERE ns.neighborhood_slug = p_neighborhood_slug
                AND l."SubdivisionName" = ns.subdivision_label)))
...
COMMENT ON FUNCTION search_listings_advanced IS 'Advanced listing search: flat + details jsonb + amenities. Single authoritative overload. Supports off-market statuses (expired/withdrawn/canceled/off_market/active_or_offmarket), multi-city (p_cities), either-or view match (p_view_contains_any vs view_description), off-market recency window (p_off_market_within_days), exclude-sold-since (p_exclude_sold_since), and neighborhood scope (p_neighborhood_slug: bend-* districts via listing_boundary_xref_mv polygon, resort communities via neighborhood_subdivisions aliases). Used by /listings and /search.';
