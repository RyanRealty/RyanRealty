-- Market Truth Step 4 — shadow compute of REGISTRY.md stats.
-- Writes public.market_metric. Does not repoint any public reader (D3).
...
BEGIN
  ...
  segs AS (
    SELECT unnest(ARRAY['detached', 'all_residential']) AS segment
  ),
  wins AS (
  ...
  closed AS (
    ...
    JOIN public.market_fact_sale f
     ...
     AND f.close_date <= v_end
    WHERE (
      (s.segment = 'detached' AND f.segment = 'detached')
      OR (s.segment = 'all_residential' AND f.property_type = 'A' AND f.segment IS DISTINCT FROM 'fractional')
    )
    GROUP BY 1, 2, 3, 4
  ),
  active AS (
    ...
    WHERE l."StandardStatus" = 'Active'
      AND l."PropertyType" IS DISTINCT FROM 'G'
      AND (
        (s.segment = 'detached'
          AND l."PropertyType" = 'A'
          AND l.property_sub_type = 'Single Family Residence')
        OR (s.segment = 'all_residential'
          AND l."PropertyType" = 'A'
          AND coalesce(l.property_sub_type, '') NOT IN (
    ...
    WHERE l."StandardStatus" IN ('Pending', 'Active Under Contract')
      ...
      AND (
        (s.segment = 'detached'
          AND l."PropertyType" = 'A'
          AND l.property_sub_type = 'Single Family Residence')
        OR (s.segment = 'all_residential' AND l."PropertyType" = 'A')
      )
    GROUP BY 1, 2, 3
  ),
  cells AS (
