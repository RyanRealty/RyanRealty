-- Fix the PropertyType filter in search_listings_advanced.
--
-- BUG: the clause read `l."PropertyType" ILIKE '%' || p_property_type || '%'`,
-- matching the parameter as a substring of the listing's PropertyType. But
-- listings."PropertyType" stores the MLS *code* (single letters A-H), NOT a
-- label. The search UI sends labels (Residential / Land / Commercial), so
-- `ILIKE '%Residential%'` against PropertyType 'A' matched ZERO rows — every
-- property-type-filtered search silently emptied (e.g. /homes-for-sale/<city>/<preset>
-- with ?propertyType=Residential returned nothing while 263 homes qualified).
--
-- FIX: the DAL now maps the label to its code set via propertyTypeFilterToCodes
-- (lib/property-type.ts: Residential -> A,B,C; Land -> D; Commercial -> E,F,G,H)
-- and passes the codes comma-joined. The function matches them exactly with
-- `= ANY(string_to_array(p_property_type, ','))`. A single raw code ('A') still
-- works (a one-element array). Every other clause is byte-identical to the live
-- definition in 20260603140000_fix_golf_course_filter_populated_signals.sql.
...
COMMENT ON FUNCTION public.search_listings_advanced(text,text,text,numeric,numeric,integer,integer,numeric,numeric,numeric,numeric,integer,integer,numeric,numeric,text,text,text,text,boolean,integer,boolean,boolean,boolean,boolean,boolean,text,integer,text,integer,integer)
  IS 'Advanced listing search: flat + details jsonb. PropertyType filter matches the MLS code set exactly via ANY(string_to_array(p_property_type, '','')) (the DAL maps Residential/Land/Commercial labels -> codes A-H). golf course filter (p_has_golf_course) reads populated signals: details.View ILIKE golf OR PublicRemarks ILIKE golf course/fairway/golf community. Used by /listings and /search + presets.';
