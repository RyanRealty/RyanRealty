-- A closed sale the MLS no longer serves is left out of every Market Truth
-- statistic (Matt 2026-09-25: "Leave them out").
--
-- The daily closings reconciliation (lib/sync/closingsReconcile.ts) looks up,
-- by listing key, every sale we hold as closed that Spark did not return for
-- its window. A key Spark returns nothing for at all is recorded here; a key
-- that reappears in Spark is removed again. On 2026-09-25 three were found:
-- 15714 Tumble Weed Turn (Sisters), 18581 Couch Market (Bend) and 717 Larch
-- (Redmond), each missing from Spark by listing key, by MLS number and from
-- the full list of that town's closings for the month.
--
-- refresh_market_fact_sale (20260822233000) marks such a sale
-- 'absent_from_mls' and unpublishable; the monthly report and the Market
-- Truth metrics read only publishable sales. In the duplicate-parcel ranking a
-- sale still in the MLS now wins over one that is not, so a removed entry can
-- never push its live twin out as the duplicate.

CREATE TABLE IF NOT EXISTS public.market_listing_absent_from_mls (
...
COMMENT ON TABLE public.market_listing_absent_from_mls IS
  'Closed sales we hold that the MLS (Spark) no longer serves at all, found by the daily closings reconciliation by listing key. Excluded from Market Truth sale facts as absent_from_mls (Matt 2026-09-25).';

ALTER TABLE public.market_listing_absent_from_mls ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.market_listing_absent_from_mls FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.market_listing_absent_from_mls TO service_role;
...
BEGIN
  ...
  WITH src AS (
    SELECT
      ...
      (ab.listing_key IS NOT NULL) AS absent_from_mls
    FROM public.listings l
    LEFT JOIN public.market_service_area sa
      ON lower(sa.city_proper) = lower(btrim(coalesce(l."City", '')))
    LEFT JOIN public.market_listing_absent_from_mls ab
      ON ab.listing_key = l."ListingKey"
  ...
  ranked AS (
    SELECT
      ...
      row_number() OVER (
        ...
        ORDER BY s.absent_from_mls ASC, s.source_updated_at DESC NULLS LAST, s.listing_key DESC
      ) AS dup_rn
    FROM src s
  ),
  stamped AS (
    SELECT
      ...
      ARRAY_REMOVE(ARRAY[
        ...
        CASE WHEN r.absent_from_mls THEN 'absent_from_mls' END
