-- 20260712010000_listing_search_mv_private_from_private_table.sql
--
-- Repoint listing_search_mv.private_remarks to read from listing_private (the
-- service-role-only table) now that the confidential keys are stripped from
-- listings.details (migration 20260712000000). Without this the next MV refresh
-- would produce NULL private_remarks and break the admin remarks search. The
-- column-level grant is preserved so anon/authenticated still cannot select it.
--
-- PROD APPLY NOTE (2026-07-12): applied to hosted dwvlophlbvvygjfxcrhm.
...
SELECT
  ...
  -- Sourced from the service-role-only listing_private (details no longer
  -- carries it — attack finding 2026-07-11). Still column-granted below so
  -- anon/authenticated cannot select it.
  nullif(btrim(lp.private_data->>'PrivateRemarks'), '') AS private_remarks,
