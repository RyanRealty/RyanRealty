...
export default function BrokerBio({ broker }: Props) {
  ...
  return (
    <section className="bg-[var(--brand-cream)] px-4 py-12 sm:px-6 sm:py-16" aria-labelledby="broker-bio-heading">
      <div className="mx-auto max-w-7xl">
        <h2 id="broker-bio-heading" className="text-2xl font-bold tracking-tight text-[var(--brand-navy)]">
          About {firstName}
        </h2>
        {bio ? (
          <div className="mt-4">
            <div className="prose prose-[var(--brand-navy)] max-w-none text-[var(--text-secondary)]">
        ...
        {(broker.specialties ?? []).filter((s): s is string => Boolean(s?.trim())).length > 0 && (
          <div className="mt-6">
            <h3 className="text-sm font-semibold text-[var(--brand-navy)]">Specialties</h3>
            <div className="mt-2 flex flex-wrap gap-2">
              {(broker.specialties ?? []).filter((s): s is string => Boolean(s?.trim())).map((s) => (
                <span
                  key={s}
                  className="rounded-full bg-[var(--gray-bg)] px-3 py-1 text-sm text-[var(--text-secondary)]"
                >
                  {s.trim()}
                </span>
              ))}
