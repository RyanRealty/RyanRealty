...
function cardPriceLabel(price: number | null): string {
  const published = publishListingAsk(price)
  return published ? formatPrice(published.ask) : 'Price on request'
}

export default function VideoListingCard({ listing }: { listing: ListingCardData }) {
  const [playing, setPlaying] = useState(false)
  const priceLabel = cardPriceLabel(listing.price)
    ...
    <>
      <div className="relative aspect-[4/3] bg-muted">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="relative flex h-16 w-16 items-center justify-center rounded-full bg-background/95 text-primary shadow-lg transition group-hover:scale-105 group-hover:bg-background">
        ...
        <div className="absolute top-2.5 left-2.5">
          <span className="inline-flex items-center rounded-full border border-border bg-background/95 px-2.5 py-0.5 text-[11px] font-medium text-primary">
  ...
  if (!embeddable) {
    ...
    return (
      <Link
        ...
        aria-label={`View ${listing.addressLine}, ${priceLabel} (opens the listing with its video tour)`}
      >
        {poster}
      </Link>
    )
  ...
  return (
    <button
      ...
      aria-label={`Play the video tour of ${listing.addressLine}, ${priceLabel}`}
    >
      {poster}
    </button>
  )
