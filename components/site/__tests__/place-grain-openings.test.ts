...
describe('master-plan opening', () => {
  ...
  it('puts belonging on the still as a caption, then Atlas, then one sold chart', () => {
    ...
    expect(page).toMatch(/chartFirst/)
    expect(page).toMatch(/foldAfter=\{0\}/)
    expect(page.indexOf('place-opening__caption')).toBeLessThan(page.indexOf('<PlaceSubdivisionAtlas'))
    expect(page.indexOf('<PlaceSubdivisionAtlas')).toBeLessThan(page.indexOf('id="market"'))
