 ...
 * Data: getCoMarketAnnual / series via parent. §0 methodology footer required.
 */
...
type Props = {
  /** Newest year first or chronological — parent decides order */
  series: CoMarketAnnualRow[]
  /** Highlight year (default: max year in series) */
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
        <p className="text-xs font-medium uppercase tracking-widest text-[var(--kb-ink-muted,#5c5c5c)]">
          Closed sales · Central Oregon service area
        </p>
        <h2
          id="co-market-size-heading"
          className="mt-2 font-[family-name:var(--font-amboqia,serif)] text-3xl text-[var(--kb-navy,#0a1628)] md:text-4xl"
        >
          Size of the market
        </h2>
        <p className="mt-3 max-w-2xl text-base text-[var(--kb-ink,#1a1a1a)]">
          ...
          {sorted.map((row) => {
            ...
            return (
              <div
                ...
                className={
                  isHi
                    ? 'border-2 border-[var(--kb-navy,#0a1628)] bg-[var(--kb-cream,#f7f3eb)] p-4'
                    : 'border border-[var(--kb-border,#d9d2c5)] bg-white p-4'
                }
              >
                <div className="text-xs uppercase tracking-wide text-[var(--kb-ink-muted,#5c5c5c)]">
                  {row.year}
                  {isHi ? ' · latest full year' : ''}
                </div>
                <div className="mt-1 text-2xl font-semibold tabular-nums text-[var(--kb-navy,#0a1628)]">
                  {money(row.totalVolume)}
                </div>
                <div className="mt-1 text-sm tabular-nums text-[var(--kb-ink,#1a1a1a)]">
                  {row.soldCount.toLocaleString('en-US')} closes
                </div>
                <div className="mt-0.5 text-xs text-[var(--kb-ink-muted,#5c5c5c)]">
        ...
        <p className="mt-6 text-xs leading-relaxed text-[var(--kb-ink-muted,#5c5c5c)]">
