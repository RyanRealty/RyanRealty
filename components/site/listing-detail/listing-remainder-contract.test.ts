...
describe('listing remainder composition', () => {
  ...
  it('keeps listing cards on one line in equal tracks instead of wrapping', () => {
    const CLOSE = readFileSync(resolve('components/site/v3/V3ListingClose.client.tsx'), 'utf8')
    const CLOSE_CSS = readFileSync(resolve('components/site/v3/V3ListingClose.css'), 'utf8')
    const CSS = readFileSync(resolve('components/site/listing-detail/listing-detail.css'), 'utf8')
    const STRIP = readFileSync(resolve('components/site/listing-detail/PriceCtaStrip.tsx'), 'utf8')
    expect(CLOSE).toContain("label: 'Watch this price'")
    expect(CLOSE).toContain("label: 'Walk through it'")
    expect(CLOSE).toContain("label: 'Email this payment'")
    expect(CLOSE).not.toContain('Tell me if this price drops')
    expect(CLOSE_CSS).toMatch(/\.v3-close__tabs[\s\S]*align-items:\s*stretch/)
    expect(CLOSE_CSS).toMatch(/grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)/)
    expect(CLOSE_CSS).toMatch(/\.v3-close__tab-label[\s\S]*white-space:\s*nowrap/)
    expect(CLOSE_CSS).toMatch(/\.v3-close__tab-hint[\s\S]*white-space:\s*nowrap/)
    expect(CSS).toMatch(/\.listing-near__list[\s\S]*align-items:\s*stretch/)
    expect(CSS).toMatch(/\.listing-near__name[\s\S]*white-space:\s*nowrap/)
    expect(CSS).toMatch(/\.v3-figure__label[\s\S]*white-space:\s*nowrap/)
