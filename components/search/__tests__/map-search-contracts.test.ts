...
describe('390 Map uses one camera', () => {
  ...
  it('search chrome: mic in bar, Map|Sort floating pill, school district Places grain', () => {
    ...
    expect(chrome).toMatch(/v3-chrome__panel--mega/)
    expect(chrome).toMatch(/chromeMegaModel/)
    expect(chrome).not.toMatch(/placesMegaSections/)
    expect(chrome).not.toMatch(/v3-chrome__panel--places/)
    const resolver = readSrc('lib/search/resolve-search-place-boundary.ts')
