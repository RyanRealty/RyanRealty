...
describe('SITE-121 rematch craft', () => {
  ...
  it('phone header morph stays typeable and catalog-imported', () => {
    ...
    expect(readSrc('components/site/v3/V3MorphSearch.css')).not.toMatch(/v3-morph-search__catalog \{\s*width: 100%/)
    expect(readSrc('components/site/v3/V3Chrome.css')).toContain('rr-chrome-search-open')
    expect(readSrc('components/motion/morphing-search.tsx')).toContain('isPhoneMorphViewport')
    expect(readSrc('components/motion/morphing-search.tsx')).toContain('catalogPanelWidth')
  })
})
