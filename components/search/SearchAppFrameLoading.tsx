...
/**
 * Streaming fallback for the map/split search app-frame (SITE-158 / SITE-155).
 *
 * Shared by `app/search/loading.tsx` and `app/search/[...slug]/loading.tsx`
 * (publicly `/homes-for-sale*`). The slug route used to model a ~1,900px
 * hero + grid page this app no longer renders. That stale shell was about
 * 2× the live ~910px split pane, so swapping it for real content shrank
 * the document out from under a mid-load scroll. One component keeps the
 * two routes from drifting apart again.
 */
