...
export function polygonInteriorPoint(rings: readonly Ring[]): LonLat | null {
  ...
  for (let round = 0; round < ROUNDS; round += 1) {
    const spanLon = window.maxLon - window.minLon
    const spanLat = window.maxLat - window.minLat
    if (spanLon <= 0 || spanLat <= 0) break
    let roundBest: LonLat | null = null
    let roundBestD = -Infinity
    for (let ix = 0; ix < GRID; ix += 1) {
      for (let iy = 0; iy < GRID; iy += 1) {
        const lon = window.minLon + ((ix + 0.5) / GRID) * spanLon
        const lat = window.minLat + ((iy + 0.5) / GRID) * spanLat
    ...
    // Zoom the window into the best cell plus a one-cell margin, so the next
