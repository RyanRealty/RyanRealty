...
describe('buildHouseMeRows', () => {
  ...
  it('omits True cost when HOA and tax are absent', () => {
    ...
    const foley = buildHouseMeRows(emptyFacts({ hoaMonthly: 22, taxAnnualAmount: 13168 }))
    expect(foley.find((r) => r.id === 'true-cost')?.value).toBe('HOA $22 per month. Tax $13,168 per year.')
    const canyons = buildHouseMeRows(emptyFacts({ hoaMonthly: 1529, associationFee: 2000 }))
    expect(canyons.find((r) => r.id === 'true-cost')?.value).toBe('HOA $1,529 per month.')
