...
describe('pricingTierLadder — time before distance', () => {
  it('walks 3 then 6 then 9 months inside the subdivision before any mile ring', () => {
    ...
    expect(names.indexOf('subdivision-9mo-wide')).toBeLessThan(names.indexOf('neighborhood-3mo'))
    expect(names.indexOf('neighborhood-9mo')).toBeLessThan(names.indexOf('nearby-0.5mi-3mo'))
    expect(names.indexOf('nearby-0.5mi-9mo')).toBeLessThan(names.indexOf('nearby-1mi-3mo'))
