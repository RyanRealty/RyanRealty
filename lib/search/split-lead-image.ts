...
/**
 * The image the split page leads with: the first card's first photo, run
 * through the same `listingRowPhotoSrc` the card itself renders, so
...
export function splitLeadImageUrl(listings: readonly LeadRow[]): string | null {
  ...
  return listingRowPhotoSrc(raw) || null
}
