  ...
  try {
    ...
    const priceSet = (set: typeof selection.comps) => {
      ...
      if (p && exclusivePocket) {
        p.notes.unshift(
          `These sales are the exclusive pocket. Date adjustment does not walk the ${subject.city} city index — that series includes tracts already excluded from this set. Each sale stays on its sold and last-ask price, then size — story class does not adjust.`,
        )
      } else if (p && usePath) {
        p.notes.unshift(
