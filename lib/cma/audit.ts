 ...
 *  - v4 (2026-09-07): a `critical` in ANY category now forces `review`,
 *    because v2's price-opinion exemption argued about MAJORS and swallowed
 *    criticals with it — 66 of 271 audit-passed rows sat in the sendable
 *    `ready` lane carrying a critical such as "every priced sale was excluded
 *    and none were kept, yet a Supportable $961k opinion is still issued"
 *    (cma-1235-hartford), which §0 forbids shipping unread. Majors are
 *    untouched: a price-level disagreement between two models is still broker
 *    judgment, so the gate is a human read, never `fail`.
 */
export function computeAuditVerdict(findings: AuditFinding[]): AuditVerdict {
  ...
  // v4 (2026-09-07): `critical` means the auditor says the recommendation is
  // wrong or indefensible. That is a broker read in every category — including
  // price-opinion and other, whose MAJORS stay advisory below.
  if (findings.some((f) => f.severity === 'critical')) return 'review'
  ...
  // (reflex), a lone narrative-major rewording, every price-opinion MAJOR
...
export async function auditCma(args: {
  ...
  const siteLine = site
    ? `Zoning ${site.zone ?? 'UNRESOLVED'}${site.zoneOverlays.length ? ` (overlays ${site.zoneOverlays.join(', ')})` : ''}. ` +
      ...
      `Water: ${site.water.source}${site.water.wellLog ? ` (nearest domestic well ${site.water.wellLog.completedDepthFt ?? '?'}ft)` : ''}${site.water.irrigationDistrict ? ` [district ${site.water.irrigationDistrict}]` : ''}. ` +
      `${site.water.rights.length ? `OWRD supply water rights of record (MAPPED, not validated; instream/non-consumptive already excluded): ${site.water.rights.map((r) => `${r.use}${r.acres != null ? ` ${r.acres}ac` : ''}${r.priorityDate ? ` pri ${r.priorityDate}` : ''}${r.supplemental ? ' suppl' : ''} [${r.stage}, ${r.holderType}${r.holder ? ` ${r.holder}` : ''}]`).join(' | ')}. ${site.water.hasPrivateAppurtenant ? '' : 'ALL rights are supplier-held (city/district), NOT private appurtenant. '}${site.water.mappedIrrigationAcres != null ? `Perfected primary irrigation ~${site.water.mappedIrrigationAcres}ac (largest single right, NOT a sum)${site.water.primaryIrrigationPriorityDate ? ` pri ${site.water.primaryIrrigationPriorityDate}` : ''}. ` : ''}` : `No mapped OWRD supply right on the parcel${site.water.rightsQueryOk ? '' : ' (QUERY DID NOT COMPLETE — do not assert dryness)'}. `}` +
