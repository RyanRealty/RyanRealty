...
describe('print CMA price-opinion spine', () => {
  ...
  it('renders conditional chapters when extras and sellerNet are provided', () => {
      ...
      args({
        ...
        pricing: {
          ...
          sellerNet: {
            ...
            net: 457000,
            sentence:
              'From a $465,000 list, less $8,000 in seller concessions, $457,000 remains. This figure does not include the listing and buyer-broker commission, title insurance, escrow and closing fees, recording and transfer fees and your loan payoff.',
    ...
    // Taxlot, flood zone and the permit list went with Home location and
    // Permits: a homeowner reading a pricing document on a phone never asked.
    expect(html).not.toContain('151000000')
    expect(html).not.toContain('Zone X. Not a Special Flood Hazard Area.')
    expect(html).not.toContain('<h2 class="section">Photos</h2>')
    ...
    expect(nextAt).toBeGreaterThan(discAt)
  })
})
