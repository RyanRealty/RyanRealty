...
describe('print CMA price-opinion spine', () => {
  it('always renders cover numbers, sales/map/matrix, demoted snapshot, disclosure', () => {
    ...
    const snapAt = html.indexOf('<h2 class="section">Home location</h2>')
    ...
    expect(snapAt).toBeGreaterThan(salesAt)
    expect(discAt).toBeGreaterThan(snapAt)
  })
  ...
  it('renders conditional chapters when extras and sellerNet are provided', () => {
    ...
    expect(houseAt).toBeGreaterThan(priceAt)
    expect(legalAt).toBeGreaterThan(houseAt)
    expect(permitsAt).toBeGreaterThan(houseAt)
