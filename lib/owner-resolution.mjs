...
export async function batchDataSkipTrace(params) {
  ...
  try {
    ...
    // The skip-trace auto-determines the REAL owner even when the county record
    // is an entity (LLC / trust). Capture that human name so entity-owned rows
    // resolve to an actual person, not "SOME LLC". The response `name` may be an
    // object ({first,last,full}) or a flat string; fall back to `owner`.
    const nm = person.name
    const nameFirst = nm && typeof nm === 'object' ? (nm.first ?? null) : null
    const nameLast = nm && typeof nm === 'object' ? (nm.last ?? null) : null
    const nameFull =
      (nm && typeof nm === 'object' ? (nm.full ?? null) : typeof nm === 'string' ? nm : null) ??
      (typeof person.owner === 'string' ? person.owner : null)
    const personName =
      nameFirst || nameLast || nameFull ? { first: nameFirst, last: nameLast, full: nameFull } : null
    // Full skip-trace intelligence — same field paths the westside farm enrichment
    // reads off the persons[] entry. Kept structured; the persist layer maps to
    // namespaced custom fields on the CRM contact.
    const d = person.demographics || {}
    const f = person.flags || {}
    const dobRaw = d.birthDate || d.dateOfBirth || d.dob || d.birthday || null
    const demographics = {
      age: d.age ?? null,
      ...
      presenceOfChildren: d.presenceOfChildren ?? null,
      occupation: d.occupation ?? null,
      income: d.income ?? null,
      netWorth: d.netWorth ?? null,
    }
    const hasDemographics = Object.values(demographics).some((v) => v != null && v !== '')
    const flags = {
      recentlyMoved: !!f.recentlyMoved,
      recentlyDivorced: !!f.recentlyDivorced,
      equityRich: !!f.equityRich,
      vacant: !!f.vacant,
    }
    return {
      phones,
      ...
      dncTcpa,
      deceased,
      hardStop: litigator || dncTcpa || deceased,
      person: personName,
