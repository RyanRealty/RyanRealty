...
describe('atlas taxlots URL (Matt 2026-09-23: lots inside a selected district)', () => {
  ...
  it('refuses a slug outside the boundaries shape, so a public URL cannot mint cache keys', () => {
    expect(parseAtlasTaxlotsScope(new URLSearchParams('s=Bend'))).toBeNull()
    expect(parseAtlasTaxlotsScope(new URLSearchParams('s=bend%20awbrey'))).toBeNull()
    expect(parseAtlasTaxlotsScope(new URLSearchParams('s=bend%27%3B--'))).toBeNull()
    expect(parseAtlasTaxlotsScope(new URLSearchParams(`s=${'a'.repeat(121)}`))).toBeNull()
    expect(atlasTaxlotsHref({ geoType: null, geoSlug: '../../etc' })).toBeNull()
    expect(parseAtlasTaxlotsScope(new URLSearchParams('s=ridge-at-eagle-crest-36'))).toEqual({
      geoType: null,
      geoSlug: 'ridge-at-eagle-crest-36',
    })
  })
})
