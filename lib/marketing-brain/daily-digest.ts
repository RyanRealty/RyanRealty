...
/**
 * Inbox-sourced subset of the queue. Brokers email marketing@ and the
 * resulting action rows get tagged `data_evidence.audit_source='marketing-inbox'`.
 * The digest surfaces these separately so Matt sees "what brokers are
 * waiting on" without having to filter the broader queue.
 */
export interface InboxSummary {
  pending_total: number
  pending_by_sender: Record<string, number>
  pending_by_action_type: Record<string, number>
  oldest_pending_at: string | null
  inbox_events_24h: number
  inbox_killed_24h: number
}
...
export async function gatherDigestStats(): Promise<DigestStats> {
  ...
  // Pull inbox-sourced pending rows (audit_source = 'marketing-inbox')
  // and recent inbox event activity for the broker-visible portion of
  // the digest.
  const [inboxPendingRes, inboxEvents24hRes, inboxKilled24hRes] = await Promise.all([
    supabase
      .from('marketing_brain_actions')
      .select('id, action_type, target, created_at, payload, data_evidence')
      .eq('status', 'pending')
      .filter('data_evidence->>audit_source', 'eq', 'marketing-inbox')
  ...
  const inboxPending = (inboxPendingRes.data ?? []) as Array<{
    id: string
    action_type: string
    target: string
    created_at: string
  ...
  const pendingBySender: Record<string, number> = {}
  const pendingByActionType: Record<string, number> = {}
  for (const row of inboxPending) {
    const sender = String((row.payload as { sender_email?: string } | null)?.sender_email ?? 'unknown')
    pendingBySender[sender] = (pendingBySender[sender] ?? 0) + 1
    pendingByActionType[row.action_type] = (pendingByActionType[row.action_type] ?? 0) + 1
  }

  const inboxSummary: InboxSummary = {
    pending_total: inboxPending.length,
    pending_by_sender: pendingBySender,
    pending_by_action_type: pendingByActionType,
    oldest_pending_at: inboxPending.length > 0 ? inboxPending[0].created_at : null,
    inbox_events_24h: inboxEvents24hRes.count ?? 0,
    inbox_killed_24h: inboxKilled24hRes.count ?? 0,
  }
...
export function composeMarkdownDigest(asOfDate: string, stats: DigestStats): string {
  ...
  // Inbox section — what brokers are waiting on right now.
  const inbox = stats.inbox
  if (
    inbox.pending_total > 0 ||
    inbox.inbox_events_24h > 0 ||
    inbox.inbox_killed_24h > 0
  ) {
    lines.push(`## Brokers are waiting on you`)
    lines.push(``)
    lines.push(`- Inbox emails (last 24h): **${inbox.inbox_events_24h}**`)
    if (inbox.inbox_killed_24h > 0) {
      lines.push(`  - Rejected (not-allowlisted senders): ${inbox.inbox_killed_24h}`)
    }
    lines.push(`- Broker requests still pending: **${inbox.pending_total}**`)
    if (inbox.oldest_pending_at) {
      const age = Math.floor((Date.now() - Date.parse(inbox.oldest_pending_at)) / 3_600_000)
      lines.push(`  - Oldest waiting: ${age}h ago`)
    }
    const senders = Object.entries(inbox.pending_by_sender).sort((a, b) => b[1] - a[1]).slice(0, 4)
    if (senders.length > 0) {
    ...
    const types = Object.entries(inbox.pending_by_action_type).sort((a, b) => b[1] - a[1]).slice(0, 6)
    if (types.length > 0) {
    ...
    lines.push(``)
    lines.push(`Audit them at /dashboard/marketing/inbox.`)
    lines.push(``)
  }
...
export function composeShortDigest(asOfDate: string, stats: DigestStats): string {
  ...
  const parts = [
    ...
    stats.inbox.pending_total > 0 ? `${stats.inbox.pending_total} broker req` : null,
