 ...
 * MLS plat name. Powers the sales-history table on /subdivisions/[slug].
 *
 * Population: slugify_geo("SubdivisionName") = p_slug, PropertyType A
 * (single-family name join). This is not recorded-plat membership
 * (place_membership is_primary / getSubdivisionCounts). GIS slug and MLS
 * name often diverge. Public copy must not call these yearly counts a
 * recorded plat. REGISTRY §4: counts only on the public grain. The RPC
 * still returns median_close_price. publishSubdivisionClosedPrice withholds it.
 *
 * ODS §5-4 A.4: sold LISTING data is VOW-only. This function returns only
 * per-year aggregates (year, closed count, median close price). Do not
 ...
 * No raw listings aggregation happens in app code. Do not rewrite the RPC.
 *
...
export type SubdivisionSalesYear = {
  ...
  year: number
  /**
   * Closed single-family count for that year. MLS SubdivisionName slug
   * match, not recorded-plat membership.
   */
  closedCount: number
  /**
   * Median close price across those sales. Public plat surfaces withhold
   * this through publishSubdivisionClosedPrice.
   */
