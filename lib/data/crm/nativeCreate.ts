...
import { JOURNEY_ENTRY_STAGE } from '@/lib/crm/journey-advance'
 ...
 * assigned broker, a `source:<x>` tag, and (downstream) a normalized
 * crm_contact_points row — or a native lead lands incomplete and invisible to
 * the workflow. This single builder is the chokepoint that keeps the two paths
 * from drifting: both call it for the crm_people insert payload, so a field
 * added here lands on every native-create path at once.
 *
...
export type NativePersonRow = {
  ...
  stage: typeof JOURNEY_ENTRY_STAGE
  source: string
 ...
 *   - `stage` is always 'Lead' (inbound entry; named trigger: native-create).
...
export function nativeCreateGaps(row: NativePersonRow): string[] {
  ...
  if (row.stage !== JOURNEY_ENTRY_STAGE) gaps.push('stage is not Lead')
