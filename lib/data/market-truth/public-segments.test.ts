...
describe('public place pages', () => {
  it('city and housing-market city pages read getPublicPlaceSegments', () => {
    ...
    const hubSections = readFileSync(resolve('app/housing-market/_v3/hub-sections.ts'), 'utf8')
    expect(hubSections).toMatch(/publicSegmentDisplayBits/)
    ...
    expect(searchTail).not.toMatch(/publicSegmentBrowseHref/)
    expect(searchTail).not.toMatch(/leftover and other types/)
