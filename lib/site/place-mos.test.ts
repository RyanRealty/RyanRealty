...
describe('buildPlaceMosView', () => {
  it('hands the primitive preformatted bars, a caption that is not a MOS tile, and a tooltip with both counts plus the source line', () => {
    ...
    expect(view!.tooltip.source).toContain('public.market_metric')
    expect(view!.tooltip.source).toContain('city:bend')
    expect(view!.tooltip.source).toContain('segment=detached')
    expect(view!.tooltip.source).toContain(MOS_METHODOLOGY_CLAUSE)
    expect(view!.tooltip.source).toContain(MOS_THRESHOLD_CLAUSE)
    expect(view!.tooltip.source).toContain('updated Sep 9, 2026')
    expect(view!.source).toBe(view!.tooltip.source)
  })
