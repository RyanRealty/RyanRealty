...
describe('buildPlaceMosView', () => {
  it('hands the primitive preformatted bars, a caption that is not a MOS tile, and a tooltip with both counts plus the source line', () => {
    ...
    expect(view!.source).toContain('as of Sep 9, 2026')
    expect(view!.source).not.toContain('leftoverHudKpis')
    expect(view!.source).not.toContain('market_metric')
    expect(view!.source).not.toContain('public.market_metric')
  })
