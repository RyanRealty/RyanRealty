...
export function buildPresetFaq(
  ...
  if (cityPulse?.medianListPrice != null && cityPulse.medianListPrice > 0) {
    faqs.push({
      ...
      answer: `Across all of ${city}, the median list price for a single-family home is ${roundedThousand(cityPulse.medianListPrice)}${asOf}, based on ${cityPulse.source === 'market-truth' ? 'a direct count of the active MLS listings' : 'live MLS data'}. That figure covers the whole city, not just the listings in this search.`,
    })
  }
