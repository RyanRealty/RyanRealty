...
description: >
  ...
  IG caption, static card, and optional Reel hook. Sources live verified data from the
  getPriceDropDigest DAL function. Never invents numbers.
action_types:
  - content:price_drop_digest
output_type: caption+json
target_platforms:
  - ig_feed
  - fb_feed
  - li_feed
asset_destination: out/price-drop-digest/<city>/<date>/
auto_inputs:
  - getPriceDropDigest DAL (live from Supabase)
required_inputs:
  - action_row with action_type=content:price_drop_digest
optional_inputs:
  - city slug (omit for region-wide)
  - days window (default 14)
  - surface (ig|fb|linkedin, default ig)
estimated_runtime_min: 1
cost_usd_estimate: 0.02
thumbnail_uri: ""
example_outputs:
  - out/price-drop-digest/bend/2026-06-09/caption.txt
...
Every run of this producer MUST load all of the following before generating any output.

### Tier 1 - Mandatory for all producers
