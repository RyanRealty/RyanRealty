...
4.5. **Comprehensive property & site intelligence (`lib/cma/county.ts`, `resolveCmaSiteData`): GIS-authoritative, never LLM-inferred (§0, expanded 2026-07-14).** Every build resolves, from the source of record, the parcel's full authoritative fact set: **base zoning + every overlay** (Deschutes LandFD `ZONE` + discrete overlay layers.  LM road/water 6/7, Wildlife Area 10, Airport Safety 4, Surface Mining 8, Wildfire 9), **taxlot + Township/Range/Section + acreage**, **FEMA flood zone + Special Flood Hazard Area flag** (FEMA National Flood Hazard Layer.  the county's own flood/slope services are now empty), **water** (nearest OWRD domestic well log + irrigation-district membership, with the explicit caveat that a district boundary is NOT a certificated water right), **septic + a permit-history digest** (Deschutes DIAL), the **dwelling-entitlement pathway** for resource zones, **buildability constraints** (flood, by-right vs conditional, overlay setbacks.  WA ~300 ft deer-range, LM 100 ft scenic), **hunting / landowner-preference eligibility** for qualifying acreage (40ac antlerless deer+elk, 160ac buck/bull, game unit + No Shooting District status, caveated), and municipal-vs-private / inside-UGB / public-land determinations. Slope, wetland, and canal layers were retired by the county, so they surface as explicit "confirm at listing" caveats.  never fabricated. Pure `fetch` against public GIS/records.  no model touches these facts (CLAUDE.md §0 forbids LLM-inferring zoning/well/flood). Fail-open: any source that errors leaves its field unknown with a note. Recorded in `build_summary.site` (admin summary) + `citations.site` (full trace), rendered as the "Property & site intelligence" + "Buildability & constraints" + "Hunting, recreation & landowner tags" + "Confirm at listing" blocks on the subject page via the shared `propertyIntelligenceBlock` renderer (used verbatim by the BPO), and fed to the adversarial auditor so it can attack any buildability / flood / hunting / water-right overclaim. **The judge + audit reason OVER this verified data; they never generate it.** BPO parity: `lib/bpo/build.ts` resolves the identical intelligence, renders the same block, gates on the same site checks, and records it in `citations.site` + `build_summary.site`.
4.8. **Development potential + verify-it-yourself directory (`lib/cma/development.ts`, added 2026-07-14).** Every CMA/BPO answers the questions owners actually ask: can I subdivide, build an ADU, add a second dwelling, do middle housing (STR joins when its research pass verifies)? Zone-keyed registry covering Bend, Redmond, and unincorporated Deschutes County. **Every rule was researched against its PRIMARY code source and independently re-verified (58-fact adversarial pass, zero rejected); each rendered item carries its code citation + hyperlinked source.** Pure function over the verified zone + acreage — no LLM, no new fetches. Renders as two CMA pages (Development Potential + Verify It Yourself) and inline on the BPO. The Verify It Yourself page ALWAYS carries the standing disclaimer (we research what we can against the code in effect; verification is ultimately the reader's responsibility) + the hyperlinked agency directory (Deschutes CDD + DIAL, Bend Planning + STR program, Redmond Planning, OWRD, FEMA, ODFW LOP, the parcel's irrigation district). **Rule for agents: never edit a registry rule without re-verifying it against the cited primary source, and update `REGS_VERIFIED_DATE` when you do.**
