...
2. `lib/crm/send-event.ts`. Archived LP voice research. Not a build target. Live capture is `sendEvent` → `crm_people`.
...
- `lib/crm/send-event.ts`. Archived LP voice research. Not a build target.
