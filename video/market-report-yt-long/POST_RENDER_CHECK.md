...
```bash
python3 scripts/check_first_frame.py video/market-report-yt-long/out/<render>.mp4
```
...
```bash
cd /Users/matthewryan/RyanRealty/video/market-report-yt-long
npx remotion render src/index.ts YouTubeMarketReport out/<slug>.mp4 \
  --codec h264 --concurrency 1 --crf 22 \
  --image-format=jpeg --jpeg-quality=92 \
  --props=out/<slug>-props.json
```

Then from repo root:

```bash
python3 scripts/check_first_frame.py video/market-report-yt-long/out/<slug>.mp4
```
