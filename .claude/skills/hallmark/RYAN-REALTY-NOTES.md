...
1. **Pin the theme. Do not rotate.** Ignore Step 1/2.6 theme selection and the 20-theme catalog and the custom-OKLCH branch. The theme is always the Ryan Realty brand: navy `#102742` on cream `#faf8f4`, **Amboqia Boriango** display + **Geist** body, tabular numerals, no emoji. Source of truth: for PUBLIC SITE surfaces it is `components/site/v3/tokens.css`, applied as `bg-navy` / `text-cream`. For print, social and admin it is `design_system/ryan-realty/SKILL.md`. Structural primitives in the v3 barrel may be added; colour tokens may not be minted.
