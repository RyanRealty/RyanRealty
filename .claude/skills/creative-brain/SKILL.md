...
1. **Reference-conditioned place work is in. Prompt-only slop and fake listings are out.** (Matt 2026-08-14: the 2026-07-09 Reality Law is retired. It was prohibitive and we are better at this now.) AI may render real Central Oregon places, landmarks, terrain, and lifestyle chrome when the generation is conditioned on a real local still (owned, licensed, or Wikimedia) and a local would name the place. Prompt-only "scenic Bend" without a reference pack is refuse. Still refuse: inventing a specific listing, room, or view-from-this-address; generating people as "who lives here"; baking charts, rates, or type into a generated frame; passing a generated frame as MLS evidence of a property. Stylized authored worlds remain a valid lane. They are not the only lane.
2. **Do not regenerate licensed footage for no reason.** Check provenance first (we once animated frame 1 of a stock video). Filmable is not a ban. If a reference still exists, a new crop, season, or aspect is allowed. If we already own the motion, use it.
...
- **The reference test** — place chrome needs a real local still on disk. Prompt-only scenic is dead. A listing, room, or view-from-this-address is camera or MLS.
