...
2. **Draft-first.** The audit itself is `analyze:` — internal, no approval. But every public-facing OUTPUT (description rewrite, post, review reply, Q&A answer, services text, attribute change, category change) is a draft: written in the VOICE.md voice, then Matt approves, then it publishes through the pipeline below. Nothing is written to GBP or any directory without Matt's explicit go.
...
5. **Voice.** `marketing_brain_skills/brand-voice/VOICE.md` governs all public copy. Review replies are Matt's 1:1 correspondence voice — the canonical phrases ("a small business like ours," "genuinely a pleasure") are allowed THERE and nowhere else. Corpus: `marketing_brain_skills/brand-voice/corpus/gbp_responses.md`.
