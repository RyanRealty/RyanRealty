-- Market Truth — neighborhood calendar-month leftover cells (window_months = 1).
-- Separate from compute_market_metrics_monthly_shadow so a timeout cannot
-- DELETE city/region monthly leftover. median_close + closed_count, detached
-- only, sample-gated min_n. Does not write MoM rates, MOS, or prices on
-- subdivision. County stays out. commercial_lease stays out.

CREATE OR REPLACE FUNCTION public.compute_market_metrics_monthly_neighborhood_shadow(
...
REVOKE ALL ON FUNCTION public.compute_market_metrics_monthly_neighborhood_shadow(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.compute_market_metrics_monthly_neighborhood_shadow(integer) TO service_role;
