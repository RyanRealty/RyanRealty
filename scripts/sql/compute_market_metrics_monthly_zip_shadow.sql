-- Market Truth — zip calendar-month leftover cells (window_months = 1).
-- Separate from city/region monthly leftover so a timeout cannot DELETE those
-- cells. Canonical 10 public zips only (same allowlist as zip leftover compute).
-- Zip membership is place_membership geo_type zip (PostalCode), is_primary.
-- Detached median_close + closed_count, sample-gated min_n. No MoM. No county.
-- commercial_lease stays out.

CREATE OR REPLACE FUNCTION public.compute_market_metrics_monthly_zip_shadow(
...
BEGIN
  ...
  geos AS (
    SELECT 'zip'::text AS geo_type, x.geo_slug
    FROM unnest(ARRAY[
...
REVOKE ALL ON FUNCTION public.compute_market_metrics_monthly_zip_shadow(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.compute_market_metrics_monthly_zip_shadow(integer) TO service_role;
