/**
 * Complete the Southern Crossing index-1 vs place-23 fleet finding.
 *
 *   npx tsx scripts/loop-complete-southern-crossing-count.ts <evidence>
 */
...
const OWNER = 'cursor-loop-chain-2026-08-16t23-20-2fa9'
const COLLIDING_OWNER = 'cursor-loop-chain-2026-08-16t23-20-dd33'
const NODE_ID = '08704f1f-2ccd-46dd-86c7-a3ac65bdf073'
const FLEET = '8a95d715ef63989d964b9d643d2938f4'

async function main() {
  const evidence = process.argv.slice(2).join(' ').trim()
  if (!evidence) {
    console.error('usage: npx tsx scripts/loop-complete-southern-crossing-count.ts <evidence>')
    process.exit(2)
  }
  ...
  if (row.state === 'done') {
    ...
    if (
      row.owner_session &&
      row.owner_session !== OWNER &&
      row.owner_session !== COLLIDING_OWNER
    ) {
