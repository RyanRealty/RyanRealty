 ...
 *
 * Fans out to 8 sub-producers in parallel: flyer + sign + reel + tour + postcard + eflyer.
 */
...
<head>
  ...
  <title>Listing launch — ${listing.street_number ?? ''} ${listing.street_name ?? targetSlug}</title>
  <style>
...
<body>
  <h1>At-Active listing launch</h1>
  <p class="meta">${listing.street_number ?? ''} ${listing.street_name ?? ''}, ${listing.city ?? ''} &middot; ${listing.list_price_display ?? ''} &middot; ${SUB_PRODUCERS.length} deliverables &middot; generated ${new Date().toISOString()}</p>
  <table>
    <thead>
...
${rows}
  ...
  await writeFile(join(outDir, 'card.json'), JSON.stringify({
    ...
    notes: 'At-Active launch suite — flyer + sign + reel + tour + postcard + eflyer.',
    data_traces: [],
    generated_at: new Date().toISOString(),
  }, null, 2))
