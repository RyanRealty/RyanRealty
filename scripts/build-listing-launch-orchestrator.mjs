#!/usr/bin/env node
/**
 * build-listing-launch-orchestrator.mjs — At-Active listing launch suite
 *
 ...
 *
 * Usage:
 *   node scripts/build-listing-launch-orchestrator.mjs <payload.json> [--out <dir>]
 */
...
const PRODUCER = 'listing_launch_orchestrator'
...
async function main() {
  ...
  if (!payloadPath) {
    console.error('Usage: node scripts/build-listing-launch-orchestrator.mjs <payload.json> [--out <dir>]')
    process.exit(2)
  }
  ...
  const priceDisplay = listing.list_price_display ? ` listed at ${listing.list_price_display}` : ''
  ...
  const purposeText = `This is the launch-day suite for ${addrDisplay}${cityDisplay}${priceDisplay}. These are the ${SUB_PRODUCERS.length} deliverables that go live on day one: listing description, print flyer, IG single post, agent co-op e-flyer, postcard farm mailer, yard sign rider, listing reveal reel, and TikTok listing tour. Each producer ran in parallel. Review each artifact before publishing. The reel and tour video need Matt approval before going to social.`
  ...
  const html = [
    ...
    `  <title>Listing launch — ${addrDisplay}</title>`,
    `  <style>\n    ${HTML_STYLES}\n  </style>`,
    '</head>',
    '<body>',
    '  <h1>Listing launch</h1>',
    '  <div class="purpose">',
    '    <strong>What this kit is and why it exists</strong>',
    `    ${purposeText}`,
    '  </div>',
    `  <p class="meta">${addrDisplay}${cityDisplay} &middot; ${listing.list_price_display ?? ''} &middot; ${SUB_PRODUCERS.length} deliverables &middot; generated ${new Date().toISOString()}</p>`,
    '  <div class="grid">',
    cards,
    '  </div>',
    '</body>',
  ...
  await writeFile(join(outDir, 'card.json'), JSON.stringify({ producer: PRODUCER, primary_artifact: 'kit.html', notes: 'At-Active launch suite. Purpose lead + thumbnail grid.', data_traces: [], generated_at: new Date().toISOString() }, null, 2))
