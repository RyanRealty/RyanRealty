/**
 * Claim the Tetherow regional-median fleet finding.
 *
 *   npx tsx scripts/loop-claim-tetherow-regional-median.ts
 */
...
const MARKER = '5f0ec58d60988a52e76b8a559ef22f0c'
...
async function main() {
  ...
  const row = rows?.[0]
  if (!row) {
    console.error(`no node for fleet:${MARKER}`)
    process.exit(1)
  }
