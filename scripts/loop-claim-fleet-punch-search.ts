 ...
 *
 *   npx tsx scripts/loop-claim-fleet-punch-search.ts
 */
...
async function main() {
  ...
  const { data, error } = await sb
    ...
    .single()
  if (error || !data?.id) {
    console.error(JSON.stringify({ ok: false, failed: error?.message ?? 'no row' }))
    process.exit(1)
  }
  console.log(JSON.stringify({ ok: true, claimed: [data.id], owner: data.owner_session }, null, 2))
}
