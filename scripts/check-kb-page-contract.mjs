...
import { isResilientMarketFaq } from './lib/kb-market-faq-resilience.mjs'
...
for (const p of kbPages) {
  ...
  if (s.includes('buildMarketFaq')) {
    ...
    // again 2026-08-25 for MARKET_TRUTH D26). The rule and its failing fixtures
    // live in scripts/lib/kb-market-faq-resilience.mjs — including why the
    // vanishing shape `pulse ? buildMarketFaq(...) : null` still fails.
    const resilient = isResilientMarketFaq(s, stripComments(s))
    if (!resilient)
      fails.push(
