 ...
 *
 * Asserts:
 *   - the closed `.menu-overlay { ... }` rule declares opacity:0,
 *     visibility:hidden, AND pointer-events:none.
 *   - the `.menu-overlay.open { ... }` rule re-enables all three
 *     (opacity:1, visibility:visible, pointer-events:auto).
 *
 * 2026-08-10: PublicNav mounts outside .kb-root, so selectors are no longer
 * `.kb-root .menu-overlay` (optional `.kb-root` prefix still accepted).
 *
...
// Find closed rule: a `.menu-overlay {` block that includes opacity:0 (skip the
// token-only block that co-lists .topbar,.menu-overlay for CSS variables).
const closedRe = /(?:\.kb-root\s+)?\.menu-overlay\s*\{([^}]*)\}/g
let closedBody = null
for (const m of css.matchAll(closedRe)) {
  if (/opacity\s*:\s*0/.test(m[1])) {
    closedBody = m[1]
    break
  }
}
...
if (!closedBody) {
  fails.push('could not find the closed `.menu-overlay { ... }` rule with opacity:0')
} else {
  for (const decl of ['opacity:0', 'visibility:hidden', 'pointer-events:none']) {
    if (!has(closedBody, decl)) fails.push(`closed .menu-overlay is missing "${decl}" (transform-only hiding regresses the giant CONTACT ghost)`)
  }
}

if (!open) {
  fails.push('could not find the `.menu-overlay.open { ... }` rule')
} else {
  const body = open[1]
