#!/usr/bin/env node
/**
 * Production accept for the Awbrey Butte 52 / 63 / 62 / 23 four-count finding.
 * Rejects the stale observation when index, city tile, hero, FAQ, Dataset,
 * #homes, and the hydrated map badge share one count and one median, and the
 * hero names Awbrey Butte (not Bend).
 *
 *   node scripts/probe-awbrey-four-counts-prod.mjs
 */
...
const OUT = 'out/awbrey-four-counts-prod'
...
function extract(placeHtml, indexHtml, cityHtml) {
  ...
  return {
    ...
    homesForSaleInBend: /homes for sale in Bend/.test(placeText),
...
async function readHydratedMapCount(browser) {
  ...
  await page.screenshot({ path: `${OUT}/place_map_1280.png`, fullPage: false })
  await page.setViewportSize({ width: 390, height: 844 })
  await page.waitForTimeout(400)
  await page.screenshot({ path: `${OUT}/place_map_390.png`, fullPage: false })
  await page.close()
...
for (const name of names) {
  copyFileSync(`${OUT}/${name}.png`, `${ART}/awbrey_four_${name}.png`)
}
