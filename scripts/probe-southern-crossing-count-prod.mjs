#!/usr/bin/env node
/**
 * Production accept for the Southern Crossing index-1 vs place-23 finding.
 * Rejects the stale observation when index, city tile, hero, FAQ, Dataset,
 * #homes, and the hydrated map badge share one count and one median.
 *
...
function extract(placeHtml, indexHtml, cityHtml) {
  ...
  const cityMedian = cityText.match(/Southern Crossing\s+(\d+)\s+Active\s+\$([\d,]+)/)
  return {
    ...
    cityMedian: cityMedian ? Number(cityMedian[2].replace(/,/g, '')) : null,
    placeHas23Homes: /\b23\s+homes for sale\b/.test(placeText),
    placeHas23Active: /\b23\s+ACTIVE\b/i.test(placeText) || /\b23\s+active listings\b/i.test(placeText),
    placeHasActiveListingsCaps: /ACTIVE LISTINGS/.test(placeHtml),
    indexHas1NearSouthern:
      /Southern Crossing[\s\S]{0,240}1\s+Active|1\s+Active[\s\S]{0,80}Southern Crossing/.test(indexText),
  }
}
...
async function readHydratedMapCount(browser) {
  const page = await browser.newPage({
    viewport: { width: 1280, height: 900 },
    extraHTTPHeaders: CI_PROBE_HEADERS,
  })
  await page.goto(PLACE, { waitUntil: 'domcontentloaded', timeout: 90_000 })
  ...
  const legend = await page.locator('#map .neigh-legend').innerText().catch(() => '')
  const match = legend.match(/(\d+)\s+active listings/i)
  await page.close()
  return { legend: legend.replace(/\s+/g, ' ').trim(), count: match ? Number(match[1]) : null }
}
...
const staleGone =
  !extracted.placeHas23Homes &&
  !extracted.placeHas23Active &&
  !extracted.placeHasActiveListingsCaps &&
  !extracted.indexHas1NearSouthern &&
...
const mapBadge = await readHydratedMapCount(browser)
...
const mapAgrees = mapBadge.count === shared
const ok = countsAgree && mediansAgree && staleGone && mapAgrees

const names = ['place_1280', 'place_390', 'index_tile_1280', 'index_tile_390', 'city_nbh_1280', 'city_nbh_390']
for (const name of names) {
  copyFileSync(`${OUT}/${name}.png`, `${ART}/southern_crossing_count_${name}.png`)
}
