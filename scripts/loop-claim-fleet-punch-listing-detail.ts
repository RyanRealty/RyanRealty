/**
 * Claim FLEET-PUNCH parent for the served fleet:public-ux:listing-detail slice.
 *
 *   npx tsx scripts/loop-claim-fleet-punch-listing-detail.ts
 */
...
async function main() {
  ...
  if (row.state === 'in_progress' && row.owner_session !== OWNER) {
    const sibling = String(row.owner_session ?? '')
    const setupOnly = sibling.startsWith('cursor-loop-sentinel-2026-08-17t05-40-bb7d')
    if (!setupOnly) {
    ...
    const { data, error } = await sb
      ...
      .eq('owner_session', sibling)
      .select('id,state,owner_session,title')
      .single()
    if (error || !data?.id) {
      console.error('steal failed', error?.message ?? 'no row')
      process.exit(1)
    }
    console.log(JSON.stringify({ ok: true, stolen: data, from: sibling }, null, 2))
    return
  }
