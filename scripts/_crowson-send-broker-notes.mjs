...
const SALE_GUID = '1f4436e6-25b8-4b26-84f2-14f0d9e2b81c'
const PDF_PATH = `${ROOT}/tmp/crowson-rd-2026-05-28/phase6/JJ02192025_X_Broker Notes - Transaction Summary.pdf`
const BASE_NAME = 'JJ02192025_X_Broker Notes - Transaction Summary'
const CANONICAL = 'JJ02192025_X_Broker Notes - Transaction Summary.pdf'
const SUBJECT = '[534 Crowson Rd Ashland] Broker Notes - Transaction Summary'
...
console.log('portalEmail:', sale.portalEmail, 'BN activity:', bnAct.activityId, 'existing:', (bnAct.checklistDocs||[]).length)
...
const body = [`From: ${FROM_EMAIL}`,`To: ${sale.portalEmail}`,`Subject: ${SUBJECT}`,'MIME-Version: 1.0',`Content-Type: multipart/mixed; boundary="${boundary}"`,'',`--${boundary}`,'Content-Type: text/plain; charset=utf-8','','Crowson Rd Broker Notes (v2 pass 2026-05-28).','',`--${boundary}`,`Content-Type: application/pdf; name="${BASE_NAME}.pdf"`,`Content-Disposition: attachment; filename="${BASE_NAME}.pdf"`,'Content-Transfer-Encoding: base64','',b64,'',`--${boundary}--`,''].join('\r\n')
...
while (Date.now()-start < 25*60_000 && !newDocId) {
  ...
  if (cand.length) { newDocId = cand[0].id; console.log(`[INGESTED] ${newDocId.substring(0,8)} "${cand[0].fileName}"`) }
  else console.log(`  ...waiting (${Math.round((Date.now()-start)/60000)}m)`)
}
