/**
 * Complete the Awbrey place-page city-grain hero-copy fleet finding.
 *
 *   npx tsx scripts/loop-complete-awbrey-grain.ts <evidence>
 */
...
const NODE_ID = '8d924b82-afd7-4cf8-a4ca-5fa5dd13a05a'

async function main() {
  const evidence = process.argv.slice(2).join(' ').trim()
  if (!evidence) {
    console.error('usage: npx tsx scripts/loop-complete-awbrey-grain.ts <evidence>')
    process.exit(2)
  }
