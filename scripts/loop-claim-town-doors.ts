/**
 * Claim the homepage town-doors fleet finding for this loop session.
 *
 *   npx tsx scripts/loop-claim-town-doors.ts
 */
...
async function main() {
  ...
  const { data: row, error: readErr } = await sb
    ...
    .eq('state', 'open')
    .ilike('title', 'Fleet finding [major]: All six doors%')
    .maybeSingle()
  if (readErr || !row) {
    console.error('read failed', readErr?.message ?? 'no open town-doors finding')
    process.exit(1)
  }
