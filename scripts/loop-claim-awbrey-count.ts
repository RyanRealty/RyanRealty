/**
 * Claim the Awbrey Butte index-vs-place active-count fleet finding.
 *
 *   npx tsx scripts/loop-claim-awbrey-count.ts
 */
...
const OWNER = 'cursor-loop-chain-2026-08-16t16-01-5039'
...
async function main() {
  ...
  const { data: rows, error: readErr } = await sb
    ...
    .eq('state', 'open')
    .ilike('objective', '%e782a439d6407e5e46a4208dce833653%')
  if (readErr) {
  ...
  const row = rows?.[0]
  if (!row) {
    console.error('no open node for fleet:e782a439d6407e5e46a4208dce833653')
    process.exit(1)
  }
  ...
  const { data, error } = await sb
    ...
    .eq('id', row.id)
    .eq('state', row.state)
    .select('id,state,owner_session,title,domain,accept')
    .single()
  if (error || !data?.id) {
