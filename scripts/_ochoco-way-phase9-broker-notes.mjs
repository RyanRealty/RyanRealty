...
const OUT_DIR = `${ROOT}/tmp/ochoco-way-broker-notes`
const FROM_EMAIL = 'matt@ryan-realty.com'

const MANIFEST = [
  {
    saleNumber: 'MB/KS09202024',
    pdfPath: `${OUT_DIR}/MB-KS09202024_X_Broker Notes - Transaction Summary.pdf`,
    baseName: 'MB-KS09202024_X_Broker Notes - Transaction Summary',
    isClosingCycle: true,
    label: 'Closing Stradford',
  },
  {
    saleNumber: 'JB9524',
    pdfPath: `${OUT_DIR}/JB9524_X_Broker Notes - Transaction Summary - Failed Cycle.pdf`,
    baseName: 'JB9524_X_Broker Notes - Transaction Summary - Failed Cycle',
    isClosingCycle: false,
    label: 'Failed Henry',
  },
]
...
for (const entry of MANIFEST) {
  ...
  const subject = `[29500 SE Ochoco Way ${entry.isClosingCycle ? 'Closed' : 'Failed Cycle'} forward] Broker Notes - Transaction Summary (${entry.saleNumber})`
  ...
  const body = [
    ...
    '',
    `Broker Notes - Transaction Summary for 29500 SE Ochoco Way (${entry.saleNumber}).\nGenerated 2026-05-27 audit pass via the skyslope-form-compliance skill.`,
    '',
    `--${boundary}`,
