...
describe('demoStates are the capture the evaluator can actually see', () => {
  ...
  it('prints year-open and source-open specs for market-report-region', () => {
    ...
    expect(specs.some((s) => /year-open/.test(s) && /data-insight-next/.test(s) && /@#market-insights/.test(s) && /click/.test(s))).toBe(true)
    expect(specs.some((s) => /source-open/.test(s) && /v3-mos__animate/.test(s) && /click/.test(s))).toBe(true)
  })
  ...
  it('prints save-open, share-open, and gallery-open specs for listing-detail', () => {
    ...
    expect(specs.some((s) => /share-open/.test(s) && /button\[aria-label/.test(s) && /click/.test(s))).toBe(true)
