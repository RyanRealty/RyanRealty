...
for (const file of pageFiles(ROOT)) {
  ...
  const hasFooter = isV3
    ...
    : /<KbFooter\b/.test(src) || /\bKbFooter\b/.test(code) || /<HoodDFooter\b/.test(src)
  if (!hasFooter) {
    fails.push(
      isV3
        ? `${file} opens the v3 token scope but does NOT render <V3Footer> (shared footer required)`
        : `${file} renders .kb-root but does NOT render <KbFooter> or <HoodDFooter> (shared footer required)`,
    )
  }
