 ...
 *
 *   npx tsx scripts/loop-claim-fleet-punch-place-v10.ts
 */
...
async function main() {
  ...
  if (row.state === 'in_progress' && row.owner_session !== OWNER) {
    console.log(JSON.stringify({ ok: false, alreadyClaimedBy: row.owner_session, updatedAt: row.updated_at }, null, 2))
    process.exit(1)
  }
