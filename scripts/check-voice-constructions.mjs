...
try {
  ...
  // Persist progress, but never churn: this gate runs inside ci:gates on every
  // push, and rewriting only updated_at/files_scanned dirtied EVERY agent's
  // tree after every push (worktrees scan slightly different file sets, so
  // files_scanned flapped too). Skip the write when nothing that tracks
  // grinder progress has changed.
  const meaningful = (s) => {
    const { updated_at, files_scanned, ...rest } = s
    return JSON.stringify(rest)
  }
  const prior = existsSync(STATE_PATH) ? JSON.parse(readFileSync(STATE_PATH, 'utf8')) : null
  if (!prior || meaningful(prior) !== meaningful(state)) {
