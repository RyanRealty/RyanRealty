...
/**
 * The reports hub's one on-page ask, as a barrel Sheet.
 *
 * THE CAPTURE CONTRACT IS UNCHANGED. It calls submitMarketPageInquiry in
 * app/housing-market/actions.ts with variant 'inquiry' and fields name / email /
 * message. The payload is an object literal rather than LeadCapturePayload so
 * this file imports nothing from the legacy flat register.
 *
 * Inquiry, not a listing-alert signup: no recurring email, so the alert
 * disclosure gate does not apply. Do not hardcode company: ''.
 *
 * Client because the step and the answers are visitor-caused state.
 */
...
const ASK_STEPS: readonly V3SheetStep[] = [
  {
    ...
    field: {
      ...
      rows: 4,
      maxLength: 1200,
      placeholder: 'A city, a weekly report, or a number on this page',
    },
...
export function ReportsInquirySheet() {
  ...
  return (
    <V3Sheet
      id="ask"
      heading="Ask a broker about these reports"
