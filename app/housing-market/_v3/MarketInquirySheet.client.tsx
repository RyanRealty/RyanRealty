...
/**
 * The market hub's one on-page ask, as a barrel Sheet.
 *
 * THE CAPTURE CONTRACT IS UNCHANGED. It calls the same route-local server action
 * the KB page called, `submitMarketPageInquiry` in app/housing-market/actions.ts,
 * with the same payload shape and the same three field names: `name`, `email`,
 * `message`, under `variant: 'inquiry'`. Nothing about what reaches
 * submitPageCTA (CRM person and event, Meta CAPI, canonical tagging, GA4 mirror)
 * moved. Only the markup did. The payload is written as an object literal rather
 * than typed through LeadCapturePayload so this file imports nothing from the
 * legacy flat register; TypeScript matches it structurally against the action's
 * InquiryPayload, so a renamed field is still a compile error.
 *
 ...
 * value and always change together, which is what keeps the sheet off its
 * "that step is not available" path: it never sees an id that names no step.
 * While the send is in flight the only rendered step is the terminal one, so a
 * second click cannot fire a second submit.
 */
...
const ASK_STEPS: readonly V3SheetStep[] = [
  {
    ...
    field: {
      ...
      rows: 4,
      maxLength: 1200,
      placeholder: 'Buying, selling, or reading the numbers for a specific place',
    },
...
export function MarketInquirySheet() {
  ...
  return (
    <V3Sheet
      id="ask"
      heading="Ask a broker about the Central Oregon market"
