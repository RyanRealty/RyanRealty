...
/**
 * The region report's one on-page ask, as a barrel Sheet.
 *
 * THE CAPTURE CONTRACT IS UNCHANGED. It calls the same server action the KB page's
 * LeadCaptureBlock called, `submitMarketPageInquiry` in app/housing-market/actions.ts,
 * with the same payload shape and the same three field names: `name`, `email`,
 * `message`, under `variant: 'inquiry'`. Nothing about what reaches submitPageCTA
 * (CRM person and event, Meta CAPI, canonical tagging, GA4 mirror) moved. Only the
 * markup did. The payload is written as an object literal rather than typed through
 * LeadCapturePayload so this file imports nothing from the legacy flat register;
 * TypeScript matches it structurally against the action's InquiryPayload, so a
 * renamed field is still a compile error. Three fields, no phone, so the page still
 * ships no SMS-consent surface.
 *
 * This file is the region route's own, not an import of the hub's sheet: each route
 * owns its ask, and a shared component would couple two pages that are migrating
 * separately.
 *
 ...
 * value and always change together, which keeps the sheet off its "that step is not
 * available" path: it never sees an id that names no step. While the send is in
 * flight the only rendered step is the terminal one, so a second click cannot fire a
 * second submit.
 */
...
const ASK_STEPS: readonly V3SheetStep[] = [
  {
    ...
    field: {
      ...
      rows: 4,
      maxLength: 1200,
      placeholder: 'Buying, selling, or reading the numbers for a specific city',
    },
...
export function RegionInquirySheet() {
  ...
  return (
    <V3Sheet
      id="ask"
      heading="Questions about the Central Oregon market?"
      eyebrow="Talk to a broker"
      steps={steps}
      currentStepId={currentStepId}
      showProgress={status === 'asking'}
      onStepChange={(id) => {
        // Back out of the failure screen returns the sheet to the questions.
