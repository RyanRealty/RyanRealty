...
import { syncSparkListings, syncListingHistory, syncPhotosOnly } from '../../actions/sync-spark'
import type { SyncSparkResult, SyncHistoryResult, SyncPhotosResult } from '../../actions/sync-spark'
    ...
    <div className="rounded-xl border-2 border-zinc-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            disabled={listingsRunning || historyRunning || photosRunning}
            className="rounded-lg bg-zinc-800 px-5 py-2.5 text-sm font-semibold text-white hover:bg-zinc-900 disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg bg-zinc-50 p-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Listings sync</p>
          ...
          {listingsPagesTotal != null && listingsRunning && (
            <p className="mt-0.5 text-xs text-zinc-500">Page {listingsPagesDone} of {listingsPagesTotal.toLocaleString()}</p>
          )}
          {listingsError && <p className="mt-1 text-sm text-red-600">{listingsError}</p>}
        </div>
        <div className="rounded-lg bg-zinc-50 p-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">History sync</p>
          <p className="mt-1 font-mono text-sm text-zinc-900">
            {historyState === 'idle' && '—'}
          ...
          {historyTotalListings != null && historyRunning && (
            <p className="mt-0.5 text-xs text-zinc-500">{historyListingsProcessed.toLocaleString()} of {historyTotalListings.toLocaleString()} listings</p>
          )}
          {historyError && <p className="mt-1 text-sm text-red-600">{historyError}</p>}
        </div>
        <div className="rounded-lg bg-zinc-50 p-3">
          <p className="text-xs font-medium uppercase tracking-wider text-zinc-500">Photos sync</p>
          <p className="mt-1 font-mono text-sm text-zinc-900">
            {photosState === 'idle' && '—'}
          ...
          {photosPagesTotal != null && photosRunning && (
            <p className="mt-0.5 text-xs text-zinc-500">Page {photosPagesDone} of {photosPagesTotal.toLocaleString()}</p>
          )}
