...
describe('SITE-111 sell catalog install', () => {
  it('imports the three replaceWith sources on the route _v3 set', () => {
    ...
    expect(button).toContain('justify-around')
    expect(button).toContain('cx="16" cy="14"')
    expect(form).toContain('justify-self-start')
    expect(form).not.toMatch(/sell-stage-submit mt-4 w-full/)
    expect(form).toContain('sellStreetFrameUrl')
    expect(form).toContain('mountSellStreetFrame')
    expect(form).toContain('sell-stage-street')
