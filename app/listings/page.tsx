...
import { getListings, getListingKeysWithRecentPriceChange } from '../actions/listings'
...
export default async function ListingsPage({
  ...
  const { view, minPrice, maxPrice, beds, baths, minSqFt, propertyType, sort, includeClosed } = p
  ...
  const [listings, priceChangeKeys, session] = await Promise.all([
    getListings({
      limit: 200,
      ...
      sort: sort === 'newest' || sort === 'oldest' || sort === 'price_asc' || sort === 'price_desc' ? sort : undefined,
    }),
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <header className="mb-6">
        ...
        <p className="mt-1 text-zinc-600">
          {listings.length} homes for sale. View on{' '}
