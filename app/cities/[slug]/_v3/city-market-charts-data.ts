...
/** Chart Room Relate peer: Bend, unless this page is Bend. */
export function relatePeerSlug(subjectSlug: string): string {
  return subjectSlug === 'bend' ? 'redmond' : 'bend'
}

/**
 * Two live median-close series on one Chart Room line: this city and its peer.
 * Years must exist on both sides. Missing years are omitted, never filled.
 */
export function buildRelateCard(
  yearRows: readonly CityYearPricingRow[],
  opts: {
    subjectSlug: string
    subjectName: string
  ...
  const subject = yearRows
    .filter(
      (r) =>
        r.citySlug === opts.subjectSlug &&
        r.medianClose != null &&
        r.medianClose > 0 &&
  ...
  const peer = yearRows
    .filter(
      (r) =>
        r.citySlug === opts.peerSlug &&
        r.medianClose != null &&
        r.medianClose > 0 &&
        r.year < opts.currentYear,
    )
    .sort((a, b) => a.year - b.year)
  if (subject.length < 2 || peer.length < 2) return null

  const peerYears = new Set(peer.map((r) => r.year))
  const years = subject.map((r) => r.year).filter((y) => peerYears.has(y))
  if (years.length < 2) return null

  const subjectByYear = new Map(subject.map((r) => [r.year, r]))
  const peerByYear = new Map(peer.map((r) => [r.year, r]))
  const peerName = peer[0]!.city
  const first = years[0]!
  ...
  return {
    ...
    kind: 'line',
    marks: true,
    title: `${opts.subjectName} beside ${peerName}`,
    displayLine: `Median detached close, ${opts.subjectName} and ${peerName}, ${first}–${last}.`,
    series: [
      {
        name: v3Text(opts.subjectName),
        points: years.map((year) => {
          const row = subjectByYear.get(year)!
          return {
            value: row.medianClose!,
            tick: v3Text(String(year)),
            label: v3Text(money(row.medianClose!)),
            at: year,
          }
        }),
      },
      {
        name: v3Text(peerName),
        points: years.map((year) => {
          const row = peerByYear.get(year)!
          return {
            value: row.medianClose!,
            tick: v3Text(String(year)),
            label: v3Text(money(row.medianClose!)),
    ...
    source:
      `sale_pricing_facts detached closes, aggregated by the city_year_pricing function. ` +
      `Median close price per calendar year for ${opts.subjectName} (${opts.subjectSlug}) and ` +
      `${peerName} (${opts.peerSlug}). Years with at least 3 closings on both sides; ` +
      `${opts.currentYear} is still in progress and not charted. Close price 0.1–10x of last ask, 300+ sqft. ` +
      `The second series is the Chart Room relate overlay, not a fitted line.`,
    updatedAt: opts.factsAsOf,
  }
}
