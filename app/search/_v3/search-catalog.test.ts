...
describe('SITE-110 search catalog install', () => {
  it('route page and _v3 import the installed catalog files', () => {
    ...
    expect(command).toContain('onOpenMorph')
    expect(command).toContain('sr-only')
    expect(command).toContain('aria-hidden')
    expect(command).not.toContain('aria-label="Search command"')
    expect(command).not.toContain('CommandDialog')
    ...
    expect(morph).toContain('items={[...items]}')
    expect(morph).toContain('placeholder={placeholder}')
    expect(morph).toContain('resultsMaxHeight={220}')
    expect(morph).toContain('triggerAriaLabel="Search command"')
    expect(morph).toContain('No places or homes match that.')
    expect(morph).not.toContain('Find components')
    expect(morph).not.toContain('Project Folder')
    ...
    expect(sheet).toContain('What kind of home?')
    expect(sheet).toContain('eyebrow="Filters"')
    expect(sheet).not.toContain('eyebrow="This search"')
    expect(sheet).not.toContain("from '@/components/motion/range-slider'")
    expect(sheet).not.toContain('<RangeSlider')
    expect(sheet).toContain("from '@/components/ui/checkbox'")
    expect(sheet).toContain('<Checkbox')
    expect(sheet).toContain('House')
    expect(sheet).toContain('Condo')
    expect(sheet).not.toContain("kind: 'facts'")
    expect(sheet).not.toContain("kind: 'select'")
    ...
    expect(atlas).toContain('priceScrub="none"')
    expect(atlas).toContain('showTypes={false}')
  ...
  it('search dock does not control MorphingSearch open — the catalog owns the morph', () => {
    ...
    expect(filters).toContain('places.slice(0, 2)')
    expect(filters).toMatch(/open=\{morphOpen\}/)
    expect(filters).toContain('onOpenChange')
    expect(filters).toContain('onOpenMorph')
    expect(filters).toContain('srch-morph-reserve')
