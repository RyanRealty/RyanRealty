...
describe('SITE-110 search catalog install', () => {
  it('route page and _v3 import the installed catalog files', () => {
    ...
    expect(price).toContain('maxAriaLabel="Maximum ask"')
    expect(price).toContain('values={[loIdx, hiIdx]}')
    expect(price).toContain('Min ask')
    expect(price).toContain('Max ask')
    expect(price.match(/<RangeSlider/g)?.length).toBe(1)
    expect(morph).toContain("from '@/components/motion/morphing-search'")
    expect(morph).toContain('<MorphingSearch')
    expect(morph).toContain('placeholder="Find components"')
    expect(morph).toContain('Project Folder')
    expect(morph).toContain('Motion components')
    ...
    expect(sheet).toContain('showProgress')
    expect(sheet).not.toContain('showProgress={false}')
    expect(sheet).toContain('showEcho')
    expect(sheet).toContain('What ask should this search include?')
    expect(sheet).toContain("from '@/components/motion/range-slider'")
    expect(sheet).toContain('<RangeSlider')
    expect(sheet).toContain('values={[loIdx, hiIdx]}')
    expect(sheet).toContain('aria-label="Minimum ask"')
    expect(sheet).toContain('maxAriaLabel="Maximum ask"')
    expect(sheet.match(/<RangeSlider/g)?.length).toBe(1)
  ...
  it('catalog sources keep the official demo markers', () => {
    ...
    expect(range).toContain('values?: readonly [number, number]')
    expect(range).toContain('maxAriaLabel')
    expect(range).toContain('function DualTickRange')
