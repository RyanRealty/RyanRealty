 ...
 * reconciliation gate (CLAUDE.md §0: any monthly market missing more than 1%
 * of the closings Spark holds stops the edition), renders the PDF under THE
 ...
 * ?month=YYYY-MM publishes a specific month (a late re-run).
 *
  ...
  try {
    ...
    if (outcome.status === 'held') {
      await queueBrokerHealthAlert({
        ...
        body: `The ${monthLabel(month)} market report is held as a draft: our closed sales disagree with the MLS by more than 1%. ${outcome.holdReason?.slice(0, 200) ?? ''}`,
