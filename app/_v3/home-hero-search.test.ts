...
describe('homepage hero search uses the public search stack', () => {
  ...
  it('mounts HomeHeroSearch on the Stage; recommended homes before doors (Redfin lock)', () => {
    ...
    expect(fieldAt).toBeGreaterThan(searchAt)
    expect(doorsAt).toBeGreaterThan(fieldAt)
  })
...
describe('homepage Field stays on the barrel', () => {
  ...
  it('homepage has no Atlas and no explore map; Field is the photographed list (Redfin lock)', () => {
    expect(PAGE).not.toMatch(/<V3Atlas/)
    expect(PAGE).not.toMatch(/<HomeExploreMap/)
  ...
  it('opens Stage → Field → Doors → faces → places → proof (Redfin lock)', () => {
    expect(PAGE).toMatch(/<V3Stage/)
    expect(PAGE).not.toMatch(/<V3Atlas/)
