...
describe('homepage hero search uses the public search stack', () => {
  ...
  it('adapts catalog modules into house primitives on the hero and rails', () => {
    ...
    expect(RAIL_CLIENT).not.toContain('AnimatedNumber')
    expect(RAIL_CLIENT).not.toContain('home-rail__live')
  ...
  it('prints the ask on the Card header, not on the photograph and not via v3-lrow', () => {
    ...
    expect(RAIL_CLIENT).not.toContain('v3-lrow--card')
...
describe('homepage house rails use SplitCardMedia cards', () => {
  ...
  it('does not print the leftover inventory caption', () => {
    ...
    expect(RAIL_CLIENT).not.toContain('homes for sale across Central Oregon')
  ...
  it('opens Stage → rails → featured community → Doors → faces → places → proof (expanded Home lock)', () => {
    ...
    expect(RAIL_CLIENT).not.toContain('ListingTourOverlay')
    expect(RAIL_CLIENT).toContain('tourUrl={card.tourUrl}')
