...
describe('SITE-163 /about faces fold', () => {
  ...
  it('loads the firm closing set, not a one-row tease', () => {
    ...
    expect(CLOSINGS).toContain('about-closings__card-link')
    expect(CLOSINGS).toContain('const href = row.href.trim()')
    expect(CLOSINGS).toContain('<Link href={href} className="about-closings__card-link">')
    expect(CLOSINGS).not.toContain('<Link href={row.href}>See this closing</Link>')
