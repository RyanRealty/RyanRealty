...
export async function loadGlobalActivity(input: {
  types?: string[]
  before?: string | null
  /** Requested broker filter ('all' or a slug). A restricted broker is forced to their own. */
  broker?: string | null
  ...
  const brokerScope = resolveActivityScope(scopeBroker(access), input.broker)
  return getGlobalActivityFeed({ types, before: input.before ?? null, limit: 50, brokerScope })
}
