...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // bids for the family's name. The families read is time-limited like every
  // other read here (P3): a slow read titles the plat as a plain plat.
  const families = await withTimeoutFallback(getPlatFamilies(), null, 5000, 'sub:meta-families')
  const role = families ? platFamilyRole(families, slug) : null
  const cityName =
    registryMatch?.city ??
